...
@php
    ...
    $funSelected = isset($_GET['filFunction']) ? $_GET['filFunction'] : null;
    $subFunSelected = isset($_GET['filSubFunction']) ? $_GET['filSubFunction'] : null;
@endphp
...
<script type="text/javascript">
    ...
    /*Filter Function on change Module*/
    $("#filModule").change(function(event) {
        var moduleId = $(this).val();
        var csrf = "{{csrf_token()}}";
        $.ajax({
            url: './gnrGetFunctionBaseOnModule',
        ...
        .done(function(functions) {
            ...
            $("#filFunction").empty();
            $("#filFunction").append("<option value=''>--All--</option>");
            $.each(functions, function(index, obj) {
                 $("#filFunction").append("<option value='"+obj.id+"'>"+obj.name+"</option>");
            });
            console.log("success");
        })
        .fail(function() {
