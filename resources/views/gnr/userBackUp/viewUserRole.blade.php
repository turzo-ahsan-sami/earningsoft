    ...
    <div class="">
      <div class="panel panel-default" style="background-color:#708090;">
        ...
        <div>
          ...
          {
            $("#gnrUserRoleView").dataTable().yadcf([
                ...
                <tbody> 
                  ...
                  @foreach($userRoleLists as $userRoleList)
                    <tr class="item{{$userRoleList->id}}">
                      ...
                      <td>
                          <?php
                            $UserName = DB::table('users')->select('name')->where('id',$userRoleList->userId)->first();
                          ?>
                          {{$UserName->name}}
                      </td>
                      <td>
                          <?php
                            $roleName = DB::table('gnr_role')->select('name')->where('id',$userRoleList->roleId)->first();
                          ?>
                          {{$roleName->name}}
                      </td>
                      <td class="text-center" width="100" style="">
                      <form action="{{ url('getCheckedValue/') }}" method="post"  style="max-height: 0px; align:center; padding-bottom:7px">
