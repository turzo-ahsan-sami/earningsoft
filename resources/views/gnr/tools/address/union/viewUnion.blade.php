      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">
              <a href="{{url($route['path'].'/addUnion/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add Union</a>
          </div>
                        ...
                        @foreach($unions as $union)
                          <tr class="item{{$union->id}}">
                            ...
                            <td style='text-align: left; padding-left: 5px;'>
                                <?php
                                  $upzillaName = DB::table('gnr_upzilla')->select('name')->where('id', $union->upzillaId)->first();
                                  // dd($upzillaName->upzilla_name, $union);
                                ?>
                                @if ($upzillaName != null)
                                    {{ $upzillaName->name }}
                                @else
                                  @php
                                      $upzillaName = DB::table('gnr_upzilla')->select('name')->where('id', $union->upzillaId)->first();
                                      // dd($union->upzillaId, $upzillaName);
                                  @endphp
                                @endif
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      if (data.responseTitle == 'Warning!') {
        ...
        toastr.success(data.responseText, data.responseTitle);
        $('.item' + $('.id').text()).remove();
      }
    },
    error: function (data) {
