        ...
        <div class="card-body">
            <form action="{{ route("admin.products.store") }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group {{ $errors->has('modules') ? 'has-error' : '' }}">
                    ...
                    <select name="modules[]" id="modules" class="form-control select2" multiple="multiple" required>
                        @foreach($modules as $id => $modules)
                            <option value="{{ $id }}">{{ $modules }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('numberOfUser') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="numberOfUser" name="numberOfUser" class="form-control" value="{{ old('numberOfUser', isset($product) ? $product->numberOfUser : '') }}">
                ...
                <div class="form-group {{ $errors->has('renewal_charge') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="renewal_charge" name="renewal_charge" class="form-control" renewal_charge="{{ old('renewal_charge', isset($plan) ? $plan->renewal_charge : '') }}" required>
                ...
                <div class="form-group  {{ $errors->has('features') ? 'has-error' : '' }}">
                    ...
                    <div class="row">
                        {{-- input field --}}
                        <div class="col-sm-12">
                            <input type="text" id="features" name="features[]" class="form-control" value="{{ old('features', isset($product) ? $product->features : '') }}">
                        </div>
                    </div>
                ...
                <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                    ...
                    <input type="file" id="image" name="image" class="form-control" value="{{ old('logo', isset($banner) ? $banner->image : '') }}">
                    @if($errors->has('image'))
                        <em class="invalid-feedback">
                            {{ $errors->first('image') }}
                        </em>
