...
<div id="tableDiv">
    <table id="famsAssetRegisterReportTable" class="table table-striped table-bordered" style="color:black;font-size:11px;margin-bottom:50px;border-collapse: collapse;" border= "1px solid ash;" cellpadding="0" cellspacing="0">
        ...
        <tbody>
            ...
            @endphp
            @if($branchObj==$product->branchId || $transferedOutProduct->count()>0)
      ...
      $isTransferdInProduct = DB::table('fams_tra_transfer')->where('productId',$product->id)->where('branchIdTo',$branchObj)->where('transferDate','>=',$startDate)->orderBy('id','desc')->first();
...
<script type="text/javascript">
    jQuery(document).ready(function($) {
      ...
      today = dd+'-'+mm+'-'+yyyy;
        ...
        let a = $("<a />", {
          href: url,
          download: "Fixed Assets Register Report_"+ today + ".xls"}).appendTo("body").get(0).click();
        e.preventDefault();
    });
