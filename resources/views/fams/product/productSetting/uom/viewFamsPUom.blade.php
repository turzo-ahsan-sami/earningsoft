      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">
              <a href="{{url('addFamsPUom/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add UOM</a>
          </div>
        ...
        <div>
          ...
          {
            /*$("#famsProUomView").dataTable().yadcf([
              
            ]);*/
            $("#famsProUomView").dataTable({
               "oLanguage": {
          ...
          <table class="table table-striped table-bordered" id="famsProUomView">
            <thead>
                  <tr>
                    <th width="30">SL#</th>
                    <th>Name</th>
                ...
                <tbody>
                    ...
                    @php                    
                    $isBelongToProduct = DB::table('fams_product')->where('uomId',$productUom->id)->value('id');                   
                  @endphp
                      <tr class="item{{$productUom->id}}">
                        <td class="text-center slNo">{{++$no}}</td>
                        <td style="text-align: left; padding-left: 15px;">{{$productUom->name}}</td>
                        <td class="text-center" width="80">
                          ...
                          <a id="deleteIcone" href="javascript:;" class="delete-modal" data-id="{{$productUom->id}}" @php if($isBelongToProduct>0){echo "style=\"pointer-events: none;cursor: not-allowed;\"";} @endphp>
      ...
      success: function(data){
        ...
        $('.item'+data["productUom"].id).replaceWith(
                                    "<tr class='item"+data["productUom"].id+"'><td class='text-center slNo'>"+ data.slno + 
                                                                    ...
                                                                    "</td><td class='text-center'><a href='javascript:;' class='edit-modal' data-id='" + data["productUom"].id + "' data-name='" + data["productUom"].name + "' data-slno='" + data.slno + "'><span class='glyphicon glyphicon-edit'></span></a>\xa0\xa0\xa0<a href='javascript:;' class='delete-modal' data-id='" + data["productUom"].id + "'><span class='glyphicon glyphicon-trash'></span></a></td></tr>");
...
$(document).on('click', '.delete-modal', function() {
  if(hasAccess('deleteFamsPuomItem')){
    $('#MSGE').empty();
    $('#MSGS').empty();
