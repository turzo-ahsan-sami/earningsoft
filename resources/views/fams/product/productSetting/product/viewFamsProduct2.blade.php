                ...
                <div class="">
                    <div class="panel panel-default" style="background-color:#708090;">
                        ...
                        <div class="form-horizontal form-groups" id="filtering-group">
                            ...
                            {!! Form::open(['url' => 'famsGetFilteredProduct']) !!}
                        ...
                        <div class="col-md-2">
                                <div class="form-group" style="font-size: 13px; color:black">
                                    ...
                                    <div class="col-sm-12">
                                     @php
                                        $project = array('' => '') + DB::table('gnr_project')->pluck('name','id')->all();
                                    @endphp
                                    @if($projectSelected)
                                    {!! Form::select('filterProjectId', ($project), $projectSelected, array('class'=>'form-control', 'id' => 'filter-project-name')) !!}
                                    @elseif(Session::has('projectSelected'))
                                    {!! Form::select('filterProjectId', ($project),Session::get('projectSelected'), array('class'=>'form-control', 'id' => 'filter-project-name')) !!}
                                    @else
                                        {!! Form::select('filterProjectId', ($project), null, array('class'=>'form-control', 'id' => 'filter-project-name')) !!}
                        ...
                        <div class="col-md-2">
                                <div class="form-group" style="font-size: 13px; color:black">
                                    ...
                                    <select name="filterbranchId" id="filter-branch-name" class="form-control">
                                        <option value=""></option>
                                        ...
                                        @elseif(Session::has('branchSelected'))
                                        <option value="{{$branch->id}}" @if($branch->id==Session::get('branchSelected')) {{"selected=selected"}} @endif >{{str_pad($branch->branchCode, 3, "0", STR_PAD_LEFT ).'-'.$branch->name}}</option>
                                        @else
                        ...
                        <div class="panel-body panelBodyView">
                            ...
                            <div>
                                <script type="text/javascript">
                                    jQuery(document).ready(function($)
                                    {
                                       ...
                                       /* var oTable;
                                        oTable = $('#famsProductTable').dataTable(); */

                                        /* $('#filter-product-name').change( function() { 
                                        oTable.fnFilter( $(this).find(":selected").text(),5 );
                                        });

                                          $('#filter-branch-name').change( function() { 
                                        oTable.fnFilter( $(this).find(":selected").val(),4 );
                                        });*/
                            ...
                            <table class="table table-striped table-bordered" id="famsProductTable">
                                <thead>
                                <tr>
                                    <th width="32">SL#</th>
                                    <th width="100"  style="padding-left: 2px;padding-right: 2px;">Purchase Date</th>
                                    <th width="100">Product Name</th>
                                    <th>Product ID Number</th>
                                    <th>Branch Location</th>
                                    <th width="100">Project</th>
                                    <th width="100">Project Type</th>
                                    <th width="100">Dep. Opening Balance</th>                                    
                                    <th width="75">Dep. Rate (%)</th>
                                    <th width="60">Cost Price</th>
                                    <th id="details" width="50" style="pointer-events:none;">Action</th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php $no=0; ?>
                                @foreach($products as $product)
                                    <tr class="item{{$product->id}}">
                                        ...
                                        @php
                                            $additionalCharge = DB::table('fams_additional_charge')->where('productId',$product->id)->value('amount');
                                            if(!$additionalCharge){
                                                $additionalCharge = 0;
                                            }
                                        ...
                                        @endphp
                                            ...
                                            <a href="javascript:;" class="view-modal" productId="{{$product->id}}" productName="{{$product->name}}" productCode="{{$prefix.$product->productCode}}" branchName="{{$BranchName}}" projectName="{{$projectName}}" projectTypeName="{{$projectTypeName}}" purchaseDate="{{$purchaseDate}}" warranty="{{$warranty}}" warrantyExpireDate="{{$warrantyExpireDate}}" serviceWarranty="{{$serviceWarranty}}" serviceWarrantyExpireDate="{{$serviceWarrantyExpireDate}}" description="{{$product->description}}" costPrice="{{$product->costPrice}}" additionalCost="{{$product->additionalCost1}}" vatTax="{{$product->additionalCost2}}" totalCost="{{$product->totalCost}}" addCharge="{{$additionalCharge}}" grandTotalCost="{{$grandTotalCost}}" usefulLife="{{$usefulLife}}" resaleValue="{{$product->resellValue}}" depOpeningBalance="{{$product->depreciationOpeningBalance}}" depPerYear="{{$product->depreciationAmountPerYear}}" depPercentage="{{$product->depreciationPercentage}}" productImage="{{$product->productImage}}" warrantyCardImage="{{$product->warrantyCardImage}}" billImage="{{$product->billImage}}" additionalVoucherImage="{{$product->additionalVoucherImage}}">
                                                <i class="fa fa-eye" aria-hidden="true"></i>
                                              </a>&nbsp; 
                                              <a href="javascript:;" class="edit-modal" productId="{{$product->id}}" productNameId="{{$product->productNameId}}" productCode="{{$prefix.$product->productCode}}" branchId="{{$product->branchId}}" projectId="{{$product->projectId}}" projectTypeId="{{$product->projectTypeId}}" supplierId="{{$product->supplierId}}" groupId="{{$product->groupId}}" categoryId="{{$product->categoryId}}" subCategoryId="{{$product->subCategoryId}}" productTypeId="{{$product->productTypeId}}" brandId="{{$product->brandId}}" modelId="{{$product->modelId}}" sizeId="{{$product->sizeId}}" colorId="{{$product->colorId}}" uomId="{{$product->uomId}}"  purchaseDate="{{$purchaseDate}}" warrantyYear="{{$product->warrantyYear}}" warrantyMonth="{{$product->warrantyMonth}}" warrantyExpireDate="{{$warrantyExpireDate}}" serviceWarrantyYear="{{$product->serviceWarrantyYear}}" serviceWarrantyMonth="{{$product->serviceWarrantyMonth}}" serviceWarrantyExpireDate="{{$serviceWarrantyExpireDate}}" description="{{$product->description}}" costPrice="{{$product->costPrice}}" additionalCost="{{$product->additionalCost1}}" vatTax="{{$product->additionalCost2}}" totalCost="{{$product->totalCost}}" addCharge="{{$additionalCharge}}" grandTotalCost="{{$grandTotalCost}}" usefulLifeYear="{{$product->usefulLifeYear}}" usefulLifeMonth="{{$product->usefulLifeMonth}}" resaleValue="{{$product->resellValue}}" depOpeningBalance="{{$product->depreciationOpeningBalance}}" depPerYear="{{$product->depreciationAmountPerYear}}" depPercentage="{{$product->depreciationPercentage}}" productImage="{{$product->productImage}}" warrantyCardImage="{{$product->warrantyCardImage}}" billImage="{{$product->billImage}}" additionalVoucherImage="{{$product->additionalVoucherImage}}" isEditable="{{$isEditable}}">
    ...
    $(document).ready(function() {
        $(document).on('click', '.view-modal', function() {
            ...
            $.ajax({
                ...
                success: function( data ){
                    ...
                    $("#viewModalProductName").append('Product Name: '+thisModal.attr('productName'));
                    $("#viewModalProductCode").val(thisModal.attr('productCode'));
                    ...
                    $("#viewModalBranchName").val(thisModal.attr('branchName'));
                    $("#viewModalProjectName").val(thisModal.attr('projectName'));
                    $("#viewModalProjectTypeName").val(thisModal.attr('projectTypeName'));
                    $("#viewModalPurchaseDate").val(thisModal.attr('purchaseDate'));
                    $("#viewModalWarranty").val(thisModal.attr('warranty'));
                    $("#viewModalWarrantyExpireDate").val(thisModal.attr('warrantyExpireDate'));
                    $("#viewModalServiceWarranty").val(thisModal.attr('serviceWarranty'));
                    $("#viewModalServiceWarrantyExpireDate").val(thisModal.attr('serviceWarrantyExpireDate'));
                    $("#viewModalDescription").val(thisModal.attr('description'));
                    $("#viewModalCostPrice").val(thisModal.attr('costPrice'));
                    $("#viewModalAddCost").val(thisModal.attr('additionalCost'));
                    $("#viewModalTaxVat").val(thisModal.attr('vatTax'));
                    $("#viewModalAddCharge").val(thisModal.attr('addCharge'));
                    $("#viewModalTotalCost").val(thisModal.attr('grandTotalCost'));
                    $("#viewModalUsefulLife").val(thisModal.attr('usefulLife'));
                    $("#viewModalResaleValue").val(thisModal.attr('resaleValue'));
                    $("#viewModalDepOpeningBal").val(thisModal.attr('depOpeningBalance'));
                    $("#viewModalDepPerYear").val(thisModal.attr('depPerYear'));
                    $("#viewModalDepPercentage").val(thisModal.attr('depPercentage'));                    
                    $("#viewModalProductImage").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('productImage'));
                    $("#viewModalWarrantyCardImage").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('warrantyCardImage'));
                    $("#viewModalBillImage").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('billImage'));
                    $("#viewModalAddVoucherImage").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('additionalVoucherImage'));
        ...
        $(document).on('click', '.edit-modal', function() {
            var thisModal = $(this);
                    ...
                    $("#editModalProductId").val(thisModal.attr('productId'));
                    $("#editModalProductName").val(thisModal.attr('productNameId'));
                    $("#editModalProductCode").val(thisModal.attr('productCode'));
                    $("#EMinitialProductCode").val(thisModal.attr('productCode'));
                    $("#editModalSupplierId").val(thisModal.attr('supplierId'));
                    $("#editModalGroup").val(thisModal.attr('groupId'));
                    $("#editModalCategory").val(thisModal.attr('categoryId'));
                    $("#editModalSubCategory").val(thisModal.attr('subCategoryId'));
                    $("#editModalProductType").val(thisModal.attr('productTypeId'));
                    $("#EMinitialProductType").val(thisModal.attr('productTypeId'));
                    $("#editModalBrand").val(thisModal.attr('brandId'));                    
                    $("#editModalModel").val(thisModal.attr('modelId'));
                    $("#editModalSize").val(thisModal.attr('sizeId'));
                    $("#editModalColor").val(thisModal.attr('colorId'));
                    $("#editModalUOM").val(thisModal.attr('uomId'));

                    $("#editModalBranch").val(thisModal.attr('branchId'));
                    $("#EMinitialBranch").val(thisModal.attr('branchId'));
                    $("#editModalProject").val(thisModal.attr('projectId'));
                    $("#EMinitialProject").val(thisModal.attr('projectId'));
                    $("#editModalProjectType").val(thisModal.attr('projectTypeId'));

                    $("#editModalPurchaseDate").val(thisModal.attr('purchaseDate'));
                    $("#editModalWarrantyYear").val(thisModal.attr('warrantyYear'));
                    $("#editModalWarrantyMonth").val(thisModal.attr('warrantyMonth'));
                    $("#editModalWarrantyExpireDate").val(thisModal.attr('warrantyExpireDate'));
                    $("#editModalServiceWarrantyYear").val(thisModal.attr('serviceWarrantyYear'));
                    $("#editModalServiceWarrantyMonth").val(thisModal.attr('serviceWarrantyMonth'));
                    $("#editModalSeviceWarrantyExpireDate").val(thisModal.attr('serviceWarrantyExpireDate'));

                    $("#editModalDescription").val(thisModal.attr('description'));
                    $("#editModalUsefulLifeYear").val(thisModal.attr('usefulLifeYear'));
                    $("#editModalUsefulLifeMonth").val(thisModal.attr('usefulLifeMonth'));

                    $("#editModalCostPrice").val(thisModal.attr('costPrice'));
                    $("#editModalAdditionalCostA").val(thisModal.attr('additionalCost'));
                    $("#editModalAdditionalCostB").val(thisModal.attr('vatTax'));
                    $("#editModalAdditionalCharge").val(thisModal.attr('addCharge'));
                    $("#editModalTotalCost").val(thisModal.attr('grandTotalCost'));
                    $("#editModalResellValue").val(thisModal.attr('resaleValue'));
                    $("#editModalDepOpeningBalance").val(thisModal.attr('depOpeningBalance'));
                    $("#editModalDepAmountPerYear").val(thisModal.attr('depPerYear'));
                    $("#editModalDepPercentage").val(thisModal.attr('depPercentage')); 

                    $("#editModalProductImageShow").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('productImage'));
                    $("#editModalWarrantyImageShow").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('warrantyCardImage'));
                    $("#editModalBillImageShow").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('billImage'));
                    $("#editModalAdditionalVoucherImageShow").attr('src',"{{asset('/images/fams/product/')}}/"+thisModal.attr('additionalVoucherImage'));
