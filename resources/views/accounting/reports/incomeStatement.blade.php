...
//     true => "d"
// );
// dd($voucherIdMatchedByCurrMonth);
 //var_dump($array);
...
<?php
    function eachRow($child, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $endDate, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds, $searchedBranchId, $previousYearVoucherDetails, $projectIdArray, $projectTypeIdArray, $branchIdArray, $previousFiscalYearId){ ?>
    ...
    <tr class="item{{$child->id}}" level="{{$loopTrack}}" accountType="{{$child->accountTypeId}}">
        ...
        <?php
            ...
            if ($searchedBranchId == NULL || $searchedBranchId == 0) {
                ...
                $debitAccAmount = $voucherDetails
                                ->where('ledgerId', $child->id)
                                ->sum('debitAmount');
                $creditAccAmount = $voucherDetails
                                ->where('ledgerId', $child->id)
                                ->sum('creditAmount');
            }
            else {
                ...
                $debitAccAmount = $voucherDetails->where('debitAcc', $child->id)->sum('amount');
                $creditAccAmount = $voucherDetails->where('creditAcc', $child->id)->sum('amount');
            }

            // $debitAccAmount = $voucherDetails->where('debitAcc', $child->id)
            //         ->sum('amount');
            ...
            // $creditAccAmount = $voucherDetails->where('creditAcc', $child->id)
            //         ->sum('amount');
        ...
        @if($searchedSearchMethod==1)
            @php
                $openingBalanceDebitAmount = $previousYearVoucherDetails
                                            ->where('ledgerId', $child->id)
                                            // ->where('monthEndDate','>=',$previousFisStartDate)
                                            // ->where('monthEndDate','<=',$previousFisEndDate)
                                            ->sum('debitAmount');
                $openingBalanceCreditAmount = $previousYearVoucherDetails
                                            ->where('ledgerId', $child->id)
                                            // ->where('monthEndDate','>=',$previousFisStartDate)
                                            // ->where('monthEndDate','<=',$previousFisEndDate)
                                            ->sum('creditAmount');
                // $openingBalanceDebitAmount = $openingBalanceInfo->where('fiscalYearId', $previousFiscalYearId)->sum('debitAmount');
                // $openingBalanceCreditAmount = $openingBalanceInfo->where('fiscalYearId', $previousFiscalYearId)->sum('creditAmount');
                if($child->accountTypeId==12){
        ...
        @elseif($searchedSearchMethod==3)
            ...
            @php
                ...
                if ($searchedBranchId == NULL || $searchedBranchId == 0) {
                    ...
                    $currentMonthDebitAccAmount = $voucherDetails
                                                ->where('ledgerId', $child->id)
                                                // ->where('ledgerId', 86)
                                                ->where('monthEndDate','>=',$currentMonthStartDate)
                                                // ->where('monthEndDate','<=',$endDate)
                                                ->sum('debitAmount');
                    $currentMonthCreditAccAmount = $voucherDetails
                                                ->where('ledgerId', $child->id)
                                                // ->where('ledgerId', 86)
                                                ->where('monthEndDate','>=',$currentMonthStartDate)
                                                // ->where('monthEndDate','<=',$endDate)
                                                ->sum('creditAmount');
                                                // dd($currentMonthCreditAccAmount);
                }
                else {
                    $currentMonthDebitAccAmount = $voucherDetails
                                        ->whereIn('voucherId',$voucherIdMatchedByCurrMonth)
            ...
            @php
                $currentFisYearOpeningBalanceDebitAmount = $openingBalanceInfo->sum('debitAmount');
                $currentFisYearOpeningBalanceCreditAmount = $openingBalanceInfo->sum('creditAmount');
                // $currentFisYearOpeningBalanceDebitAmount = $openingBalanceInfo->where('fiscalYearId', $previousFiscalYearId)->sum('debitAmount');
                // $currentFisYearOpeningBalanceCreditAmount = $openingBalanceInfo->where('fiscalYearId', $previousFiscalYearId)->sum('creditAmount');
                if($child->accountTypeId==12){
                    $currentFiscalYearOpeningBalance = $currentFisYearOpeningBalanceCreditAmount - $currentFisYearOpeningBalanceDebitAmount;
                }elseif($child->accountTypeId==13){
                    $currentFiscalYearOpeningBalance = $currentFisYearOpeningBalanceDebitAmount - $currentFisYearOpeningBalanceCreditAmount;
                }
                ...
                $cumulativeTotal = $totalBalance + $currentFiscalYearOpeningBalance;
            @endphp
      ...
      <div class="panel panel-default" style="background-color:#708090;">
        ...
        <div class="panel-body panelBodyView">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-1">
                            <div class="form-group" style="font-size: 13px; color:black;">
                                ...
                                <div class="col-sm-12">
                                    {!! Form::select('searchMethod',['1'=>'Fiscal Year','3'=>'Current Year'], null,['id'=>'searchMethod','class'=>'form-control input-sm']) !!}
                                </div>
                            </div>
                        </div>
                        ...
                        <?php
                            $toDate= date("d-m-Y");
                            if ($searchedDateFrom=="") { $toDateFrom=''; }else{ $toDateFrom=$searchedDateFrom; }
            ...
            // echo "<br> searchedDateTo:".$searchedDateTo;
                ...
                //Project Type
                $numOfAllProjectTypes = DB::table('gnr_project_type')->pluck('id')->toArray();
                ...
                // dd($searchedBranchId);
                //Branch
                $numOfAllBranch = DB::table('gnr_branch')->whereIn('projectId', $projectIdArray)->orWhere('id', 1)->pluck('id')->toArray();
            ...
            if ($searchedSearchMethod==1) {        //========Fiscal Year
                ...
                $previousFisYearOpeningBalanceIds = DB::table('acc_opening_balance')->whereIn('projectId',$projectIdArray)->whereIn('projectTypeId',$projectTypeIdArray)->whereIn('branchId',$branchIdArray)->where('fiscalYearId', $previousFiscalYearId)->pluck('id')->toArray();

                // print_r($preFisYearOpeningBalanceIds);


                $currentFiYearName=DB::table('gnr_fiscal_year')->where('id',$searchedFiscalYearId)->value('name');
                $previousFiYearName=DB::table('gnr_fiscal_year')->where('id',$previousFiscalYearId)->value('name');
                ...
                $yearBackFromStart = \Carbon\Carbon::parse($startDate)->subYear()->format('Y-m-d');
                $previousFisYear = DB::table('gnr_fiscal_year')->where('fyStartDate', '<=', $yearBackFromStart)->where('fyEndDate', '>=', $yearBackFromStart)->first();
                $previousFiscalYearId = $previousFisYear->id;
                $previousFisStartDate = $previousFisYear->fyStartDate;
                $previousFisEndDate = $previousFisYear->fyEndDate;
                // dd($previousFisEndDate);
            } elseif ($searchedSearchMethod==3) {        //========Current Year
                $startDate = $thisYearStartDate = date('Y-m-d',strtotime($searchedDateFrom));
                $endDate = date('Y-m-d',strtotime($searchedDateTo));
                // $startDate = $thisYearStartDate = date('Y-m-d', strtotime(DB::table('gnr_fiscal_year')->where('id',$searchedFiscalYearId)->value('fyStartDate')));
                $currentMonthStartDate=date('Y-m-d', strtotime("first day of this month".$endDate));
                $currentMonthEndDate=$endDate;

                // dd($currentMonthEndDate);
                if ($branchSelected == NULL || $branchSelected == 0) {
                    $voucherIdMatchedByCurrMonth = [];
                }
                else {
                    ...
                    ///////////// project, project type and branch filter///////////////
                    ...
                    if (count($numOfAllProjectTypes)!=count($projectTypeIdArray)) {
                        $voucherIdMatchedByCurrMonth = $voucherIdMatchedByCurrMonth->whereIn('projectTypeId',$projectTypeIdArray);
                    }
                    // dd(count($branchIdArray));
                    if (count($numOfAllBranch)!=count($branchIdArray)) {
                        $voucherIdMatchedByCurrMonth = $voucherIdMatchedByCurrMonth->whereIn('branchId',$branchIdArray);
                    }

                    // dd($voucherIdMatchedByCurrMonth->distinct('branchId')->pluck('branchId'));
                    // dd($voucherIdMatchedByCurrMonth->get());
                    ///////////// project, project type and branch filter///////////////
...
//===============================================Starts Voucher Filtering=========================================================
            ...
            if ($branchSelected == NULL || $branchSelected == 0) {
                ...
                if (count($numOfAllProjects)!=count($projectIdArray)) {
                    $voucherIdMatched = $voucherIdMatched->whereIn('projectId',$projectIdArray);
                }
                if (count($numOfAllProjectTypes)!=count($projectTypeIdArray)) {
                    $voucherIdMatched = $voucherIdMatched->whereIn('projectTypeId',$projectTypeIdArray);
                }
                if (count($numOfAllBranch)!=count($branchIdArray)) {
                    $voucherIdMatched = $voucherIdMatched->whereIn('branchId',$branchIdArray);
                }

                $voucherIdMatched = $voucherIdMatched->pluck('id')->toArray();
            }

            if ($branchSelected == NULL || $branchSelected == 0) {

                $voucherDetails = DB::table('acc_month_end_balance')
                                ->whereIn('projectId',$projectIdArray)
                                ->whereIn('projectTypeId',$projectTypeIdArray)
                                ->whereIn('branchId',$branchIdArray)
                                // ->where(function ($query) use ($startDate,$endDate){
                                ...
                                //       ->where('monthEndDate','<=',$endDate);
                                //     })
                                ->select('id', 'monthEndDate', 'ledgerId', 'debitAmount', 'creditAmount')
                                ->get();

                $previousYearVoucherDetails = $voucherDetails
                                            ->where('monthEndDate','>=',$previousFisStartDate)
                                            ->where('monthEndDate','<=',$previousFisEndDate);

                if ($searchedSearchMethod == 1 || $searchedSearchMethod == 2) {

                    $voucherDetails = $voucherDetails
                                    ->where('monthEndDate','>=',$startDate)
                                    ->where('monthEndDate','<=',$endDate);

                } elseif ($searchedSearchMethod == 3) {
                    // dd($thisYearStartDate);

                    $voucherDetails = $voucherDetails
                                    ->where('monthEndDate','>=',$thisYearStartDate)
                                    ->where('monthEndDate','<=',$currentMonthEndDate);
                ...
                $previousYearVoucherDetails = DB::table('acc_month_end_balance')
                                            ->whereIn('projectId',$projectIdArray)
                                            ->whereIn('projectTypeId',$projectTypeIdArray)
                                            ->whereIn('branchId',$branchIdArray)
                                            ->where('monthEndDate','>=',$previousFisStartDate)
                        ...
                        <table class="table table-striped table-bordered" id="accIncomeStatementTable" border="1pt solid ash" style="border-collapse: collapse;" >
                            <thead>
                                <tr>
                                    ...
                                    @elseif($searchedSearchMethod==3)
                                        ...
                                        <th style="padding: 0px; width: 160px;">
                                            This Year <br/> {{"(".date('d-m-Y',strtotime($thisYearStartDate))." To ".date('d-m-Y',strtotime($currentMonthEndDate)).")"}}
                                        </th>
                            ...
                            @if(in_array($ledger->id,$ledgerMatchedId))
                                ...
                                @if(in_array($child1->id,$ledgerMatchedId))
                                    <?php
                                    $loopTrack=1;
                                    eachRow($child1, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $endDate, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds, $searchedBranchId, $previousYearVoucherDetails, $projectIdArray, $projectTypeIdArray, $branchIdArray, $previousFiscalYearId);
                                    ...
                                    @if(in_array($child2->id,$ledgerMatchedId))
                                        <?php
                                        $loopTrack=2;
                                        eachRow($child2, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $endDate, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds, $searchedBranchId, $previousYearVoucherDetails, $projectIdArray, $projectTypeIdArray, $branchIdArray, $previousFiscalYearId);
                                        ...
                                        @if(in_array($child3->id,$ledgerMatchedId))
                                            <?php
                                                $loopTrack=3;
                                                eachRow($child3, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $endDate ,$previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds, $searchedBranchId, $previousYearVoucherDetails, $projectIdArray, $projectTypeIdArray, $branchIdArray, $previousFiscalYearId);
                                            ...
                                            @if(in_array($child4->id,$ledgerMatchedId))
                                                <?php
                                                $loopTrack=4;
                                                eachRow($child4, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $endDate, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds, $searchedBranchId, $previousYearVoucherDetails, $projectIdArray, $projectTypeIdArray, $branchIdArray, $previousFiscalYearId);
                                                ...
                                                @if(in_array($child5->id,$ledgerMatchedId))
                                                    <?php
                                                    $loopTrack=5;
                                                    eachRow($child5, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $endDate, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds, $searchedBranchId, $previousYearVoucherDetails, $projectIdArray, $projectTypeIdArray, $branchIdArray, $previousFiscalYearId);
                                                    ?>
                                                @endif
    ...
    $(document).ready(function() {
      ...
      $("#searchMethod").change(function(event) {
            ...
            //Current Year
                ...
                // var dateFrom = $.datepicker.parseDate( 'dd-mm-yy',);
                //
                //
