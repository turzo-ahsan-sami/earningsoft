...
<?php
    ...
    <tr class="item{{$child->id}}" level={{$loopTrack}} type={{$type}}>
        ...
        <?php
            ...
            // //array chunk
            // if (count($voucherIdMatched) > 50000) {
            //     $voucherIdMatched = array_chunk($voucherIdMatched, 50000);
            // }
            $monthEndsInfo = DB::table('acc_month_end_balance')
                            ...
                            ->where(function ($query) use ($startDate,$endDate){
                                  $query->where('monthEndDate','>=',$startDate)
                                  ->where('monthEndDate','<=',$endDate);
                                })
            ...
            if ($searchedBranchId == NULL || $searchedBranchId == 0) {
                // $monthEndsInfo = DB::table('acc_month_end_balance')
                //                 ->whereIn('id', $monthEndsIdArr)
                //                 ->select('ledgerId', 'debitAmount', 'creditAmount')
                //                 ->get();

                $debitAccAmount = $monthEndsInfo->sum('debitAmount');
                $creditAccAmount = $monthEndsInfo->sum('creditAmount');
            }
            else {
            ...
            // // condition for all branches selected
        ...
        {{-- Fiscal Year --}}
        @php
        $previousFisYearOpeningBalanceInfos = $previousFisYearOpeningBalanceInfo->where('fiscalYearId', $previousFisIdOfCurrentFisYear)
                                            ->where('ledgerId', $child->id);
                                            // ->whereIn('projectId',$projectIdArray)
                                            // ->whereIn('projectTypeId',$projectTypeIdArray)
                                            // ->whereIn('branchId',$branchIdArray)
                                            // ->where('fiscalYearId', $previousFisIdOfCurrentFisYear)
        ...
        @if($searchedSearchMethod==1)
            @php
               ...
               //  ->value('surplusAmount');
                ...
                if($child->id==512){
                    // $currentFisYearTotalBalanceByFisYr = $currentFisYearTotalBalanceByFisYr - $totalBalanceOfPreviousFisYear;
                    // $previousFisYearOpeningBalanceTotalByFisYr = $previousFisYearOpeningBalanceTotalByFisYr - $totalBalanceOfOneBeforePreviousFisYear;
        ...
        @elseif($searchedSearchMethod==2)
            ...
            @php
                ...
                if ($searchedBranchId == NULL || $searchedBranchId == 0) {
                    // $vouchersInfoCum = DB::table('acc_month_end_balance')
                    //                 ->whereIn('id',$monthEndsIdArr)
                    //                 ->select('ledgerId', 'debitAmount', 'creditAmount')
                    //                 ->get();

                    $cumulativeDebitAccAmount = $monthEndsInfo->sum('debitAmount');
                    $cumulativeCreditAccAmount = $monthEndsInfo->sum('creditAmount');
                }
                else {
                    $vouchersInfoCum = DB::table('acc_voucher_details')
                                        ->where('status',1)
                                        ->whereIn('voucherId',$voucherIdMatched)
                                        ->select('debitAcc', 'creditAcc', 'amount')
                                        ->get();

                    $cumulativeDebitAccAmount = $vouchersInfoCum->where('debitAcc', $child->id)->sum('amount');
                    $cumulativeCreditAccAmount = $vouchersInfoCum->where('creditAcc', $child->id)->sum('amount');
                }
                ...
                $previousFisYearOpeningBalanceInfos = $previousFisYearOpeningBalanceInfo
                                                    ->where('ledgerId', $child->id);
                                                    // ->whereIn('projectId',$projectIdArray)
                                                    // ->whereIn('projectTypeId',$projectTypeIdArray)
                                                    // ->whereIn('branchId',$branchIdArray)
                ...
                if($child->id==512){
                    // $thisYearTotal = $thisYearTotal - $totalBalanceOfPreviousFisYear;
                    // $previousFisYearOpeningBalanceAmount = $previousFisYearOpeningBalanceAmount - $totalBalanceOfOneBeforePreviousFisYear;
                    $thisYearTotal = $thisYearTotal + $totalCumSurplusOnPreviousFisYear;
                    $previousFisYearOpeningBalanceAmount = $previousFisYearOpeningBalanceAmount + $totalCumSurplusOnOneBeforePreviousFisYear;
...
<div class="" style="">
        ...
        <div class="panel panel-default" style="background-color:#708090;">
            ...
            <!-- filtering end-->
                ...
                if ($searchedSearchMethod==1) {        //========Fiscal Year
                    ...
                    $previousFisYearMonthEndInfo = DB::table('acc_month_end_balance')
                                                        ->whereIn('projectId',$projectIdArray)
                                                        ->whereIn('projectTypeId',$projectTypeIdArray)
                                                        ->whereIn('branchId',$branchIdArray)
                                                        ->where('fiscalYearId', $previousFisIdOfCurrentFisYear)
                                                        ->select('ledgerId', 'balanceAmount')
                                                        ->get();
                    // dd($previousFisYearMonthEndInfo);
                    // $previousFisYearOpeningBalanceIds = $previousFisYearOpeningBalanceInfo->pluck('id')->toArray();
                    ...
                    // month ends collection

                    // $monthEndsIdArr = DB::table('acc_month_end_balance')
                    //                 ->whereIn('projectId',$projectIdArray)
                    //                 ->whereIn('projectTypeId',$projectTypeIdArray)
                    //                 ->whereIn('branchId',$branchIdArray)
                    //                 ->where(function ($query) use ($startDate,$endDate){
                    ...
                    // $monthEndsIdArrByFIEndDay = DB::table('acc_month_end_balance')
                    //                             ->whereIn('projectId',$projectIdArray)
                    //                             ->whereIn('projectTypeId',$projectTypeIdArray)
                    //                             ->whereIn('branchId',$branchIdArray)
                    //                             ->where(function ($query) use ($startDate,$endDate){
    ...
    //===============================================Starts Calculate Surplus===============================================
                    ...
                    //     $voucherIdMatched = array_chunk($voucherIdMatched, 50000);
                    // }
                    // if (count($voucherIdMatchedByFIEndDay) > 50000) {
                    //     $voucherIdMatchedByFIEndDay = array_chunk($voucherIdMatchedByFIEndDay, 50000);
                    // }
                    ...
                    if ($branchSelected == NULL || $branchSelected == 0) {
                        $monthEndsInfo = DB::table('acc_month_end_balance')
                                        ...
                                        ->select('id','ledgerId', 'debitAmount', 'creditAmount')
                                        ->get();
                        // $monthEndsIdArr = $monthEndsInfo->pluck('id')->toArray();
                    }
                    else {
                        $monthEndsIdArr = [];
                    }
                    ...
                    // $previousFisYearOpeningBalanceInfo = DB::table('acc_opening_balance')
                    //                                     ->whereIn('id',$previousFisYearOpeningBalanceIds)
                    //                                     ->select('ledgerId', 'balanceAmount')
                    //                                     ->get();
                    // dd($previousFisYearOpeningBalanceInfo);

                    // $incomeBalanceOfPreviousFisYear = $previousFisYearOpeningBalanceInfo->whereIn('ledgerId',$incomeLedgerIds)->sum('balanceAmount');
                    // $expenceBalanceOfPreviousFisYear = $previousFisYearOpeningBalanceInfo->whereIn('ledgerId',$expenseLedgerIds)->sum('balanceAmount');


                    // if($searchedSearchMethod==1){

                    $incomeBalanceOfPreviousFisYear=$expenceBalanceOfPreviousFisYear=$cumIncomeOfPreviousFisYear=$cumExpenseOfPreviousFisYear=$cumIncomeOfOneBeforePreviousFisYear=$cumExpenseOfOneBeforePreviousFisYear=$incomeBalance=$incomeDebitAmount=$incomeCreditAmount=$expenseBalance=$expenseDebitAmount=$expenseCreditAmount=$totalSurplusOFCurrentFI=0;

                    // income loop
                    foreach ($incomeLedgerIds as $key => $id) {
                        //previous year
                        $incomeBalanceOfPreviousFisYear += $previousFisYearMonthEndInfo->where('ledgerId', $id)->sum('balanceAmount');
                        // previous cumulative
                        $cumIncomeOfPreviousFisYear += $previousFisYearOpeningBalanceInfo->where('fiscalYearId', $previousFisIdOfCurrentFisYear)->where('ledgerId', $id)->sum('balanceAmount');
                        $cumIncomeOfOneBeforePreviousFisYear += $previousFisYearOpeningBalanceInfo->where('fiscalYearId', $previousFisIdOfPreviousFisYear)->where('ledgerId', $id)->sum('balanceAmount');

                        if ($branchSelected == NULL || $branchSelected == 0) {

                            //income
                            $incomeDebitAmount += $monthEndsInfo->where('ledgerId', $id)->sum('debitAmount');
                            $incomeCreditAmount += $monthEndsInfo->where('ledgerId', $id)->sum('creditAmount');
                        }
                        else {
                            //income
                            $incomeDebitAmount += $vouchersInfo->where('debitAcc', $id)->sum('amount');
                            $incomeCreditAmount += $vouchersInfo->where('creditAcc', $id)->sum('amount');
                        }
                    }
                    ...
                    foreach ($expenseLedgerIds as $key => $id) {
                        // previous year
                        $expenceBalanceOfPreviousFisYear += $previousFisYearMonthEndInfo->where('ledgerId', $id)->sum('balanceAmount');
                        // previous cumulative
                        $cumExpenseOfPreviousFisYear += $previousFisYearOpeningBalanceInfo->where('fiscalYearId', $previousFisIdOfCurrentFisYear)->where('ledgerId', $id)->sum('balanceAmount');
                        $cumExpenseOfOneBeforePreviousFisYear += $previousFisYearOpeningBalanceInfo->where('fiscalYearId', $previousFisIdOfPreviousFisYear)->where('ledgerId', $id)->sum('balanceAmount');

                        if ($branchSelected == NULL || $branchSelected == 0) {

                            //expense
                            $expenseDebitAmount += $monthEndsInfo->where('ledgerId', $id)->sum('debitAmount');
                            $expenseCreditAmount += $monthEndsInfo->where('ledgerId', $id)->sum('creditAmount');

                        }
                        else {
                            //expense
                            $expenseDebitAmount += $vouchersInfo->where('debitAcc', $id)->sum('amount');
                            $expenseCreditAmount += $vouchersInfo->where('creditAcc', $id)->sum('amount');
                    ...
                    //
                    //     //income
                    //     $incomeDebitAmount = $monthEndsInfo->whereIn('ledgerId', $incomeLedgerIds)->sum('debitAmount');
                    //     $incomeCreditAmount = $monthEndsInfo->whereIn('ledgerId', $incomeLedgerIds)->sum('creditAmount');
                    //     //expense
                    //     $expenseDebitAmount = $monthEndsInfo->whereIn('ledgerId', $expenseLedgerIds)->sum('debitAmount');
                    //     $expenseCreditAmount = $monthEndsInfo->whereIn('ledgerId', $expenseLedgerIds)->sum('creditAmount');
                    // }
                    // else {
                    //     //income
                    ...
                    // dd($totalCumSurplusOnPreviousFisYear);
                    // $previousFisYearOpeningBalanceInfo = DB::table('acc_opening_balance')
                    //                                     ->whereIn('id',$previousFisYearOpeningBalanceIds)
                    //                                     ->select('ledgerId', 'balanceAmount')
                    //                                     ->get();
                    ...
                    if($searchedSearchMethod==2){
                        ...
                        foreach ($expenseLedgerIds as $key => $id) {
                            ...
                            // $expenceBalanceOfPreviousFisYear += $previousFisYearOpeningBalanceInfo->where('ledgerId', $id)->sum('balanceAmount');
                        ...
                        //
                        //     //income
                        //     $incomeDebitAmountByCurrFIEndDay = $monthEndsInfo->whereIn('ledgerId', $incomeLedgerIds)->sum('debitAmount');
                        //     $incomeCreditAmountByCurrFIEndDay = $monthEndsInfo->whereIn('ledgerId', $incomeLedgerIds)->sum('creditAmount');
                        //     //expense
                        //     $expenseDebitAmountByCurrFIEndDay = $monthEndsInfo->whereIn('ledgerId', $expenseLedgerIds)->sum('debitAmount');
                        //     $expenseCreditAmountByCurrFIEndDay = $monthEndsInfo->whereIn('ledgerId', $expenseLedgerIds)->sum('creditAmount');
                        // }
                        // else {
                        //     //income
                        //     $incomeDebitAmountByCurrFIEndDay = $vouchersInfo->whereIn('debitAcc', $incomeLedgerIds)->sum('amount');
                        //     $incomeCreditAmountByCurrFIEndDay = $vouchersInfo->whereIn('creditAcc', $incomeLedgerIds)->sum('amount');
                        //     //expense
                        //     $expenseDebitAmountByCurrFIEndDay = $vouchersInfo->whereIn('debitAcc', $expenseLedgerIds)->sum('amount');
                        //     $expenseCreditAmountByCurrFIEndDay = $vouchersInfo->whereIn('creditAcc', $expenseLedgerIds)->sum('amount');
                        // }
...
{{-- ===================================================Surplus Row For current Year================================================== --}}
                                        <?php if($key==2){?>
                                            ...
                                            <tr level="4" style="font-weight: bold; display:none;" >
                                                ...
                                                <td class="previousYearColumn" amount="{{$totalSurplusOfPreviousFI}}" ></td>
                                                <td class="thisYearColumn" amount="{{$totalSurplusOFCurrentFI}}" ></td>
