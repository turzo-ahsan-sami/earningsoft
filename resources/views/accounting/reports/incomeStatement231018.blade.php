...
<?php
    function eachRow($child, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds){ ?>
    ...
    <tr class="item{{$child->id}}" level="{{$loopTrack}}" accountType="{{$child->accountTypeId}}">
        ...
        <?php
            ...
            $dbOpeningBalance = DB::table('acc_opening_balance')->get();
        ...
        @if($searchedSearchMethod==1)
            @php
                $openingBalanceDebitAmount = $dbOpeningBalance->whereIn('id',$previousFisYearOpeningBalanceIds)->where('ledgerId', $child->id)->sum('debitAmount');
                $openingBalanceCreditAmount = $dbOpeningBalance->whereIn('id',$previousFisYearOpeningBalanceIds)->where('ledgerId', $child->id)->sum('creditAmount');
                if($child->accountTypeId==12){
        ...
        @elseif($searchedSearchMethod==3)
            ...
            @php
                $currentFisYearOpeningBalanceDebitAmount = $dbOpeningBalance->whereIn('id',$preFisYearOpeningBalanceIds)->where('ledgerId', $child->id)->sum('debitAmount');

                $currentFisYearOpeningBalanceCreditAmount = $dbOpeningBalance->whereIn('id',$preFisYearOpeningBalanceIds)->where('ledgerId', $child->id)->sum('creditAmount');
    ...
    <!--endE eachRow function -->


    <?php $dbOpeningBalance = DB::table('acc_opening_balance')->get(); ?>
      ...
      <div class="panel panel-default" style="background-color:#708090;">
        ...
        <div class="panel-body panelBodyView">
            ...
            if ($searchedSearchMethod==1) {        //========Fiscal Year
                ...
                $previousFisYearOpeningBalanceIds = $dbOpeningBalance->whereIn('projectId',$projectIdArray)->whereIn('projectTypeId',$projectTypeIdArray)->whereIn('branchId',$branchIdArray)->where('fiscalYearId', $previousFisId)->pluck('id')->toArray();
                ...
                $voucherIdMatchedByCurrMonth = DB::table('acc_voucher')
                            ->where('status',1)
                            ->where('voucherDate','>=',$currentMonthStartDate)
                ...
                $preFisYearOpeningBalanceIds = $dbOpeningBalance->whereIn('projectId',$projectIdArray)->whereIn('projectTypeId',$projectTypeIdArray)->whereIn('branchId',$branchIdArray)->where('fiscalYearId', $previousFiscalYearId)->pluck('id')->toArray();
...
//===============================================Starts Voucher Filtering=========================================================
            ...
            //$chunkNumber = ceil(count($voucherIdMatched)/60000);
            //$chunkNumber = $chunkNumber ==0 ? 1 : $chunkNumber;
            //$chunkArrays = array_chunk($voucherIdMatched,$chunkNumber);
//
//            //$voucherDetails = collect();
//
//            //foreach ($chunkArrays as $chunkArray) {
//            //    $tempVoucherDetails = DB::table('acc_voucher_details')
//            //                        ->where('status',1)
//            //                        ->whereIn('voucherId',$chunkArray)
//            //                        ->select('voucherId','debitAcc','creditAcc','amount')
//            //                        ->get();
//
//            //    $voucherDetails->merge($tempVoucherDetails);
//            //}
//
                        ...
                        <table class="table table-striped table-bordered" id="accIncomeStatementTable" border="1pt solid ash" style="border-collapse: collapse;" >
                            ...
                            @if(in_array($ledger->id,$ledgerMatchedId))
                                ...
                                @if(in_array($child1->id,$ledgerMatchedId))
                                    <?php
                                    $loopTrack=1;
                                    eachRow($child1, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds);
                                    ...
                                    @if(in_array($child2->id,$ledgerMatchedId))
                                        <?php
                                        $loopTrack=2;
                                        eachRow($child2, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds);
                                        ...
                                        @if(in_array($child3->id,$ledgerMatchedId))
                                            <?php
                                                $loopTrack=3;
                                                eachRow($child3, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds);
                                            ...
                                            @if(in_array($child4->id,$ledgerMatchedId))
                                                <?php
                                                $loopTrack=4;
                                                eachRow($child4, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds);
                                                ...
                                                @if(in_array($child5->id,$ledgerMatchedId))
                                                    <?php
                                                    $loopTrack=5;
                                                    eachRow($child5, $loopTrack, $voucherIdMatched,$voucherDetails, $searchedRoundUp, $searchedSearchMethod, $previousFisYearOpeningBalanceIds, $voucherIdMatchedByCurrMonth, $preFisYearOpeningBalanceIds);
                                                    ?>
                                                @endif
