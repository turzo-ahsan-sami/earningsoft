  ...
  <div class="col-md-12">
    <div class="" style="">
      <div class="">
        <div class="panel panel-default" style="background-color:#708090;">
          <div class="panel-heading" style="padding-bottom:0px">
            ...
            <h3  style=" text-align: center; font-family: Antiqua; letter-spacing: 2px; color: white;">OTS Interest Generate Report</h3>
          </div>
          {{-- <h5>kxghixd</h5> --}}
          ...
          <div class="panel-body panelBodyView">
            ...
            <div class="row" id="filtering-group">
              ...
              <div class="form-horizontal form-groups" style="padding-right: 0px;">
                ...
                {!! Form::open(['url' => 'viewOtsInterestGenerateReport','method' => 'get']) !!}
          ...
          <div id="printDiv">
            ...
            <div id="printingContent">
              ...
              <div style="display: none;text-align: center;" id="hiddenTitle">
               ...
               <h4 style="text-align: center;padding: 0px;margin: 0px;">OTS Interest Generate Report</h4>
      ...
      <tbody>
        ...
        @foreach($interestDetails as $index => $interestDetail)

        <tr>
          <td>{{$index+1}}</td>
          <td class="name">{{$accName[$index]}}</td>
          <td>{{$interestDetail->accNo}}</td>
          <td class="name">{{$branchName[$index]}}</td>
          <td class="name">{{$accNature[$index]}}</td>
          <td>{{$interestRate[$index]}}</td>
          <td class="amount">{{number_format($principalAmount[$index],2,'.',',')}}</td>
          <td class="amount">{{number_format($interestDueAmount[$index],2,'.',',')}}</td>
        </tr>

        @php
        $totalAmount = $totalAmount + (float) $interestDueAmount[$index];
        @endphp
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#btnExportExcel").click(function(e) {
        ...
        let a = $("<a />", {
          href: url,
          download: "OTS Interest Generate Report_"+ today + ".xls"}).appendTo("body").get(0).click();
        e.preventDefault();
      });
...
/* End Date Range To */
    ...
    if (firstRequest==1) {
      ...
      $("#search").click(function(event) {
        var searchInterestId = $("#searchInterestId").val();
        if ($("#dateFrom").val()=="" && searchInterestId=="") {
          event.preventDefault();
          $("#dateFrome").show();
        }
        if ($("#dateTo").val()=="" && searchInterestId=="") {
          event.preventDefault();
          $("#dateToe").show();
        }
      });
  ...
  <script type="text/javascript">
    jQuery(document).ready(function($) {
      ...
      $("#searchProject").change(function(){
        ...
        $.ajax({
          type: 'post',
          url: './getOtsInterestBaseOnProjectNbranch',
          data: {projectId:projectId,branchId: branchId,_token: csrf},
          dataType: 'json',
          success: function( data ){


            $("#searchInterestId").empty();
            $("#searchInterestId").prepend('<option selected="selected" value="">All</option>');


            $.each(data, function (key, value) {

              $('#searchInterestId').append("<option value='"+ key+"'>"+value+"</option>");
            });
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $("#searchInterestId").on('change', function() {
        if (this.value!='') {
