    ...
    <div class="" style="">
      <div class="">
        <div class="panel panel-default" style="background-color:#708090;">
          ...
          <h3  style=" text-align: center; font-family: Antiqua; letter-spacing: 2px; color: white;">OTS Balance Report</h3>
        </div>
        {{-- <h5>kxghixd</h5> --}}
        ...
        <div class="panel-body panelBodyView">
          ...
          <div class="row" id="filtering-group">
            ...
            <div class="form-horizontal form-groups" style="padding-right: 0px;">
              ...
              {!! Form::open(['url' => 'viewOtsRegisterBalanceReport','method' => 'get']) !!}
            ...
            <div class="col-md-2">
              <div class="form-group" style="font-size: 13px; color:black;">
                ...
                <div class="col-sm-12">
                  @php
                  $accNature = array(''=>'All') + DB::table('acc_ots_period')->pluck('name','id')->toArray();
                  @endphp
                  {!! Form::select('searchAccNature',$accNature,$accNatureSelected,['id'=>'searchAccNature','class'=>'form-control']) !!}
            ...
            <div class="col-md-2" id="dateRangeDiv">
                                          ...
                                          <div id="printDiv">
                                            ...
                                            <div id="printingContent">
                                              ...
                                              <div style="display: none;text-align: center;" id="hiddenTitle">
                                               ...
                                               <h4 style="text-align: center;padding: 0px;margin: 0px;">OTS Balance Report</h4>
                                      ...
                                      <table id="registerReportTable" class="table table-striped table-bordered" style="color: black;font-size:11px;border-collapse: collapse;margin-bottom:50px;" border= "1px solid black;" cellpadding="0" cellspacing="0" width="100%">
                                        <thead >
                                          <tr>
                                            ...
                                            <th rowspan="2">Payment Nature</th>
                                            <th rowspan="2">Interest Rate (%)</th>
                                            <th rowspan="2">Principal Amount (Tk)</th>
                                            <th rowspan="2">Interest Gen. <br> Amount (Tk)</th>
                                            <th rowspan="2">Interest Payment <br> Amount (Tk)</th>
                                            <th colspan="3">OTS Encashment</th>
                                            <th colspan="3">Closing Balance (Pri. + Int.)</th>
                                          </tr>
              ...
              //$gTpaymentTotal = 0;
                                          ...
                                          $gTecashmentPrincipal = 0;
                                          $gTecashmentInterest = 0;
                                          ...
                                          $paymentNatureName =  DB::table('acc_ots_period')->where('id',$account->periodId_fk)->value('name');
                                          $memberName =  DB::table('acc_ots_member')->where('id',$account->memberId_fk)->value('name');

                                          $lastPaidInterestRate = DB::table('acc_ots_payment_details')->where('accId_fk',$account->id)->orderBy('id','desc')->value('interestRate');
                                          if ($lastPaidInterestRate==null) {
                                            $lastPaidInterestRate = DB::table('acc_ots_period')->where('id',$account->periodId_fk)->value('interestRate');
                                          }
                                          /* $interestPrevious = DB::table('acc_ots_interest_details')->where('accId_fk',$account->id)->where('generateDate','<',$startDate)->sum('amount') + (float) DB::table('acc_ots_account')->where('id',$account->id)->where('openingBalanceDate','<',$startDate)->value('openingBalance');*/

                                          /*$interestTotal = $interestPrevious + $interestCurrent;*/

                                          /*$paymentPrevious = DB::table('acc_ots_payment_details')->where('accId_fk',$account->id)->where('paymentDate','<',$startDate)->sum('amount');*/
                                          if ($account->closingDate==$endDate) {
                                            $interestCurrent = 0;
                                            $paymentCurrent = 0;

                                            $encashmentDate = date('Y-m-d', strtotime($account->closingDate));
                                            $encashmentPrincipal = $account->amount;
                                            $encashmentInterest = DB::table('acc_ots_interest_details')->where('accId_fk',$account->id)->where('generateDate','>=',$startDate)->where('generateDate','<=',$endDate)->sum('amount') + (float) DB::table('acc_ots_account')->where('id',$account->id)->where('openingBalanceDate','>=',$startDate)->where('openingBalanceDate','<=',$endDate)->value('openingBalance');
                                            $closingPricipal = 0;
                                            $closingInterest = 0;
                                            $closingTotal = 0;
                                          }
                                          else{
                                            $interestCurrent = DB::table('acc_ots_interest_details')->where('accId_fk',$account->id)->where('generateDate','>=',$startDate)->where('generateDate','<=',$endDate)->sum('amount') + (float) DB::table('acc_ots_account')->where('id',$account->id)->where('openingBalanceDate','>=',$startDate)->where('openingBalanceDate','<=',$endDate)->value('openingBalance');
                                            $paymentCurrent = DB::table('acc_ots_payment_details')->where('accId_fk',$account->id)->where('paymentDate','>=',$startDate)->where('paymentDate','<=',$endDate)->sum('amount');

                                            $encashmentDate = '-';
                                            $encashmentPrincipal = 0;
                                            $encashmentInterest = 0;

                                            $closingPricipal = $account->amount;
                                            $closingInterest = $interestCurrent - $paymentCurrent;
                                            $closingTotal = $account->amount + $interestCurrent - $paymentCurrent;
                                          }

                                          /*$paymentTotal = $paymentPrevious + $paymentCurrent;*/

                                          /*$dueInterest = $interestTotal - $paymentTotal;*/
              /*$dueInterest = $interestCurrent - $paymentCurrent;
              $total = $account->amount + $dueInterest;*/

              @endphp

              <tr>
               <td>{{$key+1}}</td>
               <td>{{date('d-m-Y',strtotime($account->openingDate))}}</td>
               <td class="name">{{$memberName}}</td>
               <td>{{$account->accNo}}</td>
               <td class="name">{{$branchName}}</td>
               <td>{{$paymentNatureName}}</td>
               <td>{{number_format($lastPaidInterestRate,2)}}</td>
               <td class="amount">{{number_format($account->amount,2,'.',',')}}</td>
               <td class="amount">{{number_format($interestCurrent,2,'.',',')}}</td>
               <td class="amount">{{number_format($paymentCurrent,2,'.',',')}}</td>

               <td>{{$encashmentDate}}</td>
               <td class="amount">{{number_format($encashmentPrincipal,2,'.',',')}}</td>
               <td class="amount">{{number_format($encashmentInterest,2,'.',',')}}</td>

               <td class="amount">{{number_format($closingPricipal,2,'.',',')}}</td>
               <td class="amount">{{number_format($closingInterest,2,'.',',')}}</td>
               <td class="amount">{{number_format($closingTotal,2,'.',',')}}</td>

             </tr>

             @php
             /*$gTinterestPrevious = $gTinterestPrevious + $interestPrevious;*/
             $gTinterestCurrent = $gTinterestCurrent + $interestCurrent;
             /*$gTinterestTotal = $gTinterestTotal + $interestTotal;*/
             /*$gTpaymentPrevious = $gTpaymentPrevious + $paymentPrevious;*/
             $gTpaymentCurrent = $gTpaymentCurrent + $paymentCurrent;
             /*$gTpaymentTotal = $gTpaymentTotal + $paymentTotal;*/

             $gTecashmentPrincipal = $gTecashmentPrincipal + $encashmentPrincipal;
             $gTecashmentInterest = $gTecashmentInterest + $encashmentInterest;


             $gTsavings = $gTsavings + $closingPricipal;
             $gTdueInterest = $gTdueInterest + $closingInterest;
             $gTtotal = $gTtotal + $closingTotal;
             @endphp
           ...
           <tfoot>
            <tr>
              ...
              <td class="amount">{{number_format($gTsavings,2,'.',',')}}</td>

              {{-- <td class="amount">{{number_format($gTinterestPrevious,2,'.',',')}}</td> --}}
              <td class="amount">{{number_format($gTinterestCurrent,2,'.',',')}}</td>
              {{-- <td class="amount">{{number_format($gTinterestTotal,2,'.',',')}}</td> --}}

              {{-- <td class="amount">{{number_format($gTpaymentPrevious,2,'.',',')}}</td> --}}
              <td class="amount">{{number_format($gTpaymentCurrent,2,'.',',')}}</td>
              {{-- <td class="amount">{{number_format($gTpaymentTotal,2,'.',',')}}</td> --}}

              <td></td>
              <td class="amount">{{number_format($gTecashmentPrincipal,2,'.',',')}}</td>
              <td class="amount">{{number_format($gTecashmentInterest,2,'.',',')}}</td>

              <td class="amount">{{number_format($gTsavings,2,'.',',')}}</td>
              <td class="amount">{{number_format($gTdueInterest,2,'.',',')}}</td>
              <td class="amount">{{number_format($gTtotal,2,'.',',')}}</td>
            </tr>
          </tfoot>
        </table>
      </div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#btnExportExcel").click(function(e) {
        ...
        let a = $("<a />", {
          href: url,
          download: "OTS Balance Report_"+ today + ".xls"}).appendTo("body").get(0).click();
        e.preventDefault();
      });
    ...
    var softwareDate = "{{$softwareDate}}";
    var dt = new Date(softwareDate);
    // alert(softwareDate);
    ...
    $("#searchBranch").change(function(event) {
      ...
      var branchId = $(this).val();
            ...
            .done(function(response) {
              maxDate = response.softwareMaxDate;
              // alert(minDate);
              $("#dateTo").datepicker("option","maxDate",new Date(toDate(maxDate)));
            })
            .fail(function() {
              console.log("error");
            })
      ...
      $("#searchBranch").on('change', function() {
        ...
        $.ajax({
          ...
          success: function( data ){
            ...
            $("#searchAccId").empty();
            $("#searchAccId").prepend('<option selected="selected" value="">All</option>');


            $.each(data, function (key, value) {

              $('#searchAccId').append("<option value='"+ key+"'>"+value+"</option>");
            });
