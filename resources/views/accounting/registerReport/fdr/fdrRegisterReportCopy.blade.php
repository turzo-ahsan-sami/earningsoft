          ...
          <table id="fdrRegisterTable" class="table table-striped table-bordered" style="color: black;font-size:11px;border-collapse: collapse;margin-bottom:50px;" border= "1px solid black;" cellpadding="0" cellspacing="0" width="100%">
                ...
                <tr>
                    <td colspan="16" class="bankName">{{$bankName.', '.$branch->name}}</td>
                            ...
                            @endphp
                              ...
                              @php
                                  ...
                                  $taxAmount = DB::table('acc_fdr_interest')->where('fdrAccId_fk',$fdrAccount->id)->where('receiveDate','>=',$startDate)->where('receiveDate','<=',$endDate)->sum('taxAmount');
                                   ...
                                   $closingDate = DB::table('acc_fdr_close')->where('accId_fk',$fdrAccount->id)->value('closingDate');
                                   if ($closingDate!=null) {
                                       $closingAmount = $fdrAccount->principalAmount + $netInterest;
                                   }
                                   else{
                                    $closingAmount = 0;
                                   }

                                   $balance = $receiveAbleAmount - $closingAmount;
                             ...
                             <tr>
                              ...
                              @php
                                 $tPrincipalAmount = $tPrincipalAmount + $fdrAccount->principalAmount;
                                 $tInterest = $tInterest + $interest;
                                 $tBankCharge = $tBankCharge + $bankChange;
                                 $tTaxAmount = $tTaxAmount + $taxAmount;
                                 $tNetInterest = $tNetInterest + $netInterest;
                                 $tReceiveableAmount = $tReceiveableAmount + $receiveAbleAmount;
                                 $tClosingAmount = $tClosingAmount + $closingAmount;
                        ...
                        <tr class="subTotalRow">
                            ...
                            <td class="amount">{{number_format($tPrincipalAmount,2,'.',',')}}</td>
                            ...
                            <td class="amount">{{number_format($tNetInterest,2,'.',',')}}</td>
                            <td class="amount">{{number_format($tReceiveableAmount,2,'.',',')}}</td>
                            <td></td>
                            <td class="amount">
                             @if($closingDate!=null)
                             {{number_format($tClosingAmount,2,'.',',')}}
                             @else
                             {{"-"}}
                             @endif                                
                            </td>
                            <td class="amount">{{number_format($tBalance,2,'.',',')}}</td>
                            
                        </tr>

                        @php
                            $gtPrincipalAmount = $gtPrincipalAmount + $tPrincipalAmount;
                            $gtInterest = $gtInterest + $tInterest;
                            $gtBankCharge = $gtBankCharge + $tBankCharge;
                            $gtTaxAmount = $gtTaxAmount + $tTaxAmount;
                            $gtNetInterest = $gtNetInterest + $tNetInterest;
                            $gtReceiveableAmount = $gtReceiveableAmount + $tReceiveableAmount;
                            $gtClosingAmount = $gtClosingAmount + $tClosingAmount;
                            $gtBalance = $gtBalance + $tBalance;
                        @endphp
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="amount">{{number_format($gtPrincipalAmount,2,'.',',')}}</td>
                    ...
                    <td class="amount">{{number_format($gtNetInterest,2,'.',',')}}</td>
                    <td class="amount">{{number_format($gtReceiveableAmount,2,'.',',')}}</td>
                    <td></td>
                    <td class="amount">{{number_format($gtClosingAmount,2,'.',',')}}</td>
                    <td class="amount">{{number_format($gtBalance,2,'.',',')}}</td>
