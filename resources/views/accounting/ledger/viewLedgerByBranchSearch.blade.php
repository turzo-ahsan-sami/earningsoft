...
<?php 
    ...
    function splitProjectBranchId($rowId){


        $projectBranchValue = DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('companyIdFk', Auth::user()->company_id_fk)->where('id', $rowId)->value("projectBranchId");
            ...
            for($i=0; $i<$array_length+1; $i++){
                ...
                if($firstIndexValue==0){
                    $projectsAll=DB::table('gnr_project')->where('companyId', Auth::user()->company_id_fk)->select('name','id')->get();
                    foreach ($projectsAll as $project){
                        echo $project->name.' ' ;
                    }

                    if($secondIndexValue==0){
                        $branchesAll=DB::table('gnr_branch')->where('companyId', Auth::user()->company_id_fk)->select('name','id')->get();
                    ...
                    if($firstIndexValue!=$temp){    
                        ...
                        echo DB::table('gnr_project')->where('companyId', Auth::user()->company_id_fk)->where('id',$firstIndexValue)->value('name')." ";

                        array_push($arrayProjects, $firstIndexValue);
                    ...
                    if($secondIndexValue!=0){
                        echo DB::table('gnr_branch')->where('companyId', Auth::user()->company_id_fk)->where('id',$secondIndexValue)->value('name')." ";
                        // echo $secondIndexValue;
                        array_push($arrayBranches, $secondIndexValue);

                    }else{
                        $projectBranches=DB::table('gnr_branch')->where('companyId', Auth::user()->company_id_fk)->select('name','id')->where('projectId',$firstIndexValue)->whereNotIn('id', [1])->get();
                        echo DB::table('gnr_branch')->where('companyId', Auth::user()->company_id_fk)->where('id',1)->value('name')." ";                       

                        foreach ($projectBranches as $projectBranch){
                            echo $projectBranch->name.' ' ;
                            array_push($arrayBranches, $projectBranch->id);
                        }
                    }
                    // echo $temp1;
    ...
    function splitProjectBranchId1($rowId, $searchedBranchId){


        $projectBranchValue = DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('id', $rowId)->value("projectBranchId");
       // dd($projectBranchValue);
        $splitArray=str_replace(array('[', ']', '"', ''), '',  $projectBranchValue);    
        $splitArrayFirstValue = explode(",", $splitArray);
        $splitArraySecondValue = explode(":", $splitArrayFirstValue[0]);
        ...
        for($i=0; $i<$array_length+1; $i++){
            ...
            if($firstIndexValue==0){
                ...
                if($secondIndexValue==0){
                    ...
                    foreach ($branchesAll as $branch){
                        // echo $branch->name.' ' ;
                        array_push($arrayBranches, $branch->id);
                    }   
                }
            }else{
...
<?php
    ...
    <tr class="item{{$child->id}}">
        ...
        {{-- <td width="" style="text-align: left; padding-left: 20px;" class="projectColumn"> --}}

            <?php 

                // $arrayBranches=splitProjectBranchId($child->id);

                // echo var_dump($arrayBranches);
                // echo $searchedBranchId;
                // echo $GLOBALS['searchedBranchId'];


                // echo "<pre>";
                // print_r($arrayStore);
                // echo "</pre>";
                // foreach ($arrayStore  as $arrayStoreValue) {
                //     if($arrayStoreValue==$searchedBranchId){
                //         echo "++++Matched+++++";
                //     }
                // }

                // if(array_search($searchedBranchId, $arrayStore)){
                //     echo "====Matched====";
                // }
                            ...
                            <div class="row">
                                ...
                                {{-- <div class="col-md-1">
                                    <div class="form-group" style="font-size: 13px; color:#212F3C">
                                        ...
                                        <div class="col-sm-12">
                                         @php
                                            $branch = array('' => 'Select') + DB::table('gnr_branch')->pluck('name','id')->all();
                                        @endphp
            ...
            <tbody>
              ...
              @foreach($ledgers as $ledger)
                  <?php
                      ...
                      $arrayBranches =splitProjectBranchId1($ledger->id, $searchedBranchId);

                      $matchValue=false;
                      foreach ($arrayBranches  as $arrayBranchValue) {
                            if($arrayBranchValue==$searchedBranchId){
                                $matchValue=true;
                            }else{
                                // $matchValue=false;
                            }
                ...
                @foreach($children1 as $child1)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child1->id, $searchedBranchId);
                ...
                @foreach($children2 as $child2)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child2->id, $searchedBranchId);
                ...
                @foreach($children3 as $child3)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child3->id, $searchedBranchId);
                ...
                @foreach($children4 as $child4)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child4->id, $searchedBranchId);
                ...
                @foreach($children5 as $child5)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child5->id, $searchedBranchId);
                ...
                @foreach($children6 as $child6)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child6->id, $searchedBranchId);
                ...
                $children7=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child6->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children7 as $child7)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child7->id, $searchedBranchId);
                ...
                $children8=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child7->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children8 as $child8)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child8->id, $searchedBranchId);
                ...
                $children9=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child8->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children9 as $child9)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child9->id, $searchedBranchId);
                ...
                $children10=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child9->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children10 as $child10)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child10->id, $searchedBranchId);
                        foreach ($arrayBranches  as $arrayBranchValue) {
                            if($arrayBranchValue==$searchedBranchId){
                                $matchValue=true;
                            }
                ...
                $children11=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child10->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children11 as $child11)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child11->id, $searchedBranchId);
                ...
                $children12=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child11->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children12 as $child12)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child12->id, $searchedBranchId);
                ...
                $children13=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child12->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children13 as $child13)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child13->id, $searchedBranchId);
                ...
                $children14=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child13->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children14 as $child14)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child14->id, $searchedBranchId);
                ...
                $children15=DB::table('acc_account_ledger')->where('companyIdFk', Auth::user()->company_id_fk)->where('parentId', $child14->id)->orderBy('ordering', 'asc')->get();
                ?>
                @foreach($children15 as $child15)
                    <?php
                        ...
                        $matchValue=false;
                        $arrayBranches=splitProjectBranchId1($child15->id, $searchedBranchId);
...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        var filterBranchNameValue = "<?php echo $searchedBranchId;?>";
        $('#filterBranchName').val(filterBranchNameValue);          
