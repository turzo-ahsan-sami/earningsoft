        ...
        <div class="row">
          ...
          {{--  <div class="form-group">
              ...
              <div class="col-sm-8">
                  ...
                  @php
                     $advPaymentIdList = array(''=>'Select Advance Payment Id') + DB::table('acc_adv_register')->where('status',1)->pluck('advRegId')->toArray();
                  @endphp

                  {!! Form::select('advPaymentId', $advPaymentIdList ,null, ['class'=>'form-control', 'id' => 'advPaymentId']) !!}

                  <p id='advPaymentIde' class="error" style="max-height:3px;color: red;"></p>
            ...
            <div class="form-group">
               ...
               <div class="col-sm-8">
                   ...
                   @php
                       $advPaymentIdLists =DB::table('acc_adv_register')->where('status',1)->select('advRegId','id')->get();
                   @endphp

                   <select class="form-control" id="advPaymentId" name="advPaymentId">
                       <option>Select Project Type</option>
                       @foreach( $advPaymentIdLists as $advPaymentIdList)
                          <option  value="{{$advPaymentIdList->id}}">{{$advPaymentIdList->advRegId}}</option>
                       @endforeach
                   </select>
                   <p id='advPaymentIde' class="error" style="max-height:3px;color: red;"></p>
...
$(document).ready(function() {
   $("#advRegName").on("change", function() {
       ...
       var advPaymentId = $("#advPaymentId").val();
       ...
       //alert(advPaymentId);
          $.ajax({
            ...
            data:{advRegChange:advRegChange,advRegName:advRegName,project:project,projectType:projectType,advRegType:advRegType,advPaymentId:advPaymentId,_token: csrf},
            dataType: 'json',
            async:false,
            success: function(data) {
              //alert(JSON.stringify(data['amountA']))
