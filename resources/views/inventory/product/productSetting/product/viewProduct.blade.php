...
@php
  $productIdsFromBranchRequisitionTable = DB::table('inv_branch_requisition_details')->distinct()->pluck('productId')->toArray();
  $productIdsFromEmployeeRequisitionTable = DB::table('inv_employee_requisition_details')->distinct()->pluck('productId')->toArray();
  $productIdsFromPurchaseTable = DB::table('inv_purchase_details')->distinct()->pluck('productId')->toArray();
  $productIdsFromPurchaseReturnTable = DB::table('inv_purchase_return_details')->distinct()->pluck('productId')->toArray();
  $productIdsFromStockTable = DB::table('inv_stock')->distinct()->pluck('productId')->toArray();
  $productIdsFromIssueTable = DB::table('inv_tra_issue_details')->distinct()->pluck('issueProductId')->toArray();
  $productIdsFromIssueReturnTable = DB::table('inv_tra_issue_return_details')->distinct()->pluck('productId')->toArray();
  $productIdsFromTransferTable = DB::table('inv_tra_transfer_details')->distinct()->pluck('transferProductId')->toArray();
  $productIdsFromUseTable = DB::table('inv_tra_use_details')->distinct()->pluck('productId')->toArray();
  $productIdsFromUseReturnTable = DB::table('inv_tra_use_return_details')->distinct()->pluck('productId')->toArray();
  
  $tempArray1 = array_merge($productIdsFromBranchRequisitionTable, $productIdsFromEmployeeRequisitionTable);
  $tempArray2 = array_merge($tempArray1, $productIdsFromPurchaseTable);
  $tempArray3 = array_merge($tempArray2, $productIdsFromPurchaseReturnTable);
  $tempArray4 = array_merge($tempArray3, $productIdsFromStockTable);
  $tempArray5 = array_merge($tempArray4, $productIdsFromIssueTable);

  $tempArray6 = array_merge($tempArray4, $productIdsFromIssueReturnTable);
  $tempArray7 = array_merge($tempArray4, $productIdsFromTransferTable);
  $tempArray8 = array_merge($tempArray4, $productIdsFromUseTable);
  $tempArray9 = array_merge($tempArray4, $productIdsFromUseReturnTable);

  $foreignProductIds = array_unique($tempArray9);
      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">
              <a href="{{url('addProduct/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add Product</a>
          </div>
        ...
        <div>
          ...
          <table class="table table-striped table-bordered" id="productView">
            <thead>
                    <tr>
                      <th width="30">SL#</th>
                      <th>Product Name</th>
                    ...
                    @foreach($products as $product)
                      <tr class="item{{$product->id}}">
                        ...
                        <td style="text-align: left; padding-left: 5px;">
                            <?php
                              $groupName = DB::table('inv_product_group')->select('name')->where('id',$product->groupId)->first();
                            ?>
                            {{$groupName->name}}
                        </td>
                        <td style="text-align: left; padding-left: 5px;">
                            <?php
                              $categoryName = DB::table('inv_product_category')->select('name')->where('id',$product->categoryId)->first();
                            ?>
                            {{$categoryName->name}}
                        </td>
                        <td style="text-align: left; padding-left: 5px;">
                            <?php
                              $subCategoryName = DB::table('inv_product_sub_category')->select('name')->where('id',$product->subCategoryId)->first();
                            ?>
                            {{$subCategoryName->name}}
                        </td>
                        <td style="text-align: left; padding-left: 5px;">
                            <?php
                              $productBrandName = DB::table('inv_product_brand')->select('name')->where('id',$product->brandId)->first();
                            ?>
                            {{$productBrandName->name}}
                        </td>
                        <td style="text-align: left; padding-left: 5px;">
                            <?php
                              $productModelName = DB::table('inv_product_model')->select('name')->where('id',$product->modelId)->first();
                            ?>
                            {{$productModelName->name}}
                        </td>
                        <td>
                            <?php
                              $ProductSizeName = DB::table('inv_product_size')->select('name')->where('id',$product->sizeId)->first();
                            ?>
                            {{$ProductSizeName->name}}
                        </td>
                        <td style="text-align: left; padding-left: 5px;">
                            <?php
                              $productColorName = DB::table('inv_product_color')->select('name')->where('id',$product->colorId)->first();
                            ?>
                        ...
                        <td style="text-align: right; padding-right: 5px;">{{$product->openingStockAmount}}</td>
                        ...
                        &nbsp
                        <a href="javascript:;" class="edit-modal" data-id="{{$product->id}}" data-name="{{$product->name}}" data-description="{{$product->description}}"  data-supplierid="{{$product->supplierId}}" data-groupid="{{$product->groupId}}" data-categoryid="{{$product->categoryId}}" data-subcategoryid="{{$product->subCategoryId}}" data-brandid="{{$product->brandId}}" data-modelid="{{$product->modelId}}" data-sizeid="{{$product->sizeId}}" data-colorid="{{$product->colorId}}" data-uomid="{{$product->uomId}}" data-costprice="{{$product->costPrice}}" data-salesprice="{{$product->salesPrice}}" data-openingstock="{{$product->openingStock}}" data-openingstockamount="{{$product->openingStockAmount}}" data-minimumstock="{{$product->minimumStock}}" data-vat="{{$product->vat}}" data-barcode="{{$product->barcode}}" data-systembarcode="{{$product->systemBarcode}}" data-warranty="{{$product->warranty}}" data-servicewarranty="{{$product->serviceWarranty}}" data-compresserwarranty="{{$product->compresserWarranty}}" data-image="{{$product->image}}" data-slno="{{$no}}">
                          <span class="glyphicon glyphicon-edit"></span>
                        </a>&nbsp

                        @php
                        if (in_array($product->id, $foreignProductIds)) {
                          $canDelete = 0;
                        }
                        else{
                          $canDelete = 1;
                      ...
                      @endphp
                        ...
                        <a id="deleteIcone" href="javascript:;" class="delete-modal" data-id="{{$product->id}}" @php if($canDelete==0){ echo "style=\"pointer-events: none;cursor: not-allowed;\"";}@endphp>
...
@include('inventory/product/productSetting/product/productDetails')
...
$(document).on('click', '.delete-modal', function() {
  if(hasAccess('deleteProductItem')){
  $('#MSGE').empty();
  $('#MSGS').empty();  
...
$(document).on('click', '.form5', function() {
  if(hasAccess('productDetail')){
    ...
    $.each(data, function( index, value ){
        ...
        $('#SupplierName').text(data["supplierName"].supplierCompanyName);
        ...
        $('#UOM').text(data["productUomName"].name);
