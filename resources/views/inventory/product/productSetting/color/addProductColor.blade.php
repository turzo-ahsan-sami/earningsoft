        ...
        <div class="col-md-2"></div>
            <div class="col-md-8 fullbody">
                <div class="viewTitle" style="border-bottom: 1px solid white;">
                    <a href="{{url('viewProductColor/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-th-list viewIcon">
                    </i>Product Color List</a>
                </div>
                <div class="panel panel-default panel-border">
                            <div class="panel-heading">
                                <div class="panel-title">New Product Color</div>
                            </div>
                    ...
                    {!! Form::open(array('url' => '', 'role' => 'form', 'class'=>'form-horizontal form-groups')) !!}
                        ...
                        <div class="form-group">
                            {!! Form::label('name', 'Color:', ['class' => 'col-sm-3 control-label']) !!}
                            <div class="col-sm-9">
                        ...
                        <div class="form-group">
                            {!! Form::label('productSizeId', 'Size:', ['class' => 'col-sm-3 control-label']) !!}
                            <div class="col-sm-9">
                            <?php 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                ...
                                <a href="{{url('viewProductColor/')}}" class="btn btn-danger closeBtn">Close</a>
                                <span id="success" style="color:green; font-size:20px;" class="pull-right"></span>
                            </div>
                        </div>
                    {!! Form::close() !!}
                                </div>
                                <div class="col-md-4 emptySpace vert-offset-top-6"><img src="images/catalog/image15.png" width="90%" height="" style="float:right">
                                </div>
                            </div>
                        </div>
                    </div>
...
$('form').submit(function( event ) {
    ...
    if (_response.errors) {
            ...
            if (_response.errors['productSizeId']) {
                $('#productSizeIde').empty();
                $('#productSizeIde').append('<span class="errormsg" style="color:red;">'+_response.errors.productSizeId+'</span>');
                //return false;
            }
    } else {
            $("#name").val('');
            ...
            window.location.href = '{{url('viewProductColor/')}}';
            }
        },
...
$(document).ready(function(){
        var productSubCategoryId = $('#productSubCategoryId').val();
        if(productSubCategoryId ==''){$("#productBrandId").prop("disabled", false);}
...
$("#productBrandId").change(function(){
    ...
    $.ajax({
        type: 'post',
        url: './productBrandIdSend',//product size controller
        data: $('form').serialize(),
        dataType: 'json',   
        success: function( _response ){
...
/*$(document).ready(function(){
        var productModelId = $('#productModelId').val();
        if(productModelId ==''){$("#productSizeId").prop("disabled", false);}
$("#productModelId").change(function(){
        $("#productSizeId").empty();
        $("#productSizeId").prepend('<option selected="selected" value="">Please Select Product Size</option>');
        var productModelId = $('#productModelId').val();
        if(productModelId!==''){$("#productSizeId").prop("disabled", false);}else{$("#productSizeId").prop("disabled", true);}
    $.ajax({
        type: 'post',
        url: './productModelIdSend',
        data: $('form').serialize(),
        dataType: 'json',   
        success: function( _response ){
            $.each(_response, function( index, value ){
                    $('#productSizeId').append("<option value='"+index+"'>"+value+"</option>");
                });
                }
            });
        });
