                    ...
                    {!! Form::open(array('url' => '', 'role' => 'form',  'class'=>'form-horizontal form-groups', 'id' => 'purchaseReturnViewForm')) !!}
                     ...
                     <div class="col-md-6">   
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <?php 
                                    $pruchaseBillNo = array('' => 'Please Select')+ DB::table('inv_purchase')->where('branchId', $gnrBranchId)->pluck('billNo','id')->all();  
                                ?>      
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8 text-right" style="">
                                ...
                                <a href="{{url('viewInvPurchaseReturnList/')}}" class="btn btn-danger closeBtn">Close</a>
...
// onclick add button quantity summation          
    ...
    //to remove append row row 
      $(document).on('click', '.btn_remove', function(){ 
           ...
           var totalAmountFromInput = parseFloat($('#totalAmount').val()); //alert(totalAmountFromInput);
           var grossTotal           = parseFloat($('#grossTotal').val());
           var discountPercent      = parseFloat($('#discountPercent').val());
           var discount             = parseFloat($('#discount').val()); //alert(discount);
 ...
 $('form').submit(function( event ) {
    ...
    $.ajax({
        ...
        //alert(JSON.stringify(_response));
            ...
            if (_response.errors) {
                if (_response.errors['purchaseBillNo1']) {
                    $('#purchaseBillNo1e').empty();
                    $('#purchaseBillNo1e').append('<span class="errormsg" style="color:red;">'+_response.errors.purchaseBillNo1+'</span>');
                    return false;
                }
        }
...
$('select').on('change', function (e) {
    var purchaseBillNo1 = $("#purchaseBillNo1").val();
    if(purchaseBillNo1){$('#purchaseBillNo1e').hide();}else{$('#purchaseBillNo1e').show();}
    var productId = $("#productId").val();
    if(productId){$('#productIdError').hide(); $('#productIde').hide();}else{$('#productIdError').show(); $('#productIde').show();}
    });

    $("#purchaseReturnViewForm").mouseover(function(){
        var rows= $('#addProductTable tbody tr.forhide').length;
        if(rows>0){
            //alert(rows);
            $("#purchaseBillNo1").prop('disabled', true);
        }else{$("#purchaseBillNo1").prop('disabled', false);}
    });      
