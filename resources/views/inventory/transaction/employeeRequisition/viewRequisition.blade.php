      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading"  style="padding-bottom:0px">
          <div class="panel-options">
              <a href="{{url('addInvEmpRequi/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add Requisition</a>
          </div>
            ...
            <tbody>
                  ...
                  @foreach($requisitions as $requisition)
                    <tr class="item{{$requisition->id}}">
                      ...
                      <td style="text-align: left; padding-left: 5px;">
                      	<?php
                            //$employeeId = DB::table('hr_emp_org_info')->where('id',$requisition->employeeId)->value('id');
                            $employeehName = DB::table('hr_emp_general_info')->select('emp_id','emp_name_english')->where('id',$requisition->employeeId)->first();
                            //$employeehName = DB::table('gnr_employee')->select('name')->where('id',$requisition->employeeId)->first();
                          ?>
                      	{{$employeehName->emp_id.'-'.$employeehName->emp_name_english}}
                      </td>
                      ...
                      <td class="text-center" width="80">
                        ...
                        <a id="editIcone" href="javascript:;" class="edit-modal" data-id="{{$requisition->id}}" data-requisitionno="{{$requisition->requisitionNo}}" data-branchid="{{$grnBranchId}}" data-description="{{$requisition->description}}" data-employeeid="{{$requisition->employeeId}}" data-totalquantity="{{$requisition->totalQuantity}}"  data-totalamount="{{$requisition->totalAmount}}" data-slno="{{$no}}">
       ...
       {!! Form::open(array('url' => '', 'role' => 'form',  'class'=>'form-horizontal form-groups')) !!}
                     ...
                     <div class="col-md-6">     
                         <div class="form-group">
                            ...
                            <div class="col-sm-8">
                               <?php 
                                    $requisitionMaxId = DB::table('inv_employee_requisition')->where('branchId',$grnBranchId)->count()+1;
                                    $valueForField = 'RE'.sprintf('%04d', $grnBranchId) . sprintf('%06d', $requisitionMaxId);
                                ?>
                    ...
                    <div class="col-md-6"> 
                        <div class="form-group">
                            ...
                            $employeeId = DB::table('hr_emp_org_info')->where('branch_id_fk',$grnBranchId)->pluck('emp_id_fk')->toArray();
                            $employeeIds = DB::table('hr_emp_general_info')->select('id','emp_id','emp_name_english')->whereIn('id',$employeeId)->get(); 
                            ?>
...
@include('inventory/transaction/employeeRequisition/requisitionDetails')
...
$.ajax({
        ...
        $('.item' + data["updateDatas"][0].id).replaceWith(
                                    "<tr class='item" + data["updateDatas"][0].id + "'><td  class='text-center slNo'>" + data.slno +
                                                                    ...
                                                                    "</td><td style='text-align:left; padding-left: 5px;'>" + data["employeehName"].name +
...
$(document).on('click', '.forEmpReqDetailsModel', function() {
  if(hasAccess('invProRequiDetails')){
   ...
   $('.modal-dialog').css('width','50%');
    ...
    $.ajax({
        ...
        $('#reqEmployeeHead').text(data['employeehName'].emp_id+'-'+data['employeehName'].emp_name_english);
        $('#showRequisitionNo').text(data['empRequisitions'][0].requisitionNo);
        $('#descriptionView').text(data['empRequisitions'][0].description);
...
$(document).on('click', '.delete-modal', function() {
  if(hasAccess('deleteProEmpReqItem')){
  $('#MSGE').empty();
  $('#MSGS').empty();
