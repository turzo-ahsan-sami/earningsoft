...
@section('title', '| Purchese Details Report')
@section('content')
@include('successMsg')
                ...
                <div class="panel panel-default" style="background-color:#708090;">
                    ...
                    <div class="row" id="filtering-group">
                        ...
                        <div class="form-horizontal form-groups" style="padding-right: 0px;">
                            ...
                            {!! Form::open(['url' => 'invPurchaseDetailsReport','method' => 'get']) !!}
                            @php
...
<h2 align="center" style="font-family: Antiqua;letter-spacing: 2px; color: white;margin-top: 0px;">Purchase Details Report </h2>
...
<div class="panel-body panelBodyView">
    ...
    $selectedCategoryName = DB::table('inv_product_category')->where('id',$categorySelected)->value('name');
    ...
    <div id="printDiv">
        <div id="printingContent">
            <div style="display: none;text-align: center;" id="hiddenTitle">
             ...
             <h4 style="text-align: center;padding: 0px;margin: 0px;">Purchase Details Report</h4>                          
...
<tbody>
   ...
   @php
   $productName = DB::table('inv_product')->where('id',$purchasesDetail->productId)->value('name');
   $projectName = DB::table('gnr_project')->where('id',$purchasesDetail->productId)->value('name');
   @endphp
   ...
   <tr>
       ...
       if ($key>0) {
          ...
          if ($purchasesDetails[$key-1]->purchaseDate!=$purchasesDetail->purchaseDate) {
           $isChangedForDate = 1;
       }
       if ($purchasesDetails[$key-1]->purchaseDate!=$purchasesDetail->purchaseDate) {
   ...
   if ($key == 0 || $isChangedForDate == 1) {
    $countPurchase=clone $countpurchasesDetails;
    $countDate = $countPurchase->where('purchaseDate',$purchasesDetail->purchaseDate)->count();

    $totalcount = $totalcount + $countDate;

    $countPurchase=clone $countpurchasesDetails;
    $countDateForSubQty = $countPurchase->where('purchaseDate',$purchasesDetail->purchaseDate)->sum('quantity');

    $countPurchase=clone $countpurchasesDetails;
    $countDateForSubTotal = $countPurchase->where('purchaseDate',$purchasesDetail->purchaseDate)->sum('totalPrice');
}

if ($key == 0 || $isChangedForBillNo == 1) {
    $countPurchase=clone $countpurchasesDetails;
    $countForBillNo = $countPurchase->where('purchaseDate',$purchasesDetail->purchaseDate)->count();
}
...
<td rowspan="{{$countDate}}" class="name">{{date('d-m-Y',strtotime($purchasesDetail->purchaseDate))}}</td>
@endif

@if($isChangedForBillNo==1 || $key==0)
<td rowspan="{{$countForBillNo}}" class="name">{{$purchasesDetail->billNo}}</td>
@endif
<td style="text-align:center;">{{$productName}}</td>
<td style="text-align:center;">{{$purchasesDetail->quantity}}</td>
<td style="text-align: right;padding-right: 5px;">{{number_format($purchasesDetail->totalPrice,2)}}</td>
</tr>
...
@php
$totalPurchaseAmount = $totalPurchaseAmount + $purchasesDetail->totalPrice; 
$totalPurchaseQuantity = $totalPurchaseQuantity + $purchasesDetail->quantity;
@endphp
...
<tr class="totalRow">
    <td colspan="4" style="text-align: center;font-weight: bold;font-size:15px">Total</td>  
    <td style="text-align:center;font-weight: bold;font-size:13px;">{{$totalPurchaseQuantity}}</td>                                                
    <td style="text-align: right;padding-right: 5px;font-weight: bold;font-size:13px;">{{number_format($totalPurchaseAmount,2)}}</td>
...
<script type="text/javascript">
  $(document).ready(function() {
   ...
   $("#btnExportExcel").click(function(e) {
        ...
        let a = $("<a />", {
          href: url,
          download: "Purchase Details Report_"+ today + ".xls"}).appendTo("body").get(0).click();
        e.preventDefault();
    });
