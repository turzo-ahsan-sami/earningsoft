...
<div class="row">
    <div class="col-md-12">
                ...
                <div class="panel panel-default" style="background-color:#708090;">
                    ...
                    <div class="row" id="filtering-group">
                        ...
                        <div class="form-horizontal form-groups" style="padding-right: 0px;">
                            ...
                            {!! Form::open(['url' => 'invPurchaseReport','method' => 'get']) !!}
                            @php
                    ...
                    <div class="col-md-1">
                        ...
                        <div class="col-sm-12">
                            <select name="searchBillNo" class="form-control input-sm" id="searchBillNo">
                                ...
                                <option value="{{$billNo->id}}" @if($billNo->id==$billSelected){{"selected=selected"}}@endif>{{$billNo->billNo}}</option>
                                @endforeach
                            </select>
        ...
        <h2 align="center" style="font-family: Antiqua;letter-spacing: 2px; color: white;margin-top: 0px;">Purchase Report </h2>
        ...
        <div id="printDiv">
            <div id="printingContent">
                <div style="display: none;text-align: center;" id="hiddenTitle">
                   ...
                   <h4 style="text-align: center;padding: 0px;margin: 0px;">Purchase Report</h4>                          
...
<tbody>
    ...
    $totalIPurchaseQuantity = 0;
    @endphp

    @foreach($purchase as $key => $purchase)

    @php
    $branchId = DB::table('gnr_branch')->where('id',$purchase->branchId)->value('id');
    $productName = DB::table('inv_product')->where('branchId',$purchase->branchId)->value('name'); 
    @endphp
    <tr>

      <td style="text-align: center;">{{$key+1}}</td>
      <td style="text-align: center;">{{date('d-m-Y',strtotime($purchase->purchaseDate))}}</td>
      <td style="text-align: center;">{{$purchase->billNo}}</td>
      <td style="text-align:center;">{{$productName}}</td>
      <td style="text-align:center;">{{$purchase->totalQuantity}}</td>
      <td style="text-align: right;padding-right: 5px;">{{number_format($purchase->totalAmount,2)}}</td>

  </tr>

  @php
  $totalPurchaseAmount = $totalPurchaseAmount + $purchase->totalAmount; 

  $totalIPurchaseQuantity = $totalIPurchaseQuantity + $purchase->totalQuantity;
  @endphp
  ...
  <tr class="totalRow">
    ...
    <td style="text-align:center;font-weight: bold;">{{$totalIPurchaseQuantity}}</td> 

    <td style="text-align: right;padding-right: 5px;font-weight: bold;">{{number_format($totalPurchaseAmount,2)}}</td>
