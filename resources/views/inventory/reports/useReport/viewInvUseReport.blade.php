                    ...
                    <div class="panel-heading" style="padding-bottom:0px">
                        ...
                        <div class="row" id="filtering-group">
                            ...
                            <div class="form-horizontal form-groups" style="padding-right: 0px;">
                                ...
                                {!! Form::open(['url' => 'viewUseReport','method' => 'get']) !!}
                                @php
...
<h2 align="center" style="font-family: Antiqua;letter-spacing: 2px; color: white;margin-top: 0px;">Use Report </h2>
    ...
    <div id="printDiv">
        <div id="printingContent">
            <div style="display: none;text-align: center;" id="hiddenTitle">
               ...
               <h4 style="text-align: center;padding: 0px;margin: 0px;">Use Report</h4>                          
...
<tbody>
    ...
    @foreach($useDetails as $key => $useDetail)

    @php

    $productName = DB::table('inv_product')->where('id',$useDetail->productId)->value('name');

    $emloyeeName = DB::table('inv_tra_use')->where('id',$useDetail->useId)->value('employeeId');

    $emloyeeNames = DB::table('hr_emp_general_info')->where('id',$emloyeeName)->value('emp_name_english');

    $branchId = DB::table('inv_tra_use')->where('id',$useDetail->useId)->value('branchId');

    $branchName = DB::table('gnr_branch')->where('id',$branchId)->value('name'); 
    $roomId= DB::table('inv_tra_use')->where('id',$useDetail->useId)->value('roomId');
    $deptId= DB::table('inv_tra_use')->where('id',$useDetail->useId)->value('departmentId');
    if($roomId>0){

        $deptName = DB::table('gnr_room')->where('id',$roomId)->value('name');
    }else{

        $deptName = DB::table('gnr_department')->where('id',$deptId)->value('name');
    }
    $useDate = DB::table('inv_tra_use')->where('id',$useDetail->useId)->value('useDate');
    @endphp
    ...
    <tr>
        ...
        <td style="text-align: center;">{{date('d-m-Y',strtotime($useDate))}}</td>
        <td style="text-align: center;">{{$useDetail->useBillNo}}</td>
        <td style="text-align:center;">{{$branchName}}</td>

        <td style="text-align:center;">{{$deptName}}</td>                        


        <td style="text-align:center;">{{$emloyeeNames}}</td> 
        <td style="text-align:center;">{{$productName}}</td>

        <td style="text-align:center;">{{$useDetail->productQuantity}}</td>
        <td style="text-align: right;padding-right: 5px;">{{number_format($useDetail->totalCostPrice,2)}}</td>

    </tr>

    @php
    $totalUserAmount = $totalUserAmount + $useDetail->totalCostPrice; 

    $totalUserQuantity = $totalUserQuantity + $useDetail->productQuantity;
    @endphp
    ...
    <tr class="totalRow">
        ...
        <td style="text-align:center;font-weight: bold;">{{$totalUserQuantity}}</td>                                                
        <td style="text-align: right;padding-right: 5px;font-weight: bold;">{{number_format($totalUserAmount,2)}}</td>
...
<script type="text/javascript">
  $(document).ready(function() {
   ...
   $("#btnExportExcel").click(function(e) {
        ...
        let a = $("<a />", {
          href: url,
          download: "Use Report_"+ today + ".xls"}).appendTo("body").get(0).click();
        e.preventDefault();
    });
