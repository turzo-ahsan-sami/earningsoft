...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    <p class="fontCustomize" style="text-align: center;padding: 0px;margin: 0px;color:#333;"><b>Purchase Return Report</b></p> 
  ...
  <table class="table table-striped table-bordered" id="posSalesView" width="100%" style="color:black;font-size:14px;margin-bottom:50px;border-collapse: collapse;" border= "1px solid black;" cellpadding="0" cellspacing="0">
    ...
    <tbody>
      ...
      @foreach($posPurchaseReturnReports as $purchasereturn)

        <tr class="item{{$purchasereturn->id}}">

          <td>{{++$no}}</td>          

          @if($purchasereturn->returnDate) <td style="text-align: center" rowspan="{{$purchasereturn->daterow}}">{{ date('d-m-Y', strtotime($purchasereturn->returnDate)) }}</td> @endif
          @if($purchasereturn->billNo) <td style="text-align: center" rowspan="{{$purchasereturn->billrow}}">{{ $purchasereturn->billNo }}</td> @endif

          <td style="text-align: left; padding-left: 5px">{{$purchasereturn->name}}</td> 
        
          <td style="text-align: center">{{$purchasereturn->quantity}}</td>
    
          <td style="text-align: right; padding-right: 5px">{{ number_format($purchasereturn->price, 2) }}</td>
          <td style="text-align: right; padding-right: 5px">{{ number_format($purchasereturn->total, 2) }}</td>

            @php
              $totalReturnQty += $purchasereturn->quantity;
              $totalUnitPrice += $purchasereturn->price;
              $totalAmount += $purchasereturn->total;
            @endphp
        ...
        <tr>
          ...
          <td style="text-align: center"><b>{{$totalReturnQty}}</b></td>
