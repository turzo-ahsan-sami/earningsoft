      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">          
              <a href="{{url('pos/addPurchaseReturn/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon">
                  </i>Add Purchase Return</a>
          </div>


            <h1 align="center" style="font-family: Antiqua;letter-spacing: 2px; color: white;">PURCHASES RETURN LIST</h1>
        </div>
                    ...
                    <tbody>
                      ...
                      @foreach($purchases as $purchase)
                        <tr class="item{{$purchase->id}}">
                          ...
                          <td style="color: black;text-align: center;">{{date('d-m-Y',strtotime($purchase->returnDate))}}</td>
                          <td style="color: black; text-align: center;" >{{$purchase->billNo}}</td>
                          <td style="color: black; text-align: left; padding-left: 5px;">{{$purchase->supName}}</td>                       
                          <td style="color: black; text-align: center;">{{$purchase->qty}}</td>                      
                          <td style="color: black; text-align: right; padding-right: 5px;">{{$purchase->totalAmount}}</td>                                           
                          <td>
                            {{--  <a href="javascript:void(0);" onclick='viewPurchaseItem("{{$purchase->id}}");'>
                               <span class="fa fa-eye"></span>
                             </a> --}}
                             <a href="{{ url('pos/viewPurchaseReturnItem/'.$purchase->id) }}">
                                <span class="fa fa-eye"></span>
                            </a>                 
                             <a href="javascript:;" onclick="editOption({{ $purchase->id }})">
                                <span class="glyphicon glyphicon-edit"></span>
                            </a>
                          
                           <a href="javascript:;" class="delete-modal" data-id="{{ $purchase->id }}">
...
<script type="text/javascript">
  ...
  function viewPurchaseItem(purchaseId) {

     $.ajax({
            type: 'get',
            url: './viewPurchaseItem',
            data: {'purchaseId': purchaseId},

            success: function(data) {
              //alert(purchaseId); location.reload();
                 window.location.href = "{{ url('pos/viewPurchaseItem/')}}";
                //location.reload();
            },
            error: function(data ){
                alert('Error');
  ...
  {
      $.ajax({
          url:'./monthEndCheckForPurchaseReturn',
          type: 'GET',
          data: {purchaseReturnId:value},
          dataType: 'json',
          success: function(data) {
              if(data.status == true) window.location.href = "{{ url('pos/editPurchaseReturnItem/') }}/"+value;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                else
                {
                      ...
                      $('.modal-title').text('Delete Purchase Return');
                      $('.modal-header').css({"background-color":"black", "color":"white", "padding":"10px"});
                      $('.modal-dialog').css('width','30%');
                      $('.id').text(purchaseReturnId);
                      $('.deleteContent').show();
