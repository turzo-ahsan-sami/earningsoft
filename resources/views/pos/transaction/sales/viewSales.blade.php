      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading"  style="padding-bottom:0px">
          <div class="panel-options">
              <a href="{{url('pos/addPosSalesRequiF/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add Sales</a>
          </div>
          <h1 align="center" style="font-family: Antiqua;letter-spacing: 2px"><font color="white">SALES LIST</font></h1>
        </div>
            ...
            <tbody>
               <?php $no=0; ?>
                  @foreach($posSales as $posSales)
                    @if($posSales->salesType==1)
                     <tr class="item{{$posSales->id}}">
                      <td>{{++$no}}</td>
                      <td style="text-align: center;">{{date('d-m-Y', strtotime($posSales->salesDate))}}</td>
                      <td style="text-align: left; padding-left: 5px;">{{'SB000'.$posSales->salesBillNo}}</td>
                        <td style="text-align: left; padding-left: 5px;">
                        @if($posSales->salesType==1)
                          {{'Sales'}}
                        @elseif($posSales->salesType==2)
                         {{'Service Charge'}}
                        @else
                          {{'Null Type'}}
                      ...
                      <td class="text-center" width="80">
                        <a href="{{url('pos/salesInvoicePrint/').'/'.$posSales->id}}" class="princtSalesInvoiceId" data-token="{{csrf_token()}}" data-id="{{$posSales->id}}">
                        ...
                        @elseif($posSales->salesType==2)
                         <a href="javascript:;" class="serviceDetailsModels" data-token="{{csrf_token()}}" data-id="{{$posSales->id}}">
                              <span class="fa fa-eye"></span>
                          </a>&nbsp
                        @endif  
                        @if($posSales->salesType==1)  
                         
                            <a href="javascript:;" class="sales-modal" data-id="{{$posSales->id}}">
                              <span class="glyphicon glyphicon-edit"></span>
                            </a>&nbsp
                        @elseif($posSales->salesType==2) 
                           <a  href="javascript:;" class="service-modal" data-id="{{$posSales->id}}">
                              <span class="glyphicon glyphicon-edit"></span>
                            </a>&nbsp
                        @endif 
...
<div id="myModal" class="modal fade" style="margin-top:3%">
  <div class="modal-dialog">
      ...
      <div class="modal-body">
       {!! Form::open(array('url' => '', 'role' => 'form',  'class'=>'form-horizontal form-groups', 'id' => 'salesForm')) !!}
                     ...
                     <div class="col-md-6">   
                        ...
                        <div class="form-group">
                            {!! Form::label('salesDate', 'Sales Date:', ['class' => 'col-sm-4 control-label']) !!}
                            <div class="col-sm-8">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            {!! Form::label('due', 'Sales Due:', ['class' => 'col-sm-4 control-label']) !!}
                            <div class="col-sm-8">
    ...
    @include('pos/transaction/sales/salesDetails')
...
$(".servicePaymentType").click(function(event) {
   var bankStatmentValue = $('input[name=servicePaymentType]:checked').val();
   if(bankStatmentValue==2){
    $("#servicebankStatment").show();
   } else {
    $("#servicebankStatment").hide();
   }
});
/*$(document).ready(function() {
$(".princtSalesInvoiceId").click(function(event) {
  var salesId = $(this).data('id');
  var csrf = "<?php echo csrf_token(); ?>";
  $.ajax({
         type: 'post',
         url: './salesInvoicePrint',
         data:{id:salesId,_token: csrf},
         dataType: 'json',
         success: function(data){
            
            }
        });//End sales Invoice Print Click Function
    });//End sales Invoice Print Ajax Function
});//End sales Invoice Print Ready Function*/
$( document ).ready(function() {
  ...
  $('#serviceEmployeeId').next('span').css('width', '100%');;
...
//Sales Discount percentage calculation
...
// Calculation VAT Rate for Sales
...
//Gross total calculation For Sales
$("#vatPercent, #totalAmount, #amountAfterDiscount, #discountPercent, #discount").on("input",function(e) { // input on change
...
//Pay amount/due amount calculation for Sales
    ...
    /* Service  Calculation Jquary */

    // Calculation VAT Rate for Sservice
$("#servicevatPercent, #serviceTAmount, #serviceamountAfterDiscount, #serviceDiscountPercent, #serviceDiscount").on("input",function(e) { // input on change
    var vatPercent            = parseFloat($('#servicevatPercent').val()); 
    var amountForVat          = parseFloat($("#serviceTAmount").val()); 
    var afterDiscountForVat   = parseFloat($("#serviceamountAfterDiscount").val());
            if(isNaN(afterDiscountForVat) || afterDiscountForVat==0){
                var varBeforeDisc = parseFloat(Math.round(amountForVat*vatPercent) / 100).toFixed(5).replace(/\.0+$/,''); //alert(varBeforeDisc);
                    if(isNaN(varBeforeDisc)){$('#servicevat').val('');}
                    else{$('#servicevat').val(varBeforeDisc||'');}   
            }
            else if(isNaN(afterDiscountForVat) && amountForVat){
                var varBeforeDisc = parseFloat(Math.round(afterDiscountForVat*vatPercent) / 100).toFixed(5).replace(/\.0+$/,''); //alert(varBeforeDisc);
                    if(isNaN(varBeforeDisc)){$('#servicevat').val('');}
...
//Service Discount percentage calculation
$("#serviceDiscount").on("input",function(e) {
  var discountPer = parseFloat($("#serviceDiscountPercent").val()); //alert(discountPer);
  var tAmount = parseFloat($("#serviceTAmount").val()); //alert(tAmount);
  var discountAmount = parseFloat($("#serviceDiscount").val()); //alert(discountAmount);
  var discountRate = parseFloat(Math.round(100*discountAmount ) /tAmount);
    
    var totalAfterDiscount = parseFloat(Math.round(tAmount-discountAmount)).toFixed(5).replace(/\.0+$/,''); //alert(totalAfterDiscount);
    if(isNaN(totalAfterDiscount) || totalAfterDiscount==''){$('#serviceamountAfterDiscount').val(tAmount||''); $('#serviceDiscountPercent').val('');}
        else{ 
        $('#serviceDiscountPercent').val(discountRate);
        $('#serviceamountAfterDiscount').val(totalAfterDiscount); 
        } 
})

// Service Discount Calcualtion
$("#serviceDiscountPercent, #serviceTAmount").on("input",function(e) { // input on change
    var percent = parseFloat($('#serviceDiscountPercent').val()); //alert(percent);
    var tAmount = parseFloat($("#serviceTAmount").val()); //alert(tAmount);
    
    var result = parseFloat(Math.ceil(tAmount*percent) / 100).toFixed(5).replace(/\.0+$/,''); //alert(result);
    var totalAfterDiscount = parseFloat(Math.round(tAmount-result)).toFixed(5).replace(/\.0+$/,''); //alert(result);
  
    if(isNaN(result)){ $('#serviceDiscount').val(''); $('#serviceamountAfterDiscount').val(tAmount||'');}
    else{$('#serviceDiscount').val(result||''); $('#serviceamountAfterDiscount').val(totalAfterDiscount||''); }
  })

///Gross total calculation for Service
$("#servicevatPercent, #serviceTAmount, #serviceamountAfterDiscount, #serviceDiscount, #serviceDiscountPercent").on("input",function(e) {

    var totalVat                    = parseFloat($('#servicevat').val()); //alert(totalVat);
    var amountForGrosstotal         = parseFloat($("#serviceTAmount").val()); //alert(amountForGrosstotal);
    var amountAfterDiscountForGross = parseFloat($("#serviceamountAfterDiscount").val()); //alert(amountAfterDiscountForGross);
    var valueOfPayAmountField       = parseFloat($('#servicePayAmount').val()); //alert(valueOfPayAmountField);
    var grossTotalForDue            = parseFloat($('#serviceGrossTotal').val()); //alert(grossTotalForDue);

      if(isNaN(totalVat) && isNaN(amountAfterDiscountForGross)){  //alert('1');
        $('#serviceGrossTotal').val(amountForGrosstotal||'');
        
        $('#serviceDue').val(dueAfterPayAmount||'');
      }
      
      else if(amountAfterDiscountForGross && isNaN(totalVat)){ //alert('3');
        var dueAfterPayAmount = parseFloat(amountAfterDiscountForGross-valueOfPayAmountField);
        $('#serviceGrossTotal').val(amountAfterDiscountForGross||'');
        $('#serviceDue').val(dueAfterPayAmount);  
      }
      else if(isNaN(amountAfterDiscountForGross) && totalVat){ //alert('4');
        var dueAfterPayAmount = parseFloat(grossTotalAmount-valueOfPayAmountField);
        $('#serviceGrossTotal').val(grossTotalAmount||''); 
        $('#serviceDue').val(dueAfterPayAmount);
      }
      else if(amountAfterDiscountForGross && totalVat){ //alert('5');
        var grossTotalAmount = parseFloat(Math.ceil(amountAfterDiscountForGross+totalVat));
        ...
        $('#serviceGrossTotal').val(grossTotalAmount); //alert(dueAfterPayAmount);
        ...
        $('#serviceDue').val(dueAfterPayAmount||'');
           ...
           $('#serviceGrossTotal').val(amountAfterDiscountForGross||'');
           $('#servicePayAmount').val(amountAfterDiscountForGross||'');
           $('#servicevat').val('');
        }
      //$('#due').val(0);    
  })

 //Pay amount/due amount calculation for Service
    $("#servicePayAmount, #serviceDue").on("input",function(e) { 
      var grossTotalForDue = parseFloat($("#serviceGrossTotal").val());
      var payAmount = parseFloat($("#servicePayAmount").val());
      if(isNaN(payAmount)){$('#serviceDue').val(grossTotalForDue);}
       else if(isNaN(grossTotalForDue)){
            grossTotalForDue = 0;
      }else{
        var dueAmount = grossTotalForDue-payAmount;
        if(payAmount>grossTotalForDue){
            $("#serviceDuee").html("Pay Amount Should not be more than gross total").show().fadeOut("slow");
            $("#serviceDuee").css("color", "red");
                  event.preventDefault();
        }
        else if(isNaN(dueAmount)){$('#serviceDue').val(0);}
        else{$('#serviceDue').val(dueAmount);}
      }
    });
       ...
       var csrf = "<?php echo csrf_token(); ?>";
        $.ajax({
         ...
         success: function(data){
            ...
            $('#proTotalAmountShowFooter').html((data['salesTables'].totalSalesAmount));
...
$("#addProductsTable").hover(function(){
    $(".apnQnt,.productTotalPriceApnTable").on('input',function(){
        var getApnProductQty  = $(this).closest('tr').find('.apnQnt').val();
        var getApnProPerpri   = $(this).closest('tr').find('.productTotalPriceApnTable').val();
        var totalAmountForApnR  = parseFloat(Math.ceil(getApnProductQty*getApnProPerpri));
      ...
      sum += Number($(this).val());
      $('#totalQty').val(sum);
      $('#totalQtyFooter').text(sum);
    });

    $('.productPriceApnTable').each(function() {
      total += Number($(this).val());
      $('#serviceTAmount').val(total);
      $('#proTAmountFooter').text(total);
      });

$("#serviceTAmount").trigger('input');
$("#serviceGrossTotal").trigger('input');
...
$.ajax({
         type: 'post',
         url: './editPosProductItem',
         data: $('#salesForm').serialize(),
         dataType: 'json',
         success: function( data ){
          //alert(JSON.stringify(data));
          if(data=='false'){$('#productIde').show(); $('#productIde').removeClass('hidden'); return false;}
          if(data=='false'){$('#productIdError').show(); $('#productIdError').removeClass('hidden'); return false;}
                if (data.errors) {
    ...
    $.ajax({
        ...
        $.each(value, function (key1,value1) {
          ...
          var string="<tr>";
            ...
            string+="<td class='productPricePerPc'>"+value1.unitPrice+"</td>";
            string+="<td class='productPricePerPc'>"+value1.totalAmount+"</td>";
            string+="</tr>";
      ...
      if (key == "salesDetailsTable") {
         ...
         $.each(value, function (key2, value2) {
            var strin="<tr>";
              ...
              strin+="<td class='productPricePerPc'style='font-size:14px; text-align:right; padding-right:5px;'>"+(value2.unitPrice)+"</td>";
              strin+="<td class='productPricePerPc'style='font-size:14px; text-align:right; padding-right:5px;'>"+(value2.totalAmount)+"</td>";
              strin+="</tr>";
          ...
          $('#printTotalAmountDetails').html((data['salesDetails'][0].totalSalesAmount));
...
$(document).on('click', '.serviceDetailsModels', function() {

   $('.modal-header').css({"background-color":"white"});
   //$('#swhoAppendRowsService tbody').empty();
   $('#swhoAppendRows tbody').hide();
   $('#serviceDetailsModel').modal('show');
   $('.modal-dialog').css('width','50%');
    var id = ($(this).data('id'));
    ...
    $.ajax({
        ...
        success: function( data ){
          ...
          $('#purDateHead').text(data.createdDate);    
          $('#billNoHeadService').text(data['salesDetails'][0].salesBillNo);
          $('#salesDateHeadService').text(data.dateFromarte);
          $('#companyHeadService').text(data['companyName'].clientCompanyName);
          $('#SalesPerHeadService').text(data['personName']);
          $('#totalQtyHeadService').text(data['salesDetails'][0].totalSalesQuantity);
          $('#totalAmountHeadService').text(data['salesDetails'][0].totalSalesAmount);
          $('#payAmountHeadService').text(data['salesDetails'][0].salesPayAmount);
          $('#dueAmountHeadService').text(data['salesDetails'][0].salesDue);
        ...
        $.each(value, function (key1,value1) {
          ...
          var string="<tr>";
            string+="<td style='text-align:center' class='slNoUse'>"+z+"</td>";
            string+="<td class='productName' style='text-align:left;' id='productRowName"+z+"'>"+data.productName[key1]+"</td>";
            if(value1.branchId==1){
               string+="<td class='branchName' style='text-align:left;' id='branchId"+z+"'>"+headOffice+"</td>";
            }
            if(value1.branchId==2){
               string+="<td class='branchName' style='text-align:left;' id='branchId"+z+"'>"+branch+"</td>";
            }
            if(value1.salesType==2){
               string+="<td class='serviceDate' style='text-align:left;' id='serviceDate"+z+"'>"+value1.serviceDate+"</td>";
            }
            string+="<td class='productPricePerPc'>"+value1.salesProductQuantity+"</td>";
            string+="<td class='productPricePerPc'>"+value1.unitPrice+"</td>";
            string+="<td class='productPricePerPc'>"+value1.totalAmount+"</td>";
            string+="</tr>";
            $('#swhoAppendRowsService').append(string);
          ...
          $('#totalQtyDetailsService').val(data['salesDetails'][0].totalSalesQuantity);
          $('#totalAmountDetailsService').val(data['salesDetails'][0].totalSalesAmount);
          $('#PrintTotalQtyDetails').html(data['salesDetails'][0].totalSalesQuantity);
          $('#printTotalAmountDetails').html((data['salesDetails'][0].totalSalesAmount));
    ...
    $("#CompanyName").change(function(){
       ...
       $("#CompanyId").change(function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    $("#product").empty();
                    $("#product").prepend('<option selected="selected" value="">All</option>');
  ...
  $("#branchId1").change(function(){
   ...
   $("#branch").change(function(){
           ...
           $.ajax({
                ...
                success: function( data ) {
                    //alert(JSON.stringify(data['productPrice']));
                    $("#productPriceAdd").val(data['productPrice']);
