      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">        
            <a href="{{url('pos/addSale/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon">
                  </i>Add Sales</a>
          </div>
          ...
          <table class="table table-striped table-bordered" id="famsSaleTable">
                    <thead>
                      <tr>
                        ...
                        <th width="%">Sale Bill Number</th>
                        <th width="25%">Customers</th>
                        <th width="15%">Total Received Quantity</th>                        
                        <th width="15%">Total Sale Amount</th>                                                                   
                    ...
                    <tbody>
                      ...
                      @foreach($sales as $sale)
                        <tr class="item{{$sale->id}}">
                          ...
                          <td style="color: black;text-align: center;">{{$sale->salesDate}}</td>
                          <td style="color: black;">{{$sale->saleBillNo}}</td>
                          <td style="color: black; text-align: left;padding-left: 5px;">{{$sale->customerName}}</td>                      
                          <td style="color: black;">{{$sale->quantity}}</td>                      
                          <td style="color: black; text-align: right; padding-right: 5px;">{{$sale->totalAmount}}</td>                                      
                          <td>
                             <a href="{{ url('pos/viewSalesDetailsItemId/'.$sale->id) }}">
                                <span class="fa fa-eye"></span>
                            </a>       
                            <!-- {{ url('pos/editsalesItem/'.$sale->id) }} -->
...
<script type="text/javascript">
  ...
  function viewPurchaseItem(salesId) {
  ...
  {
      $.ajax({
          url:'./monthEndCheckForSales',
          type: 'GET',
          data: {salesId:value},
          dataType: 'json',
          success: function(data) {

              if(data.status == true) window.location.href = "{{ url('pos/editsalesItem/') }}/"+value;
