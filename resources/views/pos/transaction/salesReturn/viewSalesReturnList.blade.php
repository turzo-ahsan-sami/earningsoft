          ...
          <div class="panel panel-default" style="background-color:#708090;">
            <div class="panel-heading" style="padding-bottom:0px">
              <div class="panel-options">
                <a href="{{url('pos/addSalesReturn/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon">
                </i>Add Sales Return</a>
                
              </div>
              <h1 align="center" style="font-family: Antiqua;letter-spacing: 2px; color: white;">SALES RETURN LIST</h1>
            </div>
              ...
              <table class="table table-striped table-bordered" id="famsSaleTable">
                <thead>
                  <tr>
                    ...
                    <th width="15%">Total Sales Return Amount</th>
                ...
                <tbody>
                  ...
                  <tr class="item{{$sale->id}}">
                    ...
                    <td style="color: black;text-align: center;">{{$sale->returnDate}}</td>
                    <td style="color: black; text-align: center;">{{$sale->billNo}}</td>
                    <td style="color: black; text-align: left; padding-left: 5px;">{{$sale->cusName}}</td>
                    <td style="color: black; text-align: center;">{{$sale->qty}}</td>
                    <td style="color: black; text-align: right; padding-right: 5px;">{{$sale->totalAmount}}</td>
                    <td>
                      {{--  <a href="javascript:void(0);" onclick='viewPurchaseItem("{{$sale->id}}");'>
                        <span class="fa fa-eye"></span>
                      </a> --}}
                      <a href="{{ url('pos/viewSalesReturnItem/'.$sale->id) }}">
                        <span class="fa fa-eye"></span>
                      </a>
                      <a href="javascript:;" onclick="editOption({{ $sale->id }})">
                        <span class="glyphicon glyphicon-edit"></span>
                      </a>
                      
                      <a href="javascript:;" class="delete-modal" data-id="{{ $sale->id }}">
  ...
  {
  $.ajax({
  url:'./monthEndCheckForSalesReturn',
  type: 'GET',
  data: {salesReturnId:value},
  dataType: 'json',
  success: function(data) {

  if(data.status == true) window.location.href = "{{ url('pos/editSalesReturnItem/') }}/"+value;
  ...
  $('.modal-title').text('Delete Sales Return');
  $('.modal-header').css({"background-color":"black", "color":"white", "padding":"10px"});
  $('.modal-dialog').css('width','30%');
  $('.id').text(salesReturnId);
  $('.deleteContent').show();
