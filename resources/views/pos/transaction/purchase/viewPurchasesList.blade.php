      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">          
              <a href="{{url('pos/posAddPurchase/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon">
                        </i>Add Purchase</a>
          </div>
            <h1 align="center" style="font-family: Antiqua;letter-spacing: 2px; color: white;">PURCHASES LIST</h1>
        </div>
                ...
                <tbody>
                  ...
                  @foreach($purchases as $purchase)
                    <tr class="item{{$purchase->id}}">
                      ...
                      <td style="color: black;text-align: center;">{{date('d-m-Y',strtotime($purchase->purchaseDate))}}</td>
                      <td style="color: black;">{{$purchase->billNo}}</td> 
                      <td style="color: black; text-align: left; padding-left: 5px;">{{$purchase->supName}}</td>                              
                      <td style="color: black;">{{$purchase->qty}}</td>                      
                      ...
                      <td>
                        {{--  <a href="javascript:void(0);" onclick='viewPurchaseItem("{{$purchase->id}}");'>
                          ...
                          <a href="{{ url('pos/viewPurchaseItem/'.$purchase->id) }}">
                            <span class="fa fa-eye"></span>
                        </a>                 
...
<script type="text/javascript">
  ...
  {
      $.ajax({
          url:'./monthEndCheckForPurchase',
          type: 'GET',
          data: {purchaseId:value},
          dataType: 'json',
          success: function(data) {
              if(data.status == true) window.location.href = "{{ url('pos/editPurchaseItem/') }}/"+value;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                else
                {
                      ...
                      $('.modal-title').text('Delete Purchase');
