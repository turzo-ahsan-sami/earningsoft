...
@php
  $foreignModelIds = DB::table('pos_product')->distinct()->pluck('modelId')->toArray(); 
@endphp
                ...
                <div class="panel panel-default" style="background-color:#708090;">
                    <div class="panel-heading" style="padding-bottom:0px">
                        <div class="panel-options">
                            <a href="{{url('pos/posAddProductModel/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add Model</a>
                        </div>
                                ...
                                @foreach($productModels as $productModel)
                                    <tr class="item{{$productModel->id}}">
                                        ...
                                        <td style="padding-left: 25px; text-align: left;">
                                            <?php
                                              $productGroupName = DB::table('pos_product_group')->select('name')->where('id',$productModel->productGroupId)->first();
                                            ?>
                                            {{$productGroupName->name}}
                                        </td>
                                        <td style="padding-left: 5px; text-align: left;">
                                            <?php
                                              $productCategoryName = DB::table('pos_product_category')->select('name')->where('id',$productModel->productCategoryId)->first();
                                            ?>
                                            {{$productCategoryName->name}}
                                        </td>
                                        <td style="padding-left: 5px; text-align: left;">
                                            <?php
                                              $productSubCategoryName = DB::table('pos_product_sub_category')->select('name')->where('id',$productModel->productSubCategoryId)->first();
                                            ?>
                                            {{$productSubCategoryName->name}}
                                        </td>
                                        <td style="padding-left: 5px; text-align: left;">
                                            <?php
                                              $productBrandName = DB::table('pos_product_brand')->select('name')->where('id',$productModel->productBrandId)->first();
                                            ?>
