...
@php
  $foreignSubCategoryIds = DB::table('pos_product')->distinct()->pluck('subCategoryId')->toArray();
@endphp
      ...
      <div class="panel panel-default" style="background-color:#708090;">
        <div class="panel-heading" style="padding-bottom:0px">
          <div class="panel-options">
              <a href="{{url('pos/posAddProductSubCategory/')}}" class="btn btn-info pull-right addViewBtn"><i class="glyphicon glyphicon-plus-sign addIcon"></i>Add SubCatagory</a>
          </div>
                  ...
                  @foreach($productSubCategories as $productSubCategorie)
                    <tr class="item{{$productSubCategorie->id}}">
                      ...
                      <td style="padding-left: 5px; text-align: left;">
                          <?php
                            $productGroupName = DB::table('pos_product_group')->select('name')->where('id',$productSubCategorie->productGroupId)->first();
                          ?>
                          {{$productGroupName->name}}
                      </td>
                      <td style="padding-left: 5px; text-align: left;">
                          <?php
                            $productCategoryName = DB::table('pos_product_category')->select('name')->where('id',$productSubCategorie->productCategoryId)->first();
                          ?>
...
$(document).on('click', '.edit-modal', function() {
 ...
 /* }*/
  ...
  $.ajax({
      type: 'post',
      url: './posEditProductSubCategoryItem',
      data: $('form').serialize(),
      dataType: 'json',
...
$('.modal-footer').on('click', '.delete', function() {
  $.ajax({
    type: 'post',
    url: './posDeleteProductSubCategoryItem',
    data: {
