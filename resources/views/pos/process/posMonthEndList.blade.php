	...
	<div class="panel panel-default" style="background-color:#708090;">
		<div class="panel-heading" style="padding-bottom:0px">
			<div class="panel-options">
              <a id="exeDayEnd" href="javascript:;" class="btn btn-info pull-right addViewBtn"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i> Execute Month End</a>
            </div>
			<h1 align="center" style="font-family: Antiqua;letter-spacing: 2px"><font color="white">Month END LIST</font></h1>
		</div>
		<div class="panel-body panelBodyView">
                 {{-- Filtering --}}
      {!! Form::open(array('url' => 'pos/posMonthEndList/', 'role' => 'form', 'class'=>'form-horizontal form-groups', 'id' => 'filterFormId', 'method'=>'get')) !!}
      <div class="row">
        <div class="col-md-12">
      ...
      {{-- End Filtering --}}
        <table class="table table-striped table-bordered" id="areaListView">
                <thead>
                    ...
                    <tbody>
                        @foreach ($monthEnds as $key => $monthEnd)
                        <tr>
                          <td>{{(($pageNum-1) * 12) + $key+1}}</td>
                          <td>{{date('F Y',strtotime($monthEnd->date))}}</td>
                          <td class="name">{{$branchName}}</td>

                          <td>
                              @if ($key==0)
                                  <a href="javascript:;" class="deleteConfirmation" data-id="{{ $monthEnd->id }}">
