...
class GnrUpazilaController extends Controller {
	...
	public function index() {
		...
		return view('gnr.tools.address.upazila.viewUpazila', ['route'=>$this->route, 'upazilas' => $upazilas, 'TCN' => $TCN]);
	}

	public function addUpazila() {

		return view('gnr.tools.address.upazila.addUpazila', ['route'=>$this->route]);
	}
		...
		*/
		public function addItem(Request $req) {
			...
			if($validator->fails()) 
				...
				$logArray = array(
					'moduleId'  => 7,
					'controllerName'  => 'GnrUpazilaController',
					'tableName'  => 'gnr_upzilla',
					'operation'  => 'insert',
					'primaryIds'  => [DB::table('gnr_upzilla')->max('id')]
				);
		...
		*/
		public function updateItem(Request $req) {
			...
			if($validator->fails()) 
				...
				$previousdata = GnrUpazila::find ($req->id);
		...
		*/
		public function deleteItem(Request $req) {
			...
			$isAassigned =(int) DB::table('gnr_union')
			->where('upzillaId', $req->id)
			->value('id');

			if ($isAassigned > 0) {
				$data = array(
					'responseTitle' => 'Warning!',
					'responseText' => 'You can not delete this because it is assign to Union.'
				);
