...
use App\microfin\settings\WeeklyHoliday;
...
class WeeklyHolidayController extends Controller {
 ...
 public function viewWeeklyHoliday(){
   $weeklyholidays = WeeklyHoliday::active()->get();
   return view("gnr.tools.holiday.weeklyHoliday.viewWeeklyHoliday",['weeklyholidays'=>$weeklyholidays, 'route'=>$this->route]);
 }


 public function addWeeklyHoliday(){
   return view("gnr.tools.holiday.weeklyHoliday.addWeeklyHoliday", ['route'=>$this->route]);
 }




 public function storeWeeklyHoliday(Request $req){

   $rules = array(
    'weeklyHolidayIds'      =>  'required',
    'dateFrom'      =>  'required'
  ...
  $preDate = date('Y-m-d',strtotime($dateFrom . "-1 days"));
  $lastID = DB::table('mfn_setting_weekly_holiday')
  ->orderBy('id', 'desc')
  ->first();
  //dd($lastID);
  DB::table('mfn_setting_weekly_holiday')
  ->where('id',$lastID->id)
  ->update([
    'dateTo' =>$preDate,
    'status' =>0
  ...
  $dateFrom = Carbon::parse($req->dateFrom);
  $weeklyHolidayIds = implode(",", $req->get('weeklyHolidayIds'));
  $weeklyHoliday->dateFrom       = $dateFrom;
  $weeklyHoliday->created_at       =$createdDate;
  $weeklyHoliday->weeklyHolidayIds       =$weeklyHolidayIds;
  $weeklyHoliday->status       =1;
  $weeklyHoliday->created_by       =  $userId;
  $weeklyHoliday->save();
  ...
  $logArray = array(
    'moduleId'  => 7,
    'controllerName'  => 'WeeklyHolidayController',
    'tableName'  => 'mfn_setting_weekly_holiday',
    'operation'  => 'insert',
    'primaryIds'  => [DB::table('mfn_setting_weekly_holiday')->max('id')]
  );
  Service::createLog($logArray);
...
public function deleteWeeklyHoliday(Request $req) {
 $previousdata=WeeklyHoliday::find($req->id);
 $weeklyHoliday = WeeklyHoliday::find($req->id);
 $weeklyHoliday->softDel = 1;
 $weeklyHoliday->save();
...
public function getWeeklyHolidayDetails(Request $req){
  $weeklyholiday = WeeklyHoliday::find($req->id);
  $str = "$weeklyholiday->weeklyHolidayIds";
  $weeklyholidayArr=explode(",",$str);
  $checkboxValue = [];
  foreach ($weeklyholidayArr as $value) {

    array_push($checkboxValue, $value);

  }
  $data = array(
    'weeklyholiday'   => $weeklyholiday,
    'checkboxValue'  => $checkboxValue,
    'dateFrom'    => date('d-m-Y',strtotime($weeklyholiday->dateFrom))
...
public function updateWeeklyHoliday(Request $req){
