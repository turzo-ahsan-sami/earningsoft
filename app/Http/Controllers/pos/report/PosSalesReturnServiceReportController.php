...
class PosSalesReturnServiceReportController extends Controller
{
    ...
    public function index(Request $req){
        ...
        $pos_sales = $dbhelper->getPartitionWiseDBTableNameForJoin('pos_sales_return', 'ps');
        ...
        if($req->salesTypeId != 'all') $posAllSales = $posAllSales->where('billNo', $req->salesTypeId);
        if($req->startDate != null) $posAllSales = $posAllSales->where('returnDate','>=', $startDate);
        if($req->endDate != null) $posAllSales = $posAllSales->where('returnDate','<=', $endDate);

        $pos_product = $dbhelper->getPartitionWiseDBTableNameForJoin('pos_product', 'pd');

        $posSalesReports = $posAllSales
        ->Join('pos_sales_return_details','pos_sales_return_details.salesId', '=', 'ps.id')
        ->join('pos_customer', 'pos_customer.id', '=', 'ps.customerId')
        ->Join($pos_product, 'pd.id', '=','pos_sales_return_details.productId')    
        ->select('ps.*','pos_sales_return_details.*', 'pos_customer.name as customerName','pd.name as productName')
        ->orderBy('ps.returnDate','ASC')
        ->orderBy('ps.billNo','ASC')
        ->orderBy('pd.id','ASC')
        ->orderBy('ps.id','DESC')
        ->get();
        ...
        foreach($posSalesReports as $sale){
            $sale->returnDate != $returnDate ? $returnDate = $sale->returnDate : $sale->returnDate = "";
            $sale->billNo != $billNo ? $billNo = $sale->billNo : $sale->billNo = "";
            $sale->customerName != $customerName ? $customerName = $sale->customerName : $sale->customerName = "";
        }

        for($i = 0; $i < count($posSalesReports); $i++){
            $rs1 = 0;
            if($posSalesReports[$i]->returnDate){
                $rs1 = 1;
                for($j = $i+1; $j < count($posSalesReports); $j++){
                    if($posSalesReports[$j]->returnDate) break;                    
                    $rs1++;
                }
            }
            ...
            $rs2 = 0;
            if($posSalesReports[$i]->billNo){
                $rs2 = 1;
                for($j = $i+1; $j < count($posSalesReports); $j++){
                    if($posSalesReports[$j]->billNo) break;                    
                    $rs2++;
                }
            }
    ...
    public function getSellsBillNoCustomer(Request $request){
        ...
        $sales = DB::table(DB::raw("pos_sales_return PARTITION ($partitionName)"))->where('customerId', $request->customerId)->get();
