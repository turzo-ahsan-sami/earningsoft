    ...
    public function collectionCreditAutoVoucher($softwareDateFormat,$targetBranchId) {
       ...
       if($totalCollectionAmount > 0) {
           ...
           if($totalCashCollectionAmount > 0) {
                ...
                if(!empty($updateCollectionVoucherId)) {

                    DB::table('acc_voucher_details')->where('voucherId', $updateCollectionVoucherId)->where('debitAcc',415)->where('creditAcc',620)->update(array('amount' => $totalCashCollectionAmount));

                } else {

                $collectionCashDebitLedgerStr = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherTypeId',2)->where('misConfigId',7)->where('amountType',1)->value('ledgerId');
        ...
        if($totalBankCollectionAmount > 0) {
            ...
            $updateCollectionVoucherId = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('voucherTypeId',2)->where('moduleIdFk',8)->where('vGenerateType',1)->value('id');
                ...
                if(!empty($updateCollectionVoucherId)) {
                    ...
                    DB::table('acc_voucher_details')->where('voucherId', $updateCollectionVoucherId)->where('debitAcc',412)->where('creditAcc',620)->update(array('amount' => $totalBankCollectionAmount));
