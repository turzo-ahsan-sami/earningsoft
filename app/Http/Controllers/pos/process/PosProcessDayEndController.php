...
class PosProcessDayEndController extends Controller
{
 ...
 public function storeDayEnd(Request $req){
            ...
            $this->collectionCreditAutoVoucher($softwareDateFormat,$targetBranchId);
...
//==============================START AUTO BOUCHER FOR POS==============================
    ...
    if($totalSaleGrossAmount>0) {
               ...
               DB::table('acc_voucher_details')->insert([
                ...
                $voucherReferenceIdArr = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('moduleIdFk',8)->where('vGenerateType',1)->where('voucherTypeId',3)->pluck('referenceId')->toArray();

                $deleteReferanceId = DB::table('acc_auto_voucher_ref')->whereIn('id',$voucherReferenceIdArr)->where('salesType',1)->pluck('id')->toArray();

                $deleteVoucherId = DB::table('acc_voucher')->whereIn('referenceId',$deleteReferanceId)->where('moduleIdFk',8)->where('vGenerateType',1)->pluck('id')->toArray();

                DB::table('acc_voucher')->whereIn('id',$deleteVoucherId)->delete();

                DB::table('acc_voucher_details')->whereIn('voucherId',$deleteVoucherId)->delete();
...
// END SALES AND SERVICE FUNCTION==================================================================




    public function collectionCreditAutoVoucher($softwareDateFormat,$targetBranchId) {
       ...
       if($totalCollectionAmount > 0) {
            ...
            $totalCashCollectionAmount = DB::table('pos_collection')->where('collectionDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('paymentType',1)->sum('salesPayAmount');
           $totalBankCollectionAmount = DB::table('pos_collection')->where('collectionDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('paymentType',2)->sum('salesPayAmount');

           if($totalCashCollectionAmount > 0) {

                $updateCollectionVoucherId = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('voucherTypeId',2)->where('moduleIdFk',8)->where('vGenerateType',1)->value('id');
                $checkDetailsExsitingOrNot = DB::table('acc_voucher_details')->where('voucherId', $updateCollectionVoucherId)->where('debitAcc',412)->where('creditAcc',620)->value('id');
                if(count($checkDetailsExsitingOrNot)>0) {

                    DB::table('acc_voucher_details')->where('voucherId', $updateCollectionVoucherId)->where('debitAcc',412)->where('creditAcc',620)->update(array('amount' => $totalCashCollectionAmount));

                } else {

                $collectionCashDebitLedgerStr = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherType',2)->where('misConfigId',7)->where('amountType',1)->value('ledgerId');
                $collectionCashDebitLedgerStr =  str_replace(array('"', '[', ']'),'', $collectionCashDebitLedgerStr);
                $collectionDebitLedgerArr = array_map('intval', explode(',', $collectionCashDebitLedgerStr));
                if (count($collectionDebitLedgerArr)==1) {
                    $collectionDebitLedgerId=$collectionDebitLedgerArr[0];
                }
                $collectionCashCreditLedgerStr = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherType',2)->where('misConfigId',9)->where('amountType',2)->value('ledgerId');
                $collectionCashCreditLedgerStr =  str_replace(array('"', '[', ']'),'', $collectionCashCreditLedgerStr);
                $collectionCreditLedgerArr = array_map('intval', explode(',', $collectionCashCreditLedgerStr));
                if (count($collectionCreditLedgerArr)==1) {
                    $collectionCreditLedgerId=$collectionCreditLedgerArr[0];
                }

                $collectionLocalNarration = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherType',2)->where('misConfigId',7)->value('localNarration');

                $voucherId = DB::table('acc_voucher')->where('voucherCode',$voucherCode)->value('id');
                DB::table('acc_voucher_details')->insert([
                  'voucherId'      => $voucherId,
                  'debitAcc'       => $collectionDebitLedgerId,
                  'creditAcc'      => $collectionCreditLedgerId,
                  'amount'         => $totalCashCollectionAmount,
                  'localNarration' => $collectionLocalNarration,
                  'createdDate'    => Carbon::now(),
                  'status'         => 0
                 ]);
           ...
           } else if($totalCashCollectionAmount==0) {

            $voucherCollectionDeletedId = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('voucherTypeId',2)->where('moduleIdFk',8)->where('vGenerateType',1)->value('id');
             DB::table('acc_voucher_details')->where('voucherId',$voucherCollectionDeletedId)->where('debitAcc',412)->where('creditAcc',620)->delete();

           }


        if($totalBankCollectionAmount > 0) {

            $updateCollectionVoucherId = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('voucherTypeId',2)->where('moduleIdFk',8)->where('vGenerateType',1)->value('id');

            $checkDetailsExsitingOrNot = DB::table('acc_voucher_details')->where('voucherId', $updateCollectionVoucherId)->where('debitAcc',415)->where('creditAcc',620)->value('id');

                if(count($checkDetailsExsitingOrNot)>0) {
                    
                    DB::table('acc_voucher_details')->where('voucherId', $updateCollectionVoucherId)->where('debitAcc',415)->where('creditAcc',620)->update(array('amount' => $totalBankCollectionAmount));

                } else {

               $collectionBankDebitLedgerStr = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherType',2)->where('misConfigId',8)->where('amountType',1)->value('ledgerId');
                $collectionBankDebitLedgerStr =  str_replace(array('"', '[', ']'),'', $collectionBankDebitLedgerStr);
                $collectionDebitLedgerArr = array_map('intval', explode(',', $collectionBankDebitLedgerStr));
                if (count($collectionDebitLedgerArr)==1) {
                    $collectionDebitLedgerId=$collectionDebitLedgerArr[0];
                }
                $collectionBankCreditLedgerStr = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherType',2)->where('misConfigId',9)->where('amountType',2)->value('ledgerId');
                $collectionBankCreditLedgerStr =  str_replace(array('"', '[', ']'),'', $collectionBankCreditLedgerStr);
                $collectionCreditLedgerArr = array_map('intval', explode(',', $collectionBankCreditLedgerStr));
                if (count($collectionCreditLedgerArr)==1) {
                    $collectionCreditLedgerId=$collectionCreditLedgerArr[0];
                }

                $collectionLocalNarration = DB::table('acc_auto_voucher_config')->where('moduleId',8)->where('misTypeId_Fk',7)->where('voucherType',2)->where('misConfigId',8)->value('localNarration');
                ...
                DB::table('acc_voucher_details')->insert([
                  ...
                  'debitAcc'       => $collectionDebitLedgerId,
                  'creditAcc'      => $collectionCreditLedgerId,
                  'amount'         => $totalBankCollectionAmount,
                  'localNarration' => $collectionLocalNarration,
                  'createdDate'    => Carbon::now(),
                  'status'         => 0
                 ]);
           ...
           } else if($totalBankCollectionAmount==0){

             $voucherCollectionDeletedId = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('voucherTypeId',2)->where('moduleIdFk',8)->where('vGenerateType',1)->value('id');
             DB::table('acc_voucher_details')->where('voucherId',$voucherCollectionDeletedId)->where('debitAcc',415)->where('creditAcc',620)->delete();
           }

    } else if ($totalCollectionAmount==0) {

        $voucherCollectionDeletedIdArr = DB::table('acc_voucher')->where('voucherDate',$softwareDateFormat)->where('branchId',$targetBranchId)->where('voucherTypeId',2)->where('moduleIdFk',8)->where('vGenerateType',1)->pluck('id')->toArray();

        DB::table('acc_voucher')->whereIn('id',$voucherCollectionDeletedIdArr)->delete();

        DB::table('acc_voucher_details')->whereIn('voucherId',$voucherCollectionDeletedIdArr)->delete();
