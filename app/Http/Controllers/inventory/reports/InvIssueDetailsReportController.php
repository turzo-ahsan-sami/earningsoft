...
class InvIssueDetailsReportController extends Controller
{
    ...
    //Search Method
      ...
      elseif ($searchMethodSelected==2 || $searchMethodSelected==3) {
        ...
        $issueDetail = DB::table('inv_tra_issue_details');
        ...
        $filteredBranches=DB::table('inv_tra_issue')->whereIn('branchId', $filteredbranch)->pluck('id');

        $filteredProduct=DB::table('inv_product')->whereIn('groupId',$groupId)->whereIn('categoryId',$categoryId)->whereIn('subCategoryId',$subCategoryId)->whereIn('id',$productId)->pluck('id');

        $filteredUseProduct=DB::table('inv_tra_issue_details')->whereIn('issueProductId',$filteredProduct)->pluck('id');

         $filteredUseDetails = DB::table('inv_tra_issue_details')->whereIn('issueId',$filteredBranches)->pluck('id');
       
            $issueDetails = DB::table('inv_tra_issue_details as t1')
                            ->join('inv_tra_issue as t2','t1.issueId','t2.id')
                            ->whereIn('t1.id',$filteredUseDetails)
                            ->whereIn('t1.id',$filteredUseProduct)
                            ->where(function ($query) use ($startDate,$endDate){
                                      $query->where('t2.issueDate','>=',$startDate)
                                      ->where('t2.issueDate','<=',$endDate);
                                    })
                            ->orderBy('t2.issueDate','asc')
                              /*->orderBy('t1.issueBillNo','asc')*/;
          $countIssue = clone $issueDetails;
          $issueDetails = $issueDetails->get();
        ...
        if ($request->fiscalYear==null) {
            ...
            return view('inventory/reports/issueReport/viewInvIssueDetailsReport',['projects'=>$projects,'projectTypes'=>$projectTypes,'branches'=>$branches,'groups'=>$groups,'categories'=>$categories,'subCategories'=>$subCategories,'products'=>$products,'fiscalYears'=>$fiscalYears,'startDate'=>$startDate,'endDate'=>$endDate,'projectSelected'=>$projectSelected,'projectTypeSelected'=>$projectTypeSelected,'branchSelected'=>$branchSelected,'groupSelected'=>$groupSelected,'categorySelected'=>$categorySelected,'subCategorySelected'=>$subCategorySelected,'productSelected'=>$productSelected,'searchMethodSelected'=>$searchMethodSelected,'fiscalYearSelected'=>$fiscalYearSelected,'dateFromSelected'=>$dateFromSelected,'dateToSelected'=>$dateToSelected,'firstRequest'=>$firstRequest,'issueDetails'=>$issueDetails,'filteredProduct'=>$filteredProduct,'issueDetail'=>$issueDetail,'countIssue'=>$countIssue]);
        } 
