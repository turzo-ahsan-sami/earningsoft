...
class InvIssueReportController extends Controller
{
    public function index(Request $request){
     ...
     if ($request->searchBillNo==null) {
        ...
        $billNoId = DB::table('inv_tra_issue')->pluck('issueBillNo');
      }
      else{
        $billSelected = (int) json_decode($request->searchBillNo);
        array_push($billNoId, DB::table('inv_tra_issue')->where('id',$request->searchBillNo)->value('issueBillNo'));
      }
    ...
    //Search Method
      ...
      elseif ($searchMethodSelected==2 || $searchMethodSelected==3) {
        ...
        $branches = DB::table('gnr_branch')->whereIn('projectId',$projectId)->orWhere('id',1)->get();
         $billNos = DB::table('inv_tra_issue')->get();
        ...
        $filteredBranches=DB::table('inv_tra_issue')->whereIn('branchId', $filteredbranch)->pluck('id');
            ...
            $issue = DB::table('inv_tra_issue')->whereIn('id', $filteredBranches)
                            ->whereIn('issueBillNo',$billNoId)
                            ->where(function ($query) use ($startDate,$endDate){
                                      $query->where('issueDate','>=',$startDate)
        ...
        if ($request->fiscalYear==null) {
            ...
            return view('inventory/reports/issueReport/viewInvIssueReport',['projects'=>$projects,'projectTypes'=>$projectTypes,'branches'=>$branches,'fiscalYears'=>$fiscalYears,'startDate'=>$startDate,'endDate'=>$endDate,'projectSelected'=>$projectSelected,'projectTypeSelected'=>$projectTypeSelected,'branchSelected'=>$branchSelected,'searchMethodSelected'=>$searchMethodSelected,'fiscalYearSelected'=>$fiscalYearSelected,'dateFromSelected'=>$dateFromSelected,'dateToSelected'=>$dateToSelected,'firstRequest'=>$firstRequest,'issue'=>$issue,'billNos'=>$billNos,'billSelected'=>$billSelected]);
        } 
