...
class InvPurchaseController extends Controller
{
	...
	public function addItem(Request $req){
		...
		$logArray = array(
			'moduleId'  => 1,
			'controllerName'  => 'InvPurchaseController',
			'tableName'  => 'inv_purchase',
			'operation'  => 'insert',
			'primaryIds'  => [DB::table('inv_purchase')->max('id')]
		);
	...
	public function editPurchaseItem(Request $req){
		...
		if ($validator->fails())
			...
			if($idCount>0){
				...
				DB::table('inv_purchase_details')->insert($dataSet);
				$previousdata = InvPurchase::find ($req->id);
...
// Purchase Details
	public function purchaseDetails(Request $req){
		...
		foreach ($prchDetails as $prchDetail) {
			...
			$employeeDege = DB::table('hr_settings_position')->select('name')->where('id',$employeeDeg)->first();
			...
			$projectTypeName = DB::table('gnr_project_type')->select('name')->where('id',$prchDetail->projectTypeId)->first();

			$InvPurDetails =  InvPurchaseDetails::where('purchaseId',$prchDetail->id)->get();
			foreach ($InvPurDetails as $InvPurDetail) {
				$productName[] = DB::table('inv_product')->select('name')->where('id',$InvPurDetail->productId)->get();
