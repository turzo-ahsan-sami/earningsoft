...
class FamsFixedAssetsWriteOffReportController extends Controller
{
    public function index(Request $request){
      ...
      $filteredProduct = DB::table('fams_product')->whereIn('projectId',$projectId)->whereIn('projectTypeId',$projectTypeId)->whereIn('categoryId',$categoryId)->whereIn('productTypeId',$productTypeId)->pluck('id');

    $writeOffs = DB::table('fams_write_off')->whereIn('branchId',$branchId)
                            ->whereIn('productId',$filteredProduct)
    ...
    //$sales =  DB::table('fams_sale')->get();
      ...
      $prefix = DB::table('fams_product_prefix')->where('status',1)->value('name');
       ...
       return view('fams/reports/view_fixed_assets_writeOff_report',['projects'=>$projects,'projectTypes'=>$projectTypes,'branches'=>$branches,'categories'=>$categories,'productTypes'=>$productTypes,'fiscalYears'=>$fiscalYears,'startDate'=>$startDate,'endDate'=>$endDate,'projectSelected'=>$projectSelected,'projectTypeSelected'=>$projectTypeSelected,'branchSelected'=>$branchSelected,'categorySelected'=>$categorySelected,'productTypeSelected'=>$productTypeSelected,'searchMethodSelected'=>$searchMethodSelected,'fiscalYearSelected'=>$fiscalYearSelected,'dateFromSelected'=>$dateFromSelected,'dateToSelected'=>$dateToSelected,'firstRequest'=>$firstRequest,'writeOffs'=>$writeOffs,'prefix'=>$prefix]);
    } 
