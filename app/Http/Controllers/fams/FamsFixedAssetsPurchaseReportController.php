...
class FamsFixedAssetsPurchaseReportController extends Controller
{
    public function index(Request $request){
      ...
      $filteredProduct = DB::table('fams_product')->whereIn('projectId',$projectId)->whereIn('projectTypeId',$projectTypeId)->whereIn('branchId',$branchId)->whereIn('categoryId',$categoryId)->whereIn('productTypeId',$productTypeId)->pluck('id');

      //$purchases = DB::table('fams_purchase')->where('purchaseDate','>=',$startDate)->where('purchaseDate','<=',$endDate)->whereIn('productId',$filteredProduct)->whereNotIn('productId',$result)->orderBy('purchaseDate','asc')->get();
      ...
      $prefix = DB::table('fams_product_prefix')->where('status',1)->value('name');
       ...
       return view('fams/reports/view_fixed_assets_purchase_report',['projects'=>$projects,'projectTypes'=>$projectTypes,'branches'=>$branches,'categories'=>$categories,'productTypes'=>$productTypes,'fiscalYears'=>$fiscalYears,'startDate'=>$startDate,'endDate'=>$endDate,'projectSelected'=>$projectSelected,'projectTypeSelected'=>$projectTypeSelected,'branchSelected'=>$branchSelected,'categorySelected'=>$categorySelected,'productTypeSelected'=>$productTypeSelected,'searchMethodSelected'=>$searchMethodSelected,'fiscalYearSelected'=>$fiscalYearSelected,'dateFromSelected'=>$dateFromSelected,'dateToSelected'=>$dateToSelected,'firstRequest'=>$firstRequest,'prefix'=>$prefix,'filteredProduct'=>$filteredProduct,'result'=>$result,'branchId'=>$branchId]);
    } 
