...
class FamsFixedAssetsTransferReportController extends Controller
{
    ...
    public function index(Request $request){
      ...
      $writeOffProducts = DB::table('fams_write_off')->where('createdDate','<=',$endDate)->pluck('productId')->toArray();
      $soldProducts = DB::table('fams_sale')->where('createdDate','<=',$endDate)->pluck('productId')->toArray();
      $result = array_merge($writeOffProducts, $soldProducts);

      $transferedProducts = DB::table('fams_tra_transfer')->distinct()->pluck('productId');

      $productId = DB::table('fams_product')->whereIn('categoryId',$categoryId)->whereIn('productTypeId',$productTypeId)->whereNotIn('id',$result)->pluck('id');


      $transfers = DB::table('fams_tra_transfer')->where(function ($query) use ($branchId){
                                                          $query->whereIn('branchIdFrom',$branchId)
                                                          ->orWhereIn('branchIdTo',$branchId);
                                                        }) 
                                                  ->where(function ($query) use ($projectId){
                                                          $query->whereIn('projectIdFrom',$projectId)
                                                          ->orWhereIn('projectIdTo',$projectId);
                                                        })
                                                  ->where(function ($query) use ($projectTypeId){
                                                          $query->whereIn('projectTypeIdFrom',$projectTypeId)
                                                          ->orWhereIn('projectTypeIdTo',$projectTypeId);
                                                        })                                                  
                                                  ->where(function ($query) use ($startDate,$endDate){
                                                          $query->where('transferDate','>=',$startDate)
      ...
      $prefix = DB::table('fams_product_prefix')->where('status',1)->value('name');
       ...
       return view('fams/reports/view_fixed_assets_transfer_report',['projects'=>$projects,'projectTypes'=>$projectTypes,'branches'=>$branches,'categories'=>$categories,'productTypes'=>$productTypes,'fiscalYears'=>$fiscalYears,'startDate'=>$startDate,'endDate'=>$endDate,'projectSelected'=>$projectSelected,'projectTypeSelected'=>$projectTypeSelected,'branchSelected'=>$branchSelected,'categorySelected'=>$categorySelected,'productTypeSelected'=>$productTypeSelected,'searchMethodSelected'=>$searchMethodSelected,'fiscalYearSelected'=>$fiscalYearSelected,'dateFromSelected'=>$dateFromSelected,'dateToSelected'=>$dateToSelected,'firstRequest'=>$firstRequest,'transfers'=>$transfers,'prefix'=>$prefix]);
    } 
