...
class FamsTransactionAdditionalChargeController extends Controller
{
    ...
    public function storeAdditinalCharge(Request $request){
        ...
        $productPurchaseDate1 = Product::where('productCode',$request->productCode)->value('purchaseDate');        
        
        $productPurchaseDate = Carbon::parse( $productPurchaseDate1);
        $usefulLifeYears = (int) Product::where('id',$request->productId)->value('usefulLifeYear');
        $usefulLifeMonths = (int) Product::where('id',$request->productId)->value('usefulLifeMonth');
        $totalMonths =  ($usefulLifeYears*12) + $usefulLifeMonths;
        $usefulLifeExpireDate = $productPurchaseDate->copy()->addMonths($totalMonths);
        $remainingDays = Carbon::today()->diffInDays($usefulLifeExpireDate);
        ...
        for ($i=0; $i<$array_size; $i++){
            ...
            $facd->purchaseDate =  Carbon::today();
            $facd->save();
    ...
    public function editAdditinalCharge(Request $request){
        ...
        $productPurchaseDate1 = Product::where('productCode',$poductCode)->value('purchaseDate');        
        
        $productPurchaseDate = Carbon::parse( $productPurchaseDate1);
        $usefulLifeYears = (int) Product::where('productCode',$fac->productCode)->value('usefulLifeYear');
        $usefulLifeMonths = (int) Product::where('productCode',$fac->productCode)->value('usefulLifeMonth');
        $totalMonths =  ($usefulLifeYears*12) + $usefulLifeMonths;
        $usefulLifeExpireDate = $productPurchaseDate->copy()->addMonths($totalMonths);
        $remainingDays = Carbon::today()->diffInDays($usefulLifeExpireDate);
