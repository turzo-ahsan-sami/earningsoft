...
class MfnSavingsDepositController extends Controller
{
    ...
    public function storeDeposit(Request $req)
    {
        ...
        if ($validator->fails()) {
            ...
            $account = MfnSavingsAccount::where('id', $req->savingsCode)->select('id', 'branchIdFk', 'samityIdFk', 'savingsProductIdFk')->first();
    ...
    public function updateDeposit(Request $req)
    {
        ...
        if ($balace + (float) $req->amount - $deposit->amount < 0) {
            $data = array(
                'responseTitle' =>  'Warning!',
                'responseText'  =>  'Updating this deposit makes the account balance negetive. You can not ' . $operation . ' this deposit.'
            );
            return response::json($data);
        }
