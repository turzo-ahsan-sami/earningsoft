...
class MfnSavingsStatusController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($userBranchId != 1) {
            ...
            $samityList = $samityList->whereIn('branchId', $branchIdArray);
        }
    ...
    public function printReport(Request $req)
    {
        ...
        ->where('softDel', 0)->where('isTransferred', 0)
        ->where('paymentType', '!=', 'Interest')
        ->whereIn('accountIdFk', $savings->pluck('id'))
        ->get();

        $interests = DB::table('mfn_savings_deposit')
        ->where('softDel', 0)
        ...
        ->whereIn('savingsAccIdFk', $savings->pluck('id'))
        ->get();
    ...
    public function viewSavingDetails($savingsId, Request $req)
    {
        ...
        $deposits = DB::table('mfn_savings_deposit')->where('softDel', 0)->where('isTransferred', 0)->where('paymentType', '!=', 'Interest')->where('accountIdFk', $savingsId)->where('amount', '>', 0);
        $interests = DB::table('mfn_savings_deposit')->where('softDel', 0)->where('isTransferred', 0)->where('paymentType', '=', 'Interest')->where('accountIdFk', $savingsId);
        // $interests = DB::table('mfn_savings_interest')->where('accIdFk',$savingsId)->where('interestAmount','>',0);
        $withdraws = DB::table('mfn_savings_withdraw')->where('softDel', 0)->where('isTransferred', 0)->where('accountIdFk', $savingsId)->where('amount', '>', 0);

        $openingBalance = DB::table('mfn_opening_savings_account_info')
        ->where('savingsAccIdFk', $savingsId)
        ->first();
        ...
        if ($openingBalance != null) {
            $openingPrincipal = $openingBalance->openingPrincipal;
            $openingInterest = $openingBalance->openingInterest;
            $openingWithdraw = $openingBalance->openingWithdraw;
        }
        ...
        if ($req->dateFrom != null) {
            ...
            $interests = $interests->where('depositDate', '>=', $dateFrom);
            // $interests = $interests->where('date','>=',$dateFrom);
            $withdraws = $withdraws->where('withdrawDate', '>=', $dateFrom);

            $openingPrincipal += DB::table('mfn_savings_deposit')->where('softDel', 0)->where('isTransferred', 0)->where('paymentType', '!=', 'Interest')->where('accountIdFk', $savingsId)->where('amount', '>', 0)->where('depositDate', '<', $dateFrom)->sum('amount');
            $openingInterest += DB::table('mfn_savings_deposit')->where('softDel', 0)->where('isTransferred', 0)->where('paymentType', '=', 'Interest')->where('accountIdFk', $savingsId)->where('amount', '>', 0)->where('depositDate', '<', $dateFrom)->sum('amount');
            /* $openingInterest += DB::table('mfn_savings_interest')->where('accIdFk',$savingsId)->where('interestAmount','>',0)->where('date','<',$dateFrom)->sum('interestAmount');*/
            $openingWithdraw += DB::table('mfn_savings_withdraw')->where('softDel', 0)->where('isTransferred', 0)->where('accountIdFk', $savingsId)->where('amount', '>', 0)->where('withdrawDate', '<', $dateFrom)->sum('amount');
        }
        if ($req->dateTo != null) {
            $dateTo = Carbon::parse($req->dateTo)->format('Y-m-d');
            $deposits = $deposits->where('depositDate', '<=', $dateTo);
            $interests = $interests->where('depositDate', '<=', $dateTo);
        ...
        $dates = array_merge($dates, $interests->pluck('depositDate')->toArray());
        ...
        $data = array(
            ...
            'openingWithdraw'   => $openingWithdraw
