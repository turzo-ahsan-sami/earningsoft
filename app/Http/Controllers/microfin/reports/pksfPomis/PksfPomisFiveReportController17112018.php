...
class PksfPomisFiveReportController extends Controller {
    ...
    public function getReport(Request $req){
        ...
        $pksfLoanOpeningBalances = DB::table('mfn_opening_balance_loan as t1')
                                    ->join('mfn_loan as t2','t1.loanIdFk', 't2.id')
                                    ->whereIn('loanIdFk',$pksfLoanDisbursements->pluck('id'))
                                    ->select('t2.productIdFk','t2.memberIdFk','t1.paidLoanAmountOB','t1.principalAmountOB')
                                    ->get();
        ...
        foreach ($pksfLoanOpeningBalances as $pksfLoanOpeningBalance) {
            $pksfLoanCollections->push([
                'productIdFk'       => $pksfLoanOpeningBalance->productIdFk,
                'memberIdFk'        => $pksfLoanOpeningBalance->memberIdFk,
        ...
        $nonPksfLoanOpeningBalances = DB::table('mfn_opening_balance_loan as t1')
                                    ->join('mfn_loan as t2','t1.loanIdFk', 't2.id')
                                    ->whereIn('loanIdFk',$nonPksfLoanDisbursements->pluck('id'))
                                    ->select('t2.productIdFk','t2.memberIdFk','t1.paidLoanAmountOB','t1.principalAmountOB')
                                    ->get();
        ...
        foreach ($nonPksfLoanOpeningBalances as $nonPksfLoanOpeningBalance) {
            $nonPksfLoanCollections->push([
                'productIdFk'       => $nonPksfLoanOpeningBalance->productIdFk,
                'memberIdFk'        => $nonPksfLoanOpeningBalance->memberIdFk,
