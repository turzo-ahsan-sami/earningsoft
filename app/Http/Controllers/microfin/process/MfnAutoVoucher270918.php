<?php
    ...
    class MfnAutoVoucher {
        ...
        public function createCreditVoucher(){
            ...
            $memberAdmissionFee = DB::table('mfn_member_information')->where('softDel',0)->where('status',1)->where('branchId',$this->branchId)->where('admissionDate',$this->softwareDate)->select('primaryProductId','admissionFee')->get();

            $savingsDeposits = DB::table('mfn_savings_deposit')->where('softDel',0)->where('branchIdFk',$this->branchId)->where('depositDate',$this->softwareDate)->where('isTransferred',0)->select('primaryProductIdFk','productIdFk','amount','ledgerIdFk')->get();

            $loansFees = DB::table('mfn_loan')->where('softDel',0)->where('status',1)->where('branchIdFk',$this->branchId)->where('disbursementDate',$this->softwareDate)->select('productIdFk','additionalFee','loanFormFee','insuranceAmount','ledgerId')->get();

            $loanCollections = DB::table('mfn_loan_collection')->where('softDel',0)->where('branchIdFk',$this->branchId)->where('collectionDate',$this->softwareDate)->select('productIdFk','principalAmount','interestAmount','ledgerIdFk')->get();            
        ...
        public function createDebitVoucher(){
            ...
            $savingsWithdraws = DB::table('mfn_savings_withdraw')->where('softDel',0)->where('branchIdFk',$this->branchId)->where('withdrawDate',$this->softwareDate)->where('isTransferred',0)->select('primaryProductIdFk','productIdFk','amount','ledgerIdFk')->get();

            $loanDisbursements = DB::table('mfn_loan')->where('softDel',0)->where('status',1)->where('branchIdFk',$this->branchId)->where('disbursementDate',$this->softwareDate)->select('productIdFk','loanAmount','ledgerId')->get();
        ...
        public function createJournalVoucher(){
            ...
            $waivers = DB::table('mfn_loan_waivers')->where('branchIdFk',$this->branchId)->where('date',$this->softwareDate)->select('loanIdFk','principalAmount','interestAmount')->get();

            $writeOffs = DB::table('mfn_loan_write_off')->where('branchIdFk',$this->branchId)->where('date',$this->softwareDate)->select('loanIdFk','principalAmount','interestAmount')->get();

            $loanIds = array_merge($waivers->pluck('loanIdFk')->toArray(),$writeOffs->pluck('loanIdFk')->toArray());

            $loans = DB::table('mfn_loan')->whereIn('id',$loanIds)->select('id','productIdFk')->get();

            $voucherId = $this->storeOrUpdateVoucher(3);

            $hasTraExits = max(count($productTransfers),count($waivers),count($writeOffs));
