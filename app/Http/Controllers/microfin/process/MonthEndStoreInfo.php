...
class MonthEndStoreInfo
{
	...
	public function storeLoanInfo()
	{
		...
		$loanData = $datas['loanData'];

		foreach ($loanData as $data) {
			// if ($this->isCustomCall == 1) {
			// 	$mEloanInfo = MfnMonthEndProcessLoan::where('date', Carbon::parse($this->monthEndDate))->where('branchIdFk', $this->targetBranchId)->where('productIdFk', $data->productIdFk)->where('genderTypeId', $data->genderTypeId)->first();

			// 	$branchSoftwareStartDate = DB::table('gnr_branch')->where('id', $this->targetBranchId)->first()->softwareStartDate;
			...
			// 		if ($this->monthEndDate > $branchSoftwareStartDate) {
			...
			// 		echo '$this->targetBranchId: '.$this->targetBranchId.'<br>';
            //         echo '$data->productIdFk: '.$data->productIdFk.'<br>';
            //         echo '$data->genderTypeId: '.$data->genderTypeId.'<br>';
            //         echo '$mEloanInfo->openingBorrowerNo: '.$mEloanInfo->openingBorrowerNo.'<br>';
            //         echo '$mEloanInfo->borrowerNo: '.$mEloanInfo->borrowerNo.'<br>';
            //         echo '$mEloanInfo->fullyPaidBorrowerNo: '.$mEloanInfo->fullyPaidBorrowerNo.'<br>';
            //         echo '$mEloanInfo->closingBorrowerNo: '.$mEloanInfo->closingBorrowerNo.'<br>';
            //         echo '$mEloanInfo->cumLoanNo: '.$mEloanInfo->cumLoanNo.'<br>';
            //         echo '$mEloanInfo->noOfUniqueLoanee: '.$mEloanInfo->noOfUniqueLoanee.'<br><br>';
			// 	}
		...
		// STORE DATA FOR FUNDING ORGANIZATION
		$funOrgDatas = $datas['funOrgData'];

		foreach ($funOrgDatas as  $funOrgData) {

			$mEloanInfo = MfnMonthEndProcessLoan::where('date', Carbon::parse($this->monthEndDate))->where('branchIdFk', $this->targetBranchId)->where('fundingOrgIdFk', $funOrgData->fundingOrgIdFk)->where('genderTypeId', $funOrgData->genderTypeId)->first();
			...
			$mEloanInfo->date                   = Carbon::parse($this->monthEndDate);
			$mEloanInfo->branchIdFk             = $this->targetBranchId;
			$mEloanInfo->fundingOrgIdFk         = $funOrgData->fundingOrgIdFk;
			$mEloanInfo->genderTypeId           = $funOrgData->genderTypeId;
			$mEloanInfo->openingBorrowerNo      = $funOrgData->openingBorrowerNo;
			$mEloanInfo->borrowerNo             = $funOrgData->borrowerNo;
			$mEloanInfo->fullyPaidBorrowerNo    = $funOrgData->fullyPaidBorrowerNo;
			$mEloanInfo->closingBorrowerNo      = $funOrgData->closingBorrowerNo;
			$mEloanInfo->save();
		}
		// END STORE DATA FOR FUNDING ORGANIZATION

		// STORE DATA FOR FUNDING ORGANIZATION
		$totalDatas = $datas['totalData'];

		foreach ($totalDatas as  $totalData) {

			$mEloanInfo = MfnMonthEndProcessLoan::where('date', Carbon::parse($this->monthEndDate))->where('branchIdFk', $this->targetBranchId)->where('fundingOrgIdFk', 0)->where('productIdFk', 0)->where('genderTypeId', $totalData->genderTypeId)->first();
			...
			$mEloanInfo->date                   = Carbon::parse($this->monthEndDate);
			$mEloanInfo->branchIdFk             = $this->targetBranchId;
			$mEloanInfo->genderTypeId           = $totalData->genderTypeId;
			$mEloanInfo->openingBorrowerNo      = $totalData->openingBorrowerNo;
			$mEloanInfo->borrowerNo             = $totalData->borrowerNo;
			$mEloanInfo->fullyPaidBorrowerNo    = $totalData->fullyPaidBorrowerNo;
			$mEloanInfo->closingBorrowerNo      = $totalData->closingBorrowerNo;
			$mEloanInfo->save();
		}
		// END STORE DATA FOR FUNDING ORGANIZATION
		...
		//     $mEloanInfo->date                   = Carbon::parse($this->monthEndDate);
		//     $mEloanInfo->branchIdFk             = $this->targetBranchId;
		//     $mEloanInfo->fundingOrgIdFk         = $funOrgData->$funOrgId;
		//     $mEloanInfo->genderTypeId           = $funOrgData->genderTypeId;
		//     $mEloanInfo->openingBorrowerNo      = $funOrgData->openingBorrowerNo;
		//     $mEloanInfo->borrowerNo             = $funOrgData->borrowerNo;
		//     $mEloanInfo->fullyPaidBorrowerNo    = $funOrgData->fullyPaidBorrowerNo;
	...
	public function getLoanDetails()
	{
		...
		$loans = DB::table('mfn_loan AS loan')
			...
			->join('mfn_loans_product AS lp', 'lp.id', 'loan.productIdFk')
			->where([['loan.softDel', 0], ['loan.branchIdFk', $this->targetBranchId], ['loan.disbursementDate', '<=', $monthEndDate]])
			->where(function ($query) use ($monthFirstDate) {
				$query->where('loan.loanCompletedDate', '0000-00-00')
					->orWhere('loan.loanCompletedDate', '>=', $monthFirstDate);
			})
			->select('mi.gender', 'mi.id AS memberId', 'loan.id', 'loan.loanCode', 'loan.productIdFk', 'loan.primaryProductIdFk', 'lp.fundingOrganizationId', 'loan.loanAmount', 'loan.totalRepayAmount', 'loan.disbursementDate', 'loan.loanCompletedDate', 'loan.loanTypeId', 'loan.loanRepayPeriodIdFk', 'loan.loanCycle')
			->get();
		...
		foreach ($loans as $loan) {
			...
			$openingCollectionAmount += DB::table('mfn_loan_waivers')->where('softDel', 0)->where('loanIdFk', $loan->id)->where('date', '<', $monthFirstDate)->sum('amount');
			...
			$collectionAmount += DB::table('mfn_loan_waivers')->where('softDel', 0)->where('loanIdFk', $loan->id)->where('date', '>=', $monthFirstDate)->where('date', '<=', $monthEndDate)->sum('amount');
			...
			$openingCollectionAmount += DB::table('mfn_loan_waivers')->where('softDel', 0)->where('loanIdFk', $loan->id)->where('date', '<', $monthFirstDate)->sum('principalAmount');
			...
			$collectionAmount += DB::table('mfn_loan_waivers')->where('softDel', 0)->where('loanIdFk', $loan->id)->where('date', '>=', $monthFirstDate)->where('date', '<=', $monthEndDate)->sum('principalAmount');
			...
			$loanData->principalRecoverableAmount = $recoverableAmount;
			...
			// IF THE CYCLE IS ONE THEN IT IS UNIQUE LOANEE
			if ($loan->loanCycle == 1 && $loan->disbursementDate >= $monthFirstDate) {
				$loanData->noOfUniqueLoanee = 1;
			} else {
				$loanData->noOfUniqueLoanee = 0;
			}
		...
		$openingInfos = DB::table('mfn_month_end_process_loans')
			->where('branchIdFk', $this->targetBranchId)
			->where('date', $lastMonthEndDate)
			->select('productIdFk', 'fundingOrgIdFk', 'genderTypeId', 'closingBorrowerNo', 'closingOutstandingAmount', 'closingOutstandingAmountWithServicesCharge', 'closingDisbursedAmount', 'closingDueAmount', 'openingDueAmountWithServicesCharge', 'cumLoanNo', 'cumBorrowerNo')
			->get();
		...
		$loanProducts = DB::table('mfn_loans_product')
			->whereIn('id', $loanProductIds)
			->select('id', 'name', 'isPrimaryProduct', 'fundingOrganizationId')
			->get();

		foreach ($loanProducts as $loanProduct) {
			foreach ($genderIds as $genderId) {

				$openingInfo = $openingInfos->where('productIdFk', $loanProduct->id)->where('genderTypeId', $genderId);
				$currentData = $data->where('productIdFk', $loanProduct->id)->where('genderTypeId', $genderId);

				// IF ALL INFORMATION IS EMPTY OR ZERO SKIP THIS, FOR THE SIMPLICITY WE ARE TAKING ONLY OPENING INFORMATION.
				if ($openingInfo->sum('closingBorrowerNo') + $openingInfo->sum('closingOutstandingAmount') + $openingInfo->sum('closingOutstandingAmountWithServicesCharge') + $openingInfo->sum('closingDueAmount') + $currentData->sum('closingOutstandingAmount') + $currentData->sum('closingDueAmount') + $openingInfo->sum('cumLoanNo') + $currentData->where('disbursementDate', '>=', $monthFirstDate)->count() == 0) {
					continue;
				}

				$productData = collect();

				$fullyPaidBorrowerNo = $currentData->where('loanCompletedDate', '>=', $monthFirstDate)->where('loanCompletedDate', '<=', $monthEndDate)->unique('memberId')->count('memberId');

				if (count($openingInfo) == 0) {
					$openingBorrowerNo = DB::table('mfn_loan AS loan')
						->join('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
						->where([['loan.softDel', 0], ['loan.branchIdFk', $this->targetBranchId], ['loan.disbursementDate', '<', $monthFirstDate]])
						->where(function ($query) use ($monthFirstDate) {
							$query->where('loan.loanCompletedDate', '0000-00-00')
								->orWhere('loan.loanCompletedDate', '>=', $monthFirstDate);
						})
						->where('loan.productIdFk', $loanProduct->id)
						->where('mi.gender', $genderId)
						->select(DB::raw('COUNT(DISTINCT loan.memberIdFk) AS numOfMember'))
						->get();
					$openingBorrowerNo = $openingBorrowerNo->sum('numOfMember');
				} else {
					$openingBorrowerNo = $openingInfo->sum('closingBorrowerNo');
				}

				$productData->productIdFk               = $loanProduct->id;
				$productData->productIdName             = $loanProduct->name;
				$productData->genderTypeId              = $genderId;
				$productData->openingBorrowerNo         = $openingBorrowerNo;
				...
				// IF IT IS OPTIONAL PRODUCT AND SAME MEMBER HAVE PRIMARY PRODUCT LOAN, THEN THIS MEMBER WILL NOT BE COUNTED.
				$productData->noOfUniqueLoanee          = $currentData->sum('noOfUniqueLoanee');
				$productData->loanRebateAmount          = $currentData->sum('loanRebateAmount');

				$productData->cumBorrowerNo             = $openingInfo->sum('cumBorrowerNo') + $productData->noOfUniqueLoanee;
		...
		// STORE INFORMATION FUNDING ORGANIZATION WISE AND GRAND TOTALLY.
		$funOrgIds = $loanProducts->unique('fundingOrganizationId')->pluck('fundingOrganizationId')->toArray();

		// FOR FUNDING ORGANIZATION
		$summaryDataFunOrg = collect();

		foreach ($funOrgIds as $funOrgId) {
			foreach ($genderIds as $genderId) {

				$funOrgData = collect();

				if (count($openingInfos) == 0) {
					$openingBorrowerNo = DB::table('mfn_loan AS loan')
						->join('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
						->join('mfn_loans_product AS lp', 'lp.id', 'loan.productIdFk')
						->where([['loan.softDel', 0], ['loan.branchIdFk', $this->targetBranchId], ['loan.disbursementDate', '<', $monthFirstDate]])
						...
						->where('lp.fundingOrganizationId', $funOrgId)
						->where('mi.gender', $genderId)
						->select(DB::raw('COUNT(DISTINCT loan.memberIdFk) AS numOfMember'))
						->get();
					$openingBorrowerNo = $openingBorrowerNo->sum('numOfMember');
				} else {
					$openingBorrowerNo = $openingInfos
						->where('fundingOrgIdFk', $funOrgId)
						->where('genderTypeId', $genderId)
				...
				$borrowerNo = $currentLoans
					->where('disbursementDate', '>=', $monthFirstDate)
					->unique('memberId')
					->count();

				$fullyPaidBorrowerNo = $currentLoans->where('loanCompletedDate', '>=', $monthFirstDate)->where('loanCompletedDate', '<=', $monthEndDate)->unique('memberId')->count('memberId');

				// $closingBorrowerNo = $openingBorrowerNo + $borrowerNo - $fullyPaidBorrowerNo;
				$closingBorrowerNo = count(array_unique(array_merge($currentLoans->where('loanCompletedDate', '0000-00-00')->unique('memberId')->pluck('memberId')->toArray(), $currentLoans->where('loanCompletedDate', '>', $monthEndDate)->unique('memberId')->pluck('memberId')->toArray())));

				$funOrgData->fundingOrgIdFk         = $funOrgId;
				$funOrgData->genderTypeId           = $genderId;
				$funOrgData->openingBorrowerNo      = $openingBorrowerNo;
				$funOrgData->borrowerNo             = $borrowerNo;
				$funOrgData->fullyPaidBorrowerNo    = $fullyPaidBorrowerNo;
				$funOrgData->closingBorrowerNo      = $closingBorrowerNo;
				$summaryDataFunOrg->push($funOrgData);
			}
		}
		// END FOR FUNDING ORGANIZATION

		// FOR GRAND TOTAL
		$summaryDataTotal = collect();
		...
		foreach ($genderIds as $genderId) {
			...
			$totalData->genderTypeId           = $genderId;
			$totalData->openingBorrowerNo      = $openingBorrowerNo;
			$totalData->borrowerNo             = $borrowerNo;
			$totalData->fullyPaidBorrowerNo    = $fullyPaidBorrowerNo;
			$totalData->closingBorrowerNo      = $closingBorrowerNo;
			$summaryDataTotal->push($totalData);
		}
		// END FOR GRAND TOTAL
		...
		$data = array(
			'loanData'      => $summaryData,
			'funOrgData'    => $summaryDataFunOrg,
			'totalData'     => $summaryDataTotal
		);
	...
	public function storeStaffInfo()
	{
		...
		$fieldOfficerIds = $samities->pluck('fieldOfficerId')->toArray();
