...
class DayEndProcessController extends Controller {
    ...
    public function storeDayEnd(Request $req){
        ...
        try{
            ...
            $softwareDateFormat = MicroFin::getSoftwareDateBranchWise($targetBranchId);
            $softwareDate       = Carbon::parse($softwareDateFormat);
            ...
            ->select(DB::raw("CONCAT(code, ' - ', name) AS nameWithCode"), 'id','samityDayId','fixedDate')
            ->get();

            $samityDayChanges = DB::table('mfn_samity_day_change')
            ->whereIn('samityId',$samities->pluck('id'))
            ->where('effectiveDate','>',$softwareDateFormat)
            ...
            $todaySamitys = $samities->filter(function($obj) use ($weekDayNumber,$dayNumber){
            ...
            ->select('id','samityIdFk','memberIdFk','depositTypeIdFk','savingsProductIdFk','branchIdFk')
            ->get();
            ...
            ->select('id','loanTypeId','loanAmount','totalRepayAmount','lastInstallmentDate','interestCalculationMethodId','samityIdFk','memberIdFk','additionalFee','productIdFk','loanFormFee','insuranceAmount','ledgerId','disbursementDate','primaryProductIdFk','isLoanCompleted')
            ->get();
            ...
            ->select('id','accountIdFk','productIdFk','primaryProductIdFk','isAuthorized','depositDate','isTransferred','ledgerIdFk','amount','paymentType')
            ->get();
            ...
            if (count($missingTransactions)>0) {
                ...
                if ($missingTransactions['name']=='savings') {
                    ...
                    $missingSamityIds = $this->dbLoan->where('isLoanCompleted',0)->whereIn('id',$missingTransactions['ids'])->pluck('samityIdFk')->toArray();
            ...
            $logArray = array(
                'moduleId'  => 6,
                'controllerName'  => 'DayEndProcessController',
                'tableName'  => 'mfn_day_end',
                'operation'  => 'insert',
                'primaryIds'  => [DB::table('mfn_day_end')->max('id')]
            );
    ...
    public function storeDayEndFirstTime($targetBranchId){
        ...
        $openingSavingsAccIds = DB::table('mfn_savings_account')
        ->where('softDel',0)
        ->where('isFromOpening',1)
        ->where('branchIdFk',$targetBranchId)
        ->pluck('id')
        ...
        $numOfOpeningAccountsBalance = DB::table('mfn_opening_savings_account_info')
        ->whereIn('savingsAccIdFk',$openingSavingsAccIds)
        ->count();

        if (count($openingSavingsAccIds)>$numOfOpeningAccountsBalance) {
            $data = array(
                'responseTitle' =>  'Warning!',
    ...
    public function checkTodayMissingTransactions($softDate){
        ...
        $todaySamityLoanAccIds = $this->dbLoan->where('loanTypeId',1)->where('isLoanCompleted',0)->whereIn('samityIdFk',$this->todaySamityIds)->whereIn('id',$this->loanIdsHavingSheduleToday)->pluck('id')->toArray();
