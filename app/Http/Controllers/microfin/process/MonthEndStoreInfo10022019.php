<?php
    ...
    class MonthEndStoreInfo {
        ...
        function __construct($targetBranchId,$monthFirstDate,$monthEndDate){
            ...
            $autoProcessInfos = DB::table('mfn_auto_process_info')
                                    ->where('branchIdFk',$this->targetBranchId)
                                    ->where('date','>=',$this->monthFirstDate)
                                    ->where('date','<=',$this->monthEndDate)
                                    ->get();

            $this->memberAttendenceInfo = collect([]);

            foreach ($autoProcessInfos as $autoProcessInfo) {
                $memberPresentsInfo = json_decode($autoProcessInfo->memberAttendence);
                    foreach ($memberPresentsInfo as $key => $memberPresents) {
                        $this->memberAttendenceInfo->push([
                            'memberId' => $memberPresents[0],
                            'isPresent' => $memberPresents[1]
                        ]);
                    }
            }
            ...
            foreach ($loanProducts as $loanProduct) {
                ...
                foreach ($genderTypeIds as $genderTypeId) {
                  ...
                  /*  $newSamityNo = DB::table('mfn_samity')
                    ...
                    $numOfWeek = $depositsFromAutoProcess->unique('depositDate')->count();
                    ...
                    /// calculate avg. member attendence
                     if ($this->memberAttendenceInfo->whereIn('memberId',$thiProductMembers->pluck('id'))->where('isPresent',1)->count()>0) {
                         $avgAttendance = $this->memberAttendenceInfo->whereIn('memberId',$thiProductMembers->pluck('id'))->where('isPresent',1)->count() / $numOfWeek;
                     }
                     else{
                        $avgAttendance = 0;
                     }
                    ...
                    /// end calculating avg. member attendence
                    ...
                    $data = array(
                        '$this->targetBranchId'  => $this->targetBranchId,
                        '$loanProduct->id'  => $loanProduct->id,
                        '$genderTypeId'  => $genderTypeId,
                        '$avgSavingsDepositor'  => $avgSavingsDepositor,
                        '$nummberOfdepositors'  => $nummberOfdepositors,
                        '$numOfWeek'  => $numOfWeek,
                        '$avgAttendance'  => $avgAttendance,
                        // '$thiProductMembers'  => $thiProductMembers,
                    );
        ...
        public function storeLoanInfo(){
            ...
            $loanProductIds = $thisBranchLoans->pluck('productIdFk')->toArray();
            ...
            $allCollections = DB::table('mfn_loan_collection')
                                    ...
                                    ->whereIn('loanIdFk',$thisBranchLoans->pluck('id'))
                                    ->where('collectionDate','<=',$this->monthEndDate)
                                    ->select('loanIdFk','memberIdFk','productIdFk','amount','principalAmount','collectionDate')
                                    ->get();
