...
class MonthEndStoreInfo {
    ...
    public function storeTotalMemberInfo(){
        ...
        $temp = DB::table('mfn_loan AS loan')
        ->rightJoin('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
        ->where([ ['mi.branchId',$branchId], ['mi.softDel',0] , ['mi.admissionDate','<=',$date] ])
        ->where(function($query) use ($branchId,$date){
            $query->where([ ['loan.softDel',0], ['loan.disbursementDate','<=',$date], ['loan.loanCompletedDate','>',$date] ])
            ->orWhere([ ['loan.softDel',0], ['loan.disbursementDate','<=',$date], ['loan.loanCompletedDate','=','0000-00-00'] ])
            ->orWhere('loan.id',null);
        })
        ->groupBy('mi.gender')
        ->groupBy('loan.primaryProductIdFk')
        ->groupBy('loan.productIdFk')
        ->select(DB::raw('mi.gender, loan.primaryProductIdFk,loan.productIdFk,COUNT(DISTINCT mi.id) AS numOfMember'))
        ->get();

        $tempTwo = $temp->filter(function ($obj){
            return ($obj->primaryProductIdFk != $obj->productIdFk && $obj->productIdFk!=null);
        });
        ...
        /// this function stores data to "mfn_month_end_process_total_members" table
    public function storeTotalMemberInfoOld(){
    ...
    public function isMemberActive($memberId){
        ...
        public function storeMemberInfo(){
            ...
            ->whereIn('id',$loanProductIds)
            ->select('id','fundingOrganizationId','isPrimaryProduct')
            ->get();
            ...
            foreach ($loanProducts as $loanProduct) {
                ...
                foreach ($genderTypeIds as $genderTypeId) {
                    ...
                    if ($loanProduct->isPrimaryProduct==1) {

                        $redundantMemberNo = DB::table('mfn_loan AS loan')
                        ->join('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
                        ->where('loan.softDel',0)
                        ->where('mi.softDel',0)
                        ->where('mi.gender',$genderTypeId)
                        ->where('loan.branchIdFk',$this->targetBranchId)
                        ->where('loan.disbursementDate','<=',$this->monthEndDate)
                        ->where('loan.primaryProductIdFk',$loanProductId)
                        ->where('loan.productIdFk','!=',$loanProductId)
                        ->where(function($query) use($monthEndDate){
                            $query->where('loan.loanCompletedDate','0000-00-00')
                            ->orWhere('loan.loanCompletedDate','>',$monthEndDate);
                        })
                        ->count(DB::raw('DISTINCT loan.memberIdFk'));

                        $closingMemberForOptionalProduct = 0;
                    }
                    else{

                        $redundantMemberNo = 0;

                        $closingMemberForOptionalProduct = DB::table('mfn_loan AS loan')
                        ->join('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
                        ->where('loan.softDel',0)
                        ->where('mi.softDel',0)
                        ->where('mi.gender',$genderTypeId)
                    ...
                    if (max($openingMember,$openingSamityNo,$newSamityNo,$newMemberAdmissionNo,$newMemberAdmissionNoBpt,$memberCancellationNo,$memberCancellationNoBpt,$avgSavingsDepositor,$inactiveSamityNo,$redundantMemberNo,$closingMemberForOptionalProduct)==0 && min($openingMember,$openingSamityNo,$newSamityNo,$newMemberAdmissionNo,$newMemberAdmissionNoBpt,$memberCancellationNo,$memberCancellationNoBpt,$avgSavingsDepositor,$inactiveSamityNo,$redundantMemberNo,$closingMemberForOptionalProduct)==0) {
                        $isEmptyInfo = 1;
                    }              
                    ...
                    if ($loanProduct->id==7) {
                        echo '$closingMember: '.$closingMember.'<br>';
                        echo '$openingMember: '.$openingMember.'<br>';
                        echo '$newMemberAdmissionNo: '.$newMemberAdmissionNo.'<br>';
                        echo '$memberCancellationNo: '.$memberCancellationNo.'<br>';
                    ...
                    $monEndMember->redundantMemberNo                = $redundantMemberNo;
                    $monEndMember->closingMemberForOptionalProduct  = $closingMemberForOptionalProduct;
        ...
        public function storeLoanInfo(){
            ...
            $loanProductIds = $thisBranchLoans->unique('productIdFk')->pluck('productIdFk')->toArray();
            ...
            $openingBalanceLoans = DB::table('mfn_opening_balance_loan')
            ->where('softDel',0)
            ->whereIn('loanIdFk',$thisBranchLoans->pluck('id'))
            ->select('loanIdFk','paidLoanAmountOB','principalAmountOB','date','lastTransactionDate')
            ->get();

            foreach ($openingBalanceLoans as $key => $openingBalanceLoan) {
                $productIdFk = $thisBranchLoans->where('id',$openingBalanceLoan->loanIdFk)->first()->productIdFk;
                $memberIdFk = $thisBranchLoans->where('id',$openingBalanceLoan->loanIdFk)->first()->memberIdFk;

                $allCollections->push([
                    'loanIdFk'  => $openingBalanceLoan->loanIdFk,
                    'amount'  => $openingBalanceLoan->paidLoanAmountOB,
                    'principalAmount'  => $openingBalanceLoan->principalAmountOB,
                    'collectionDate'  => $openingBalanceLoan->lastTransactionDate,
            ...
            /*if(in_array(202916,$thisBranchActiveLoans->pluck('id')->toArray())){
                echo 'in $thisBranchActiveLoans <br>';
            } */  
            ...
            foreach ($thisBranchActiveLoans as $thisBranchLoan) {
                ...
                $paidAmount += $loanWriteOffs->where('loanIdFk',$thisBranchLoan->id)->sum('amount');
                $paidAmount += $loanWaivers->where('loanIdFk',$thisBranchLoan->id)->sum('principalAmount');
                $paidAmount += $loanRebates->where('loanIdFk',$thisBranchLoan->id)->sum('amount');
                
                /*if($thisBranchLoan->id==202916){
                        echo '$payableAmount: '.$payableAmount.'<br>';
                        echo '$paidAmount: '.$paidAmount.'<br>';
                        
                    }*/
                    
                    $thisLoanSchedules = $shedules->where('loanIdFk',$thisBranchLoan->id)->sortBy('scheduleDate');
                    $curPaidAmount = $paidAmount;
                    foreach ($thisLoanSchedules as $thisLoanSchedule) {
                        $curPaidAmount = $curPaidAmount - $thisLoanSchedule->installmentAmount;
                        if ($curPaidAmount<0) {
                            $lastPaidDate = $thisLoanSchedule->scheduleDate;
                        }
                    }
                    ...
                    $lastPaidDate = Carbon::parse($lastPaidDate);
                    $loanYear = (int) $repayments->where('id',$thisBranchLoan->loanRepayPeriodIdFk)->first()->inMonths / 12;
                    $daysDifference = $monthEndDate->diffInDays($lastPaidDate);
                    $daysDifference = (int) $daysDifference/$loanYear;
                    
                    if($thisBranchLoan->id==172525){
                        echo '$loanYear: '.$loanYear.'<br>';
                        echo '$lastCollectioDate: '.$lastPaidDate->format('Y-m-d').'<br>';
                        echo '$daysDifference: '.$daysDifference.'<br>';
                    }
                    ...
                    elseif ($daysDifference>180 && $daysDifference<=365) {
                        ...
                        array_push($badLoanIds, $thisBranchLoan->id);
                    }
                }
            }
            
            /*if(in_array(169860,$thisBranchActiveLoans->pluck('id')->toArray())){
                print_r('active');
            }*/
            ...
            $overDueLoanIds = array_merge($badLoanIds,$watchfulLoanIds,$substandarLoanIds,$doubtfullLoanIds);
            ...
            $savingsDepositsOfExpiredLoanMembers = $this->dbDeposit
            ->whereIn('memberIdFk',$overDueLoans->pluck('memberIdFk'));

            

            $savingsWIthdrawsOfExpiredLoanMembers = $this->dbWithdraw
            ->whereIn('memberIdFk',$overDueLoans->pluck('memberIdFk'));
            ...
            foreach ($loanProductIds as $loanProductId) {
                foreach ($genderTypeIds as $genderTypeId) {
                    ...
                    /////////////////////////
                    $openingCumBorrowerNo = DB::table('mfn_month_end_process_loans')
                    ->where('date',$lastMonthLastDate)
                    ->where('branchIdFk',$this->targetBranchId)
                    ->where('productIdFk',$loanProductId)
                    ->where('genderTypeId',$genderTypeId)
                    ...
                    ->where('mfn_loan.branchIdFk',$this->targetBranchId)
                    ->where('mfn_loan.productIdFk',$loanProductId)
                    ->where('mfn_member_information.gender',$genderTypeId)
                    ->where('mfn_loan.isFromOpening',0)
                    ->where('mfn_loan.disbursementDate','>=',$this->monthFirstDate)
                    ->where('mfn_loan.disbursementDate','<=',$this->monthEndDate)
                    ...
                    $openingCumLoanNo = DB::table('mfn_month_end_process_loans')
                    ->where('date',$lastMonthLastDate)
                    ->where('branchIdFk',$this->targetBranchId)
                    ->where('productIdFk',$loanProductId)
                    ->where('genderTypeId',$genderTypeId)
                   ...
                   ///// new code to fetch regular, due, advance collection
                    ...
                    foreach ($currentLoanIds as $loanId) {
                        ...
                        $pWaiverAmountPrincipal = $loanWaivers->where('date','<',$this->monthFirstDate)->where('loanIdFk',$loanId)->sum('principalAmount');
                        ...
                        $cWriteOffAmount = $loanWriteOffs->where('loanIdFk',$loanId)->sum('amount');
                        $cWriteOffAmountPrincipal = $loanWriteOffs->where('loanIdFk',$loanId)->sum('principalAmount');
                        $cWaiverAmountPrincipal = $loanWaivers->where('loanIdFk',$loanId)->sum('principalAmount');
                        $cRebateAmount = $loanRebates->where('loanIdFk',$loanId)->sum('amount');
                        $cPaidWithServiceCharge = $cWriteOffAmount + $cWaiverAmountPrincipal + $cRebateAmount;
                        $cPaidWithOutServiceCharge = $cWriteOffAmountPrincipal + $cWaiverAmountPrincipal;

                        $closingDue = $shedules->where('loanIdFk',$loanId)->sum('installmentAmount') - $allCollections->where('loanIdFk',$loanId)->sum('amount') - $cPaidWithServiceCharge;
                        $closingDuePrincipal = $shedules->where('loanIdFk',$loanId)->sum('principalAmount') - $allCollections->where('loanIdFk',$loanId)->sum('principalAmount') - $cPaidWithOutServiceCharge;
                        
                        if ($loanId==183779) {
                            echo '$closingDue: '.$closingDue.'<br>';
                            echo '$closingDuePrincipal: '.$closingDuePrincipal.'<br>';
                            echo '$cPaidWithServiceCharge: '.$cPaidWithServiceCharge.'<br>';
                            echo 'installmentAmount: '.$shedules->where('loanIdFk',$loanId)->sum('installmentAmount').'<br>';
                            echo 'collectionAmount: '.$allCollections->where('loanIdFk',$loanId)->sum('amount').'<br>';
                ...
                $watchFul       = $this->getOverDueInfo($currentWatchfulLoanIds,$loanProductId,$memberIds,$thisBranchLoans,$allCollections,$shedules,$loanWriteOffs,$loanWaivers,$loanRebates);
                $substandard    = $this->getOverDueInfo($currentSubstandarLoanIds,$loanProductId,$memberIds,$thisBranchLoans,$allCollections,$shedules,$loanWriteOffs,$loanWaivers,$loanRebates);
                $doubtfull      = $this->getOverDueInfo($currentDoubtfullLoanIds,$loanProductId,$memberIds,$thisBranchLoans,$allCollections,$shedules,$loanWriteOffs,$loanWaivers,$loanRebates);
                $bad            = $this->getOverDueInfo($currentBadLoanIds,$loanProductId,$memberIds,$thisBranchLoans,$allCollections,$shedules,$loanWriteOffs,$loanWaivers,$loanRebates);
...
*/
                    ...
                    $newDueAmount = $recoverableAmount - $regularRecoveryAmount;
                    $principalNewDueAmount = $principalRecoverableAmount - $regularPrincipalRecoveryAmount;
        ...
        public function storeLoanInfoHelper($loanProductId,$genderTypeId,$monthEndNo){
            ...
            if ($monthEndNo>=1) {
                ...
                if ($openingBalanceInfoWithSerCharge!=null) {
                    // openingOutstandingAmountWithServicesCharge
        ...
        public function storeStaffInfo(){
            ...
            ->whereIn('emp_id_fk',$allEmp->pluck('iemp_id_fkd'))
            ->select('id','emp_id_fk')
            ->get();
