...
class MonthEndStoreInfo25022019beforeDuplicateMemberCounting {
    ...
    /*public function storeLoanInfoNew(){

        $branchId = $this->targetBranchId;

        $loanPayable = DB::table('mfn_loan_schedule AS loanSc')
        ->join('mfn_loan AS loan', 'loan.id', 'loanSc.loanIdFk')
        ->where('')
    }*/
    ...
    public function storeLoanInfo(){
       ...
       /* $expiredLoans = DB::table('mfn_loan')
        ...
        ->where(function($query) use ($monthEndDate){
            $query->where('loanCompletedDate','>',$this->monthEndDate)
                ->orWhere('loanCompletedDate','0000-00-00');
        })
        ->select('id','productIdFk','memberIdFk','loanAmount','totalRepayAmount')
        ->get();*/

        /*if(in_array(170339,$expiredLoans->pluck('id')->toArray())){
            echo 'in:<br>';
        }*/
       ...
       /* $watchfulLoanIds = array_diff($watchfulLoanIds,$expiredLoanIds);
        ...
        /*$expiredLoanIds = array_diff($expiredLoanIds,$watchfulLoanIds);
        $expiredLoanIds = array_diff($expiredLoanIds,$substandarLoanIds);
        $expiredLoanIds = array_diff($expiredLoanIds,$doubtfullLoanIds);*/

        $overDueLoanIds = array_merge($badLoanIds,$watchfulLoanIds,$substandarLoanIds,$doubtfullLoanIds);

        /*if(in_array(172525,$watchfulLoanIds)){
            echo 'in $watchfulLoanIds <br>';
        }
        if(in_array(172525,$substandarLoanIds)){
            echo 'in $substandarLoanIds <br>';
        }
        if(in_array(172525,$doubtfullLoanIds)){
            echo 'in $doubtfullLoanIds <br>';
        }
        if(in_array(172525,$expiredLoanIds)){
            echo 'in $expiredLoanIds <br>';
        }*/
