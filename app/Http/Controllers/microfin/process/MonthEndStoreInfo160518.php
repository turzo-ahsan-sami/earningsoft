    ...
    class MonthEndStoreInfo {
        ...
        public function getSavingsInfo($primaryProductId,$savingsProductId){
            ...
            if ($monthEndNo>=1) {
                ...
                $openingBalanceInfo = DB::table('mfn_branch_opening_balances')
                                        ...
                                        ->first();
                $openingBalance = 0;//$openingBalanceInfo->savingsDepositAmount + $openingBalanceInfo->savingsInterestAmount - $savingsInterestAmount->savingsWithdrawnAmount;
            }
        ...
        public function storeLoanInfo(){
            ...
            $thisBranchLoans = DB::table('mfn_loan')
                                    ...
                                    ->select('id','productIdFk','memberIdFk')
                                    ->get();
            ...
            $savingsDepositsOfExpiredLoanMembers = DB::table('mfn_savings_deposit')
                                                            ->where('softDel',0)
                                                            ->whereIn('memberIdFk',$expiredLoans->pluck('memberIdFk'))
            ...
            foreach ($loanProductIds as $loanProductId) {
                foreach ($genderTypeIds as $genderTypeId) {
                    ...
                    $borrowerNo = count($thisMonthLoans);

                    $disbursedAmount = $currentLoanDetails->sum('disbursmentAmount');
                    $repayAmount = $currentLoanDetails->sum('repayAmount');
                    ...
                    $fullyPaidBorrowerNo = DB::table('mfn_loan')
                                                ->where('softDel',0)
                                                ->where('branchIdFk',$this->targetBranchId)
                   ...
                   ///// new code to fetch regular, due, advance collection
                    ...
                    foreach ($currentLoanIds as $loanId) {
                        ...
                        $previousAmountPaid = $allCollections->where('loanIdFk',$loanId)->where('collectionDate','<',$this->monthFirstDate)->sum('amount');
                        $previosDueAmount = $previousAmountPayable - $previousAmountPaid;
                        $thisMonthCollectionAmount = $thiMonthCollections->where('loanIdFk',$loanId)->sum('amount');
                        ...
                        $previousPrincipalPaid = $allCollections->where('loanIdFk',$loanId)->where('collectionDate','<',$this->monthFirstDate)->sum('principalAmount');
                        $previosPrincipalDueAmount = $previousPrincipalPayable - $previousPrincipalPaid;
                        $thisMonthCollectionAmountPrincipal = $thiMonthCollections->where('loanIdFk',$loanId)->sum('principalAmount');
                        $thisMonthPayableAmount = $thisMonthShedules->where('loanIdFk',$loanId)->sum('installmentAmount') /*+ $previosDueAmount*/;
                        $thisMonthPayableAmountPrincipal = $thisMonthShedules->where('loanIdFk',$loanId)->sum('principalAmount')/* + $previosPrincipalDueAmount*/;
                        ...
                        if ($previosDueAmount>0) {
                            ...
                            if ($thisMonthPayableAmountPrincipal - $regularCollectionPrincipal>=1) {
                                /*echo "IN IF----<br>";
                                echo "Loan Id: ".$loanId."<br>";
                                echo "thisMonthPayableAmountPrincipal: ".$thisMonthPayableAmountPrincipal."<br>";
                                echo "regularCollectionPrincipal: ".$regularCollectionPrincipal."<br><br>";*/
                                $principalNewDueAmount = $principalNewDueAmount + $thisMonthPayableAmountPrincipal + $previosPrincipalDueAmount - $regularCollectionPrincipal;
                                $newDueAmount += $thisMonthPayableAmount + $previosDueAmount - $regularCollection;
                        ...
                        // collection is equal or advance
                            ...
                            // for total amount
                            $regularCollection = $thisMonthCollectionAmount > $thisMonthPayableAmount + $previosDueAmount ? $thisMonthPayableAmount + $previosDueAmount : $thisMonthCollectionAmount;
                            
                            $advanceCollection = $thisMonthCollectionAmount > $regularCollection ? ($thisMonthCollectionAmount - $regularCollection) : 0;

                            // for principal amount
                            $regularCollectionPrincipal = $thisMonthCollectionAmountPrincipal - $thisMonthPayableAmountPrincipal - $previosPrincipalDueAmount>=1 ? $thisMonthPayableAmountPrincipal + $previosPrincipalDueAmount : $thisMonthCollectionAmountPrincipal;

                            if ($thisMonthPayableAmountPrincipal + $previosPrincipalDueAmount - $regularCollectionPrincipal>=1) {
                                /*echo "IN ELSE----<br>";
                                echo "Loan Id: ".$loanId."<br>";
                                echo "thisMonthPayableAmountPrincipal: ".$thisMonthPayableAmountPrincipal."<br>";
                                echo "regularCollectionPrincipal: ".$regularCollectionPrincipal."<br><br>";*/
                                $principalNewDueAmount += $thisMonthPayableAmountPrincipal + $previosPrincipalDueAmount - $regularCollectionPrincipal;
                                $newDueAmount += $thisMonthPayableAmount + $previosDueAmount - $regularCollection;
                ...
                $closingDueAmountWithServicesCharge = (float) $openingData['openingDueAmountWithServicesCharge'] + $recoverableAmount - $regularRecoveryAmount - $dueRecoveryAmount;
                $closingDueAmount = (float) $openingData['openingDueAmount'] + $principalRecoverableAmount - $regularPrincipalRecoveryAmount - $duePrincipalRecoveryAmount;
                ...
                $watchFul       = $this->getOverDueInfo($watchfulLoanIds,$loanProductId,$memberIds,$expiredLoans,$allCollections,$shedules);
                $substandard    = $this->getOverDueInfo($substandarLoanIds,$loanProductId,$memberIds,$expiredLoans,$allCollections,$shedules);
                $doubtfull      = $this->getOverDueInfo($doubtfullLoanIds,$loanProductId,$memberIds,$expiredLoans,$allCollections,$shedules);
                $bad            = $this->getOverDueInfo($badLoanIds,$loanProductId,$memberIds,$expiredLoans,$allCollections,$shedules);
                ...
                if ($genderTypeId==1) {
                    $expiredMemberIds = $expiredMalesMemberIds;
                }
                else{
                    $expiredMemberIds = $expiredFemaleMemberIds;
                }

                $savingsDeposit = $savingsDepositsOfExpiredLoanMembers->where('primaryProductIdFk',$loanProductId)->whereIn('memberIdFk',$expiredMemberIds)->sum('amount');

                $savingsWithdraw = $savingsWIthdrawsOfExpiredLoanMembers->where('primaryProductIdFk',$loanProductId)->whereIn('memberIdFk',$expiredMemberIds)->sum('amount');
                ...
                ////////////   End Collecting the information                  
                    ...
                    $mEloanInfo->recoveryAmount                                         = $recoveryAmount + $loanRebateAmount;
        ...
        public function getOverDueInfo($loanIds,$loanProductId,$memberIds,$expiredLoans,$allCollections,$shedules){
            ...
            $outStanding = $expiredLoans->whereIn('id',$loanIds)->where('productIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->sum('loanAmount') - $allCollections->where('loanIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->whereIn('loanIdFk',$loanIds)->sum('principalAmount');

            $outStandingWithServiceCharge = $expiredLoans->whereIn('id',$loanIds)->where('productIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->sum('totalRepayAmount') - $allCollections->where('loanIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->whereIn('loanIdFk',$loanIds)->sum('amount');

            $outstandingWithMoreThan2DueInstallments = $expiredLoans->whereIn('id',$moreThan2DueInstallmentsLoanIds)->where('productIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->sum('loanAmount') - $allCollections->where('loanIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->whereIn('loanIdFk',$moreThan2DueInstallmentsLoanIds)->sum('principalAmount');

            $outstandingWithMoreThan2DueInstallmentsServicesCharge = $expiredLoans->whereIn('id',$moreThan2DueInstallmentsLoanIds)->where('productIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->sum('totalRepayAmount') - $allCollections->where('loanIdFk',$loanProductId)->whereIn('memberIdFk',$memberIds)->whereIn('loanIdFk',$moreThan2DueInstallmentsLoanIds)->sum('amount');
