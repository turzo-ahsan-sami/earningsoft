    ...
    class MonthEndStoreInfo {
        ...
        public function storeLoanInfo(){
            ...
            foreach ($loanProductIds as $loanProductId) {
                foreach ($genderTypeIds as $genderTypeId) {
                   ...
                   ///// new code to fetch regular, due, advance collection
                    ...
                    foreach ($currentLoanIds as $loanId) {
                        ...
                        $writeOffAmount = $loanWriteOffs->where('loanIdFk',$loanId)->sum('amount');
                        $writeOffAmountPrincipal = $loanWriteOffs->where('loanIdFk',$loanId)->sum('principalAmount');
                        $waiverAmountPrincipal = $loanWaivers->where('loanIdFk',$loanId)->sum('');
                        $rebateAmount = $loanRebates->where('loanIdFk',$loanId)->sum('amount');
                ...
                if (count($overDueMemberIds)>0) {
                    var_dump($overDueMemberIds);
                }
...
*/
                    ...
                    $closingOutstandingAmountWithServicesCharge = (float) $openingData['openingOutstandingAmountWithServicesCharge'] + $repayAmount - $regularRecoveryAmount - $dueRecoveryAmount - $advanceAmount - $loanRebateAmount;
