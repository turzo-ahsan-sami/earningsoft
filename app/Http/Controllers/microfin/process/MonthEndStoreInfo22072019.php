...
class MonthEndStoreInfo
{
    ...
    public function getLoanDetails()
    {
        ...
        $loans = DB::table('mfn_loan AS loan')
            ...
            ->select('mi.gender', 'mi.id AS memberId', 'loan.id', 'loan.loanCode', 'loan.productIdFk', 'loan.primaryProductIdFk', 'lp.fundingOrganizationId', 'loan.loanAmount', 'loan.totalRepayAmount', 'loan.disbursementDate', 'loan.loanCompletedDate', 'loan.loanTypeId', 'loan.loanRepayPeriodIdFk')
            ->get();
        ...
        foreach ($loans as $loan) {
            ...
            // IF THIS LOAN PRODUCT IS PRIMARY THEN IT IS UNIQUE LOANEE, OR IF IT IS OPTIONAL PRODUCT LOAN AND IF THIS MEMBER HAS NO PRIMARY LOAN THEN IT IS ALSO UNIQUE LOANEE.
            $loanData->noOfUniqueLoanee = 1;
            if ($loan->loanCompletedDate <= $monthEndDate && $loan->loanCompletedDate != '0000-00-00') {
                $loanData->noOfUniqueLoanee = 0;
            }

            if ($loan->primaryProductIdFk != $loan->productIdFk) {
                $isAnyPrimaryLoanOne = (int) $loans->where('memberId', $loan->memberId)->where('productIdFk', '!=', $loan->productIdFk)->where('loanCompletedDate', '=', '0000-00-00')->max('id');
                $isAnyPrimaryLoanTwo = (int) $loans->where('memberId', $loan->memberId)->where('productIdFk', '!=', $loan->productIdFk)->where('loanCompletedDate', '>', $monthEndDate)->max('id');

                if (max($isAnyPrimaryLoanOne, $isAnyPrimaryLoanTwo) > 0) {
                    $loanData->noOfUniqueLoanee = 0;
                }
            }
        ...
        foreach ($loanProducts as $loanProduct) {
            foreach ($genderIds as $genderId) {
                ...
                $productData->cumBorrowerNo             = $openingInfo->sum('cumBorrowerNo') + $productData->borrowerNo;
        ...
        foreach ($funOrgIds as $funOrgId) {
            foreach ($genderIds as $genderId) {
                ...
                $closingBorrowerIds = array_unique(array_merge($currentLoans
                    ->where('loanCompletedDate', '=', '0000-00-00')
                    ->unique('memberId')
                    ->pluck('memberId')
                    ->toArray(), $currentLoans
                    ->where('loanCompletedDate', '>', '0000-00-00')
                    ->unique('memberId')
                    ->pluck('memberId')
                    ->toArray()));

                $fullyPaidBorrowerNo = count(array_diff($currentLoans
                    ->where('loanCompletedDate', '>=', $monthFirstDate)
                    ->where('loanCompletedDate', '<=', $monthEndDate)
                    ->unique('memberId')
                    ->pluck('memberId')
                    ->toArray(), $closingBorrowerIds));

                $closingBorrowerNo = count($closingBorrowerIds);
        ...
        // FOR GRAND TOTAL
        $thisMonthBorrowers = DB::table('mfn_loan AS loan')
            ->join('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
            ->where([['loan.softDel', 0], ['loan.branchIdFk', $this->targetBranchId], ['loan.disbursementDate', '<=', $monthEndDate]])
        ...
        $closingLoans = DB::table('mfn_loan AS loan')
            ->join('mfn_member_information AS mi', 'mi.id', 'loan.memberIdFk')
            ->where([['loan.softDel', 0], ['loan.branchIdFk', $this->targetBranchId], ['loan.disbursementDate', '<=', $monthEndDate]])
            ->where(function ($query) use ($monthFirstDate, $monthEndDate) {
                $query->where('loan.loanCompletedDate', '0000-00-00')
