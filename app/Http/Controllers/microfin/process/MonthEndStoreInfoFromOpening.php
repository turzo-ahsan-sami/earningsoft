...
class MonthEndStoreInfoFromOpening {
        ...
        foreach ($loanProducts as $loanProduct) {
            foreach ($genderTypeIds as $genderTypeId) {
                ...
                if ($genderTypeId==1) {
                    ...
                    // $redundantMemberNo = $curInfo->sum('redundantMaleMemberNo');
                    ...
                    // $redundantMemberNo = $curInfo->sum('redundantFemaleMemberNo');
                    $openingMember = $closingMember - $newMemberAdmissionNo + $memberCancellationNo;
                }
    ...
    public function storeMemberInfoCategoryWise(){
        ...
        ->get();
            ...
            // When it need to store data category wise, then only samity number should be taken from mfn_opening_info_samity_total table.
        $openingSamityInfo = DB::table('mfn_opening_info_samity_total')
        ->where('categoryIdFk','>',0)
        ->where('branchIdFk',$this->targetBranchId)
        ->get();
        ...
        foreach ($productCategoryIds as $productCategoryId) {
            ...
            foreach ($curFunOrgIds as $curFunOrgId) {
                ...
                foreach ($genderTypeIds as $genderTypeId) {
                    ...
                    $curSamityInfo = $openingSamityInfo->where('categoryIdFk',$productCategoryId)->where('fundingOrgIdFk',$curFunOrgId);

                    if ($genderTypeId==1) {

                        $newSamityNo = $curSamityInfo->sum('thisMonthNewMaleSamityNo');
                        $cancelSamityNo = $curSamityInfo->sum('thisMonthCloseMaleSamityNo');
                        $closingSamityNo = $curSamityInfo->sum('maleSamityNo');
                        $openingSamityNo = $closingSamityNo - $newSamityNo + $closingSamityNo;
                        ...
                        $newSamityNo = $curSamityInfo->sum('thisMonthNewFemaleSamityNo');
                        $cancelSamityNo = $curSamityInfo->sum('thisMonthCloseFemaleSamityNo');
                        $closingSamityNo = $curSamityInfo->sum('femaleSamityNo');
                        $openingSamityNo = $closingSamityNo - $newSamityNo + $closingSamityNo;
