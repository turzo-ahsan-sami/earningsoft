	...
	class MicroFinance {
		...
		*/
		...
		public function getRegularProductOptionsMemberWise($memberId) {
			...
			$memberPrimaryProductId = DB::table('mfn_member_information')->where('id', $memberId)->first()->primaryProductId;
		...
		public function getRegularLoanFirstRepayDate($memberId, $disbursementDate, $productId, $repaymentFrequencyId) {
			...
			if($repaymentFrequencyId==2):
				...
				if(date('D', strtotime($firstRepayDate))==$dayArr[$samityDayId]):
					...
					//	WHEN DISBURSEMENT DATE AND FIRST REPAY DATE ARE IN SAME MONTH,
					//	THEN SET FIRST REPAY DATE TO 1 WEEK AHEAD.
					if((int) Carbon::parse($disbursementDate)->format('n')==(int) Carbon::parse($firstRepayDate)->format('n')):
						$firstRepayDate = self::getFilterSameMonthFirstRepayDate(Carbon::parse($firstRepayDate));
					endif;
				else:
					...
					if($daysDiff>=30):
						...
						if(((int) $toDate->format('j') - (int) $dt->format('j')) > 3)
							$firstRepayDate = $dt->addWeeks(1)->toDateString();
						else
							$firstRepayDate = $dt->toDateString();
							...
							if(date_create($val)>=$firstRepayDate):
								if(date_create($val)==date_create($firstRepayDate)):
									...
									if(((int) $toDate->format('j') - (int) $dt->format('j')) > 3)
										...
										$firstRepayDate = $dt->subWeeks(1)->toDateString();
						...
						//	WHEN DISBURSEMENT DATE AND FIRST REPAY DATE ARE IN SAME MONTH,
						//	THEN SET FIRST REPAY DATE TO 1 WEEK AHEAD.
						if((int) Carbon::parse($disbursementDate)->format('n')==(int) $dt->format('n')):
							$firstRepayDate = self::getFilterSameMonthFirstRepayDate($dt);
						endif;

						//	WHEN FIRST REPAY DATE IS AFTER 2 MONTH OF DISBURSEMENT DATE,
						//	THEN SET FIRST REPAY DATE TO 1 WEEK BEFORE.
						while(((int) Carbon::parse($firstRepayDate)->format('n') - (int) Carbon::parse($disbursementDate)->format('n'))!=1 && (int) Carbon::parse($disbursementDate)->format('n')!=12):
							if(((int) $dt->format('n') - (int) Carbon::parse($disbursementDate)->format('n'))==2):
								$firstRepayDate = self::getFilterTwoMonthsGapFirstRepayDate($dt, $disbursementDate);
							endif;
						endwhile;
					endif;
				endif;
	...
	*/
		...
		public function getLoanRescheduleForHoliday($dateFrom, $dateTo, $applicableFor, $orgId, $branchId, $samityId) {
			...
			$dbLoans = DB::table('mfn_loan_schedule AS t1')
								...
								->where('t1.scheduleDate','>=',$dateFrom)
								->where('t1.scheduleDate','<=',$dateTo)
				...
				foreach($branchIdArr as $branchId):
					foreach($samityDayIdArr as $samityDayId):
						...
						foreach($samityIdArr as $key => $val):
							...
							$loanOB = $dbLoans->where('samityIdFk', $val);
						...
						foreach($loanIdArr as $loanId):
							...
							// $singleLoanOB = DB::table('mfn_loan')->where('id', $loanId)->select('loanTypeId', 'repaymentFrequencyIdFk')->first();
							$singleLoanOB = $dbLoans->where('id', $loanId)->first();
