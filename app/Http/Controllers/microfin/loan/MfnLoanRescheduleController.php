<?php
	...
	class MfnLoanRescheduleController extends Controller {
		...
		public function addReschedule($loanId, $installmentSl) {
			...
			$loanOB = $this->MicroFinance->getMultipleValueForId($table='mfn_loan', $loanId, ['loanCode', 'memberIdFk', 'samityIdFk', 'branchIdFk', 'repaymentFrequencyIdFk','disbursementDate','loanTypeId']);
			...
			$toDate 	= Carbon::parse($loanOB->disbursementDate)->addMonths(13);
			$toDate 	= $toDate->toDateString();

			//	GET HOLIDAY.
			$globalGovtHoliday = $this->MicroFinance->getGlobalGovtHolidayByDate($loanOB->disbursementDate,$toDate);
			//dd($loanOB);
			$organizationHoliday = $this->MicroFinance->getOrganizationHolidayByDate(1,$loanOB->disbursementDate);
			$branchHoliday = $this->MicroFinance->getBranchHolidayByDate($loanOB->disbursementDate);
			$samityHoliday = $this->MicroFinance->getSamityHolidayByDate($loanOB->memberIdFk,$loanOB->disbursementDate);
			...
			//dd($loanOB->disbursementDate,$globalGovtHoliday,$organizationHoliday);
			$loanSchedule = $this->MicroFinance->generateLoanSchedule([0=>$loanId])[$loanId];
			

			$dateFrom = $loanSchedule[$installmentSl-1]['scheduleDate'];
		...
		public function rescheduleGenerate(Request $req) {
			...
			try{
				...
				$installmentId = [];
				$loanSchedule = $this->MicroFinance->generateLoanSchedule([0=>$req->loanId])[$req->loanId];
			//dd($loanSchedule);

				$i = 0;
				foreach($loanSchedule as $scheduleDate):
					if($i <$req->installmentNumber-1){
						continue;
					}
				...
				for($i=0;$i<count($installmentId);$i++):
					...
					$logArray = array(
					    'moduleId'  => 6,
					    'controllerName'  => 'MfnLoanRescheduleController',
					    'tableName'  => 'mfn_loan_schedule',
					    'operation'  => 'insert',
					    'primaryIds'  => [DB::table('mfn_loan_schedule')->max('id')]
						);
