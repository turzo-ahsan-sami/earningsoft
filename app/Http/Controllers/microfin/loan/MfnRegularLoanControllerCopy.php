	...
	public function __construct() {
		...
		*/
		public function addRegularLoan() {
			...
			//dd(GetSoftwareDate::getSoftwareDate());

			// dd($this->MicroFinance->getBooleanOptions(), $isSelfEmployment);
		...
		*/
		...
		public function updateItem(Request $req) {
			...
			// dd($req->member_regular_signature_image, $req->memberRegularSignatureImage, $req->image, $req);
		...
		*/
		public function detailsRegularLoan($regularLoanId) {
			...
			$loan = DB::table('mfn_loan')->where('softDel', '=', 0)->limit(1000)->pluck('id');
			...
			//Waiver OFF
			$waiverBalance = DB::table('mfn_loan_waivers')/*->where('softDel',0)*/->where('loanIdFk',$regularLoanDetails->id)->select('principalAmount')->first();

			$totalCollectionForLoan = (float)($openingBalance == null ? 0.00 : $openingBalance->paidLoanAmountOB)  + (float) ($collectionBalance == null ? 0.00 : $collectionBalance) + (float) ($writeOffBalance == null ? 0.00 : $writeOffBalance->amount) + (float)
			($rebateBalance == null ? 0.00 : $rebateBalance->amount) + (float) ($waiverBalance == null ? 0.00 : $waiverBalance->principalAmount);
