...
class MfnRegularLoanController extends Controller
{
	...
	public function loadRegularLoanSupportData(Request $req)
	{
		...
		if ($eligibleRepaymentFrequencyArr[0] == 2) {
			$samityId = DB::table('mfn_member_information')->where('id', $req->memberId)->first()->samityId;
			$firstRepayDate = MicroFin::getFirstRepayDateForMonthlyLoan($samityId, $req->disbursementDate);
		}
	...
	public function addItem(Request $req)
	{
		...
		try {
			...
			elseif ($req->paymentTypeIdFk == 'Bank') {
				$req->request->add(['ledgerId' => $req->ledgerId]);
			}

			$member = DB::table('mfn_member_information')->where('id', $req->memberIdFk)->select('samityId', 'primaryProductId', 'branchId')->first();
			$req->request->add(['samityIdFk' => $member->samityId]);
			$req->request->add(['primaryProductIdFk' => $member->primaryProductId]);
			...
			$req->request->add(['branchIdFk' => $member->branchId]);
			...
			// CREATE LOAN
			$loan = MfnLoan::create($loanData);

			//	STORE ALL LOAN FEES.
			$req->request->add(['createdDate' => $now]);
			$req->request->add(['name' => 'loan']);
			$req->request->add(['loanIdFk' => $loan->id]);
			$req->request->add(['loanAdditionalFee' => $req->additionalFee]);
			$req->request->add(['loanFormFee' => $req->loanFormFee]);
			MfnFees::create($req->all());

			// GENERATE LOAN SCHEDULE
			$info = MicroFin::generateLoanSchedule($loan->id);
			if ($info['error'] == true) {
				$data = array(
					'responseTitle'  =>  'Warning!',
	...
	public function updateItem(Request $req)
	{
		...
		$isTransctionExits = (int) DB::table('mfn_loan_collection')
			->where('softDel', 0)
			->where('amount', '>', 0)
		...
		$softDate 			= MicroFin::getSoftwareDateBranchWise($loan->branchIdFk);
		$branchSoftwareDate = DB::table('gnr_branch')->where('id', $loan->branchIdFk)->select('softwareStartDate')->first();
		if ($loan->isFromOpening == 1) {
			...
			if ($softDate != $loan->disbursementDate) {
				$data = array(
					'responseTitle'  =>  MicroFinance::getMessage('msgWarning'),
					'responseText'   =>  MicroFinance::getMessage('updateOnNonSoftwareStartDateWarning'),
				);
