...
class CashBookReportController extends Controller {
    ...
    public function index() {
        ...
        // ledgers
        $ledgers = DB::table('acc_account_ledger')->where('companyIdFk', $userCompanyId)->where('accountTypeId', 4)->where('isGroupHead', 0)->orderBy('code','asc')->select('id','name','code')->get();
        ...
        return view('accounting.reports.cashBookReportViews.viewCashBookReportFilterForm', $accCashBookReportArr);
    }

    public function loadCashBookReport(Request $request) {
        $dbhelper = new DatabasePartitionHelper();  

        
        
        $acc_voucher = $dbhelper->getCompanyWisePartitionName('acc_voucher');           
        ...
        if ((int)$projectId == 0) {
            ...
            if ($branchId == null) {
                $branchIdArr =  [$userBranchId => $userBranchId] + DB::table('gnr_branch')->where('companyId', $user_company_id)->where('projectId', $projectId)->pluck('id', 'id')->toArray();
            }
            elseif ($branchId == 0) {
                $branchIdArr = DB::table('gnr_branch')->where('companyId', $user_company_id)->where('id', '!=', $headOfficeId)->where('companyId', $user_company_id)->where('projectId', $projectId)->pluck('id')->toArray();
            }
            else {
        ...
        $projectName = (int)$projectId == 0 ? 'All' : DB::table('gnr_project')->where('companyId', $user_company_id)->where('id', $projectId)->value('name');
        $projectTypeName = (int)$projectTypeId == 0 ? 'All' : DB::table('gnr_project_type')->where('companyId', $user_company_id)->where('id', $projectTypeId)->value('name');
        ...
        elseif ($branchId == 0) {
            ...
            $branchName = DB::table('gnr_branch')->where('id', $branchId)->where('companyId', $user_company_id)->value(DB::raw("CONCAT(LPAD(branchCode, 3, 0), ' - ', TRIM(REPLACE(name, '\t', '')))"));
        }

        // ledger head name
        $ledgerHead = DB::table('acc_account_ledger')->where('companyIdFk', $user_company_id)->where('id', $ledgerId)->value(DB::raw("CONCAT(name, '[', code, ']')"));

        // requested info array
        $cashBookReportLoadTableArr = array(
            'company'                 => $company,
            'dateFrom'                => $dateFrom,
            'dateTo'                  => $dateTo,
        ...
        if (in_array($lastOpeningDate, $yearEnds)) {
            ...
            $acc_opening_balance = $dbhelper->getCompanyWisePartitionName('acc_opening_balance');
        ...
        // structuring data to send information in view file
                       ...
                       // dd($openingBalance);
        $data = array(
            'cashBookReportLoadTableArr'    => $cashBookReportLoadTableArr,
            'ledgerId'                      => $ledgerId,
        ...
        return view('accounting.reports.cashBookReportViews.viewCashBookReportTable', $data);
    }
