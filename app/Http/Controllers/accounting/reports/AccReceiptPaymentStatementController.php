...
class AccReceiptPaymentStatementController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $acc_voucher = DatabasePartitionHelper::getDBTableName('acc_voucher', $partitionName);
        $acc_opening_balance = DatabasePartitionHelper::getDBTableName('acc_opening_balance', $partitionName);  
        ...
        if ($request->dateFrom==null) {
            ...
            elseif ($searchMethodSelected==2) {
                $endDate = Carbon::parse($request->dateTo)->format('Y-m-d');
                $startDate = DB::table('gnr_fiscal_year')->where('fyStartDate','<=', $endDate)->where('fyEndDate','>=', $endDate)->value('fyStartDate');
                $dateFromSelected = $startDate;
            }
        }
        else{
        ...
        if(isset($request->searchMethod)){
            ...
            $recpeitLegderIds = DB::table('acc_account_ledger')->whereNotIn('accountTypeId',[4,5])->whereNotIn('id',[285,348])->pluck('id')->toArray();
            $paymentLegderIds = DB::table('acc_account_ledger')->whereNotIn('accountTypeId',[4,5])->whereNotIn('id',[285,348])->pluck('id')->toArray();
            ...
            // if ($branchSelected == null || $branchSelected == 0) {
                ...
                if ($searchMethodSelected == 2) {
                    ...
                    $yearEndInfo = DB::table($acc_opening_balance)
                                    ->where('projectId', $projectId)
                                    ->whereIn('projectTypeId', $projectTypeId)
                                    ->whereIn('branchId', $branchId)
                                    ->where('fiscalYearId', $previousfiscalYearId)
                                    ->select(
                                        'openingDate', 'fiscalYearId', 'ledgerId',
        ...
        //                dd($cashTypeLedgers);
                ...
                // dd($dateRangeTransactionOfBankTypeDebit);
            ...
            if ($searchMethodSelected == 2) {
                ...
                $openingBalanceInfo = DB::table($acc_opening_balance)
                                    ...
                                    ->whereIn('projectTypeId', $projectTypeId)
                                    ->whereIn('branchId', $branchId)
                                    ->whereIn('fiscalYearId', [$previousPreviousfiscalYearId, $previousfiscalYearId])
                                    ->select('ledgerId', 'fiscalYearId',
                                            'debitAmount', 'creditAmount', 'balanceAmount',
                                            'cashDebit', 'bankDebit', 'cashCredit', 'bankCredit',
                                            'jvDebit', 'jvCredit', 'ftDebit', 'ftCredit'
                                            )
                                    ->get();
            }
            ...
            elseif ($searchMethodSelected == 2) {
                ...
                if ($previousfiscalYearId!=null) {
                    //opening of current
                    $currentYearOpeningBalance = $previousYearsInfo;
                    // cash in hand
                    $cashInHandForCurrentFiscalYear = $currentYearOpeningBalance
                                                    ->whereIn('ledgerId',$cashTypeLedgers)
                                                    ->sum('balanceAmount');
            ...
            foreach ($finalLevelReceiptPaymentLedgers as $key => $ledger) {
                ...
                if ($searchMethodSelected == 2) {
                    $openingBalanceLedgerInfo = $previousYearsInfo->where('ledgerId',$ledger->id);
                }
                else {
                    $openingBalanceLedgerInfo = $openingBalanceInfo->where('fiscalYearId', $previousfiscalYearId)->where('ledgerId',$ledger->id);
                }
                ...
                if ($voucherTypeSelected == "") {
                    ...
                    // opening for cumulative
                    $openingDebitForCumulative = $openingBalanceLedgerInfo->sum('cashDebit') + $openingBalanceLedgerInfo->sum('bankDebit');
                    $openingCreditForCumulative = $openingBalanceLedgerInfo->sum('cashCredit') + $openingBalanceLedgerInfo->sum('bankCredit');
                    //Curent Debit Credit
                    // if ($branchSelected == null || $branchSelected == 0) {
                        // previous
                        ...
                        // date range
                        $dateRangeDebit = $dateRangeLedgerInfo->sum('cashDebit') + $dateRangeLedgerInfo->sum('bankDebit');
                        $dateRangeCredit = $dateRangeLedgerInfo->sum('cashCredit') + $dateRangeLedgerInfo->sum('bankCredit');
                    // }
                    // else {
                ...
                elseif($voucherTypeSelected == 1){
                    ...
                    // opening for cumulative
                    $openingDebitForCumulative = $openingBalanceLedgerInfo->sum('cashDebit') +
                                                $openingBalanceLedgerInfo->sum('bankDebit') +
                                                $openingBalanceLedgerInfo->sum('jvDebit');
                    $openingCreditForCumulative = $openingBalanceLedgerInfo->sum('cashCredit') +
                                                $openingBalanceLedgerInfo->sum('bankCredit') +
                                                $openingBalanceLedgerInfo->sum('jvCredit');

                    // if ($branchSelected == null || $branchSelected == 0) {

                        // previous
                        $previousFiscalYearDebit = $previousYearLedgerInfo->sum('cashDebit') +
                                                    $previousYearLedgerInfo->sum('bankDebit') +
                                                    $previousYearLedgerInfo->sum('jvDebit');

                        $previousFiscalYearCredit = $previousYearLedgerInfo->sum('cashCredit') +
                                                    $previousYearLedgerInfo->sum('bankCredit') +
                                                    $previousYearLedgerInfo->sum('jvCredit');
                        // current
                        ...
                        // date range
                        $dateRangeDebit = $dateRangeLedgerInfo->sum('cashDebit') + $dateRangeLedgerInfo->sum('bankDebit') + $dateRangeLedgerInfo->sum('jvDebit');
                        $dateRangeCredit = $dateRangeLedgerInfo->sum('cashCredit') + $dateRangeLedgerInfo->sum('bankCredit') + $dateRangeLedgerInfo->sum('jvCredit');
                    // }
                    // else {
                    //     // jv
                    ...
                    //                         ->sum('amount');
                    //     // this month
                    //     $thisMonthDebitForJV = $thisChildVoucherDetails
                    //                         ->where('debitAcc', $ledger->id)
                    //                         ->whereIn('voucherId',$jvouchers)
                    //                         ->whereIn('voucherId',$thisMonthVoucherIds)
                    //                         ->sum('amount');
                    //
                    //     $thisMonthCreditForJV = $thisChildVoucherDetails
                    //                         ->where('creditAcc', $ledger->id)
                    //                         ->whereIn('voucherId',$jvouchers)
                    //                         ->whereIn('voucherId',$thisMonthVoucherIds)
                    //                         ->sum('amount');
                    ...
                    //     $thisMonthDebit = $thisMonthDebit + $thisMonthDebitForJV;
                    //     $thisMonthCredit = $thisMonthCredit + $thisMonthCreditForJV;
                    // }
                ...
                elseif($voucherTypeSelected == 2){
                    ...
                    // opening for cumulative
                    $openingDebitForCumulative = $openingBalanceLedgerInfo->sum('jvDebit');
                    $openingCreditForCumulative = $openingBalanceLedgerInfo->sum('jvCredit');
                    ...
                    // if ($branchSelected == null || $branchSelected == 0) {
                        ...
                        // date range
                        $dateRangeDebit = $dateRangeLedgerInfo->sum('jvDebit');
                        $dateRangeCredit = $dateRangeLedgerInfo->sum('jvCredit');
                    ...
                    //     $thisMonthDebit = $thisChildVoucherDetails
                    //                     ->where('debitAcc', $ledger->id)
                    //                     ->whereIn('creditAcc',$jvouchers)
                    //                     ->whereIn('voucherId',$thisMonthVoucherIds)
                    //                     ->sum('amount');
                    //
                    //     $thisMonthCredit = $thisChildVoucherDetails
                    //                     ->where('creditAcc', $ledger->id)
                    //                     ->whereIn('debitAcc',$jvouchers)
                    //                     ->whereIn('voucherId',$thisMonthVoucherIds)
                    //                     ->sum('amount');
                    // }
                ...
                //     if ($branchSelected == null || $branchSelected == 0) {
                //
                //         $openingBalanceInfo = $openingBalanceInfo->where('fiscalYearId', $previousfiscalYearId)->where('ledgerId',$ledger->id);
                //
                //     }
                //     else {
                ...
                $ledgerWiseData[] = array(
                    ...
                    'dateRangeDebit'                => $dateRangeDebit,
                    'dateRangeCredit'               => $dateRangeCredit,
