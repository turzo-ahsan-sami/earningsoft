...
class AccCashFlowStatementController extends Controller{
    ...
    public function cashFlowStatementLoadTable(Request $request) {
        ...
        // dd($thisMonthVouchers);

        // surplus calculation
        $incomeLedgers = $ledgersCollection->where('accountTypeId', 12)->pluck('id')->toArray();
        $expensesLedgers = $ledgersCollection->where('accountTypeId', 13)->pluck('id')->toArray();

        foreach ($incomeLedgers as $key => $ledgers) {

            // current year
            $cyIncomeDebitAmount  = $currentYearVouchers->where('debitAcc', $ledgers)->sum('amount');
            $cyIncomeCreditAmount = $currentYearVouchers->where('creditAcc', $ledgers)->sum('amount');
            $cyIncomeBalanceArr[] = $cyIncomeCreditAmount - $cyIncomeDebitAmount;
            // previous year
            $pyIncomeDebitAmount  = $previousYearVouchers
                                    ->where('voucherDate', '>=', $previousYearStartDate)
                                    ->where('debitAcc', $ledgers)
                                    ->sum('amount');
            $pyIncomeCreditAmount = $previousYearVouchers
                                    ->where('voucherDate', '>=', $previousYearStartDate)
                                    ->where('creditAcc', $ledgers)
                                    ->sum('amount');
            $pyIncomeBalanceArr[] = $pyIncomeCreditAmount - $pyIncomeDebitAmount;
            // this month
            $thisMonthIncomeDebitAmount  = $thisMonthVouchers->where('debitAcc', $ledgers)->sum('amount');
            $thisMonthIncomeCreditAmount = $thisMonthVouchers->where('creditAcc', $ledgers)->sum('amount');
            $thisMonthIncomeBalanceArr[] = $thisMonthIncomeCreditAmount - $thisMonthIncomeDebitAmount;
            // cumulative
            $cumulativeIncomeDebitAmount  = $cumulativeVouchers->where('debitAcc', $ledgers)->sum('amount');
            $cumulativeIncomeCreditAmount = $cumulativeVouchers->where('creditAcc', $ledgers)->sum('amount');
        ...
        foreach ($expensesLedgers as $key => $ledgers) {
            ...
            // current year
            $cyExpensesDebitAmount  = $currentYearVouchers->where('debitAcc', $ledgers)->sum('amount');
            $cyExpensesCreditAmount = $currentYearVouchers->where('creditAcc', $ledgers)->sum('amount');
            $cyExpensesBalanceArr[] = $cyExpensesDebitAmount - $cyExpensesCreditAmount;
            // previous year
            $pyExpensesDebitAmount  = $previousYearVouchers->where('debitAcc', $ledgers)->sum('amount');
            $pyExpensesCreditAmount = $previousYearVouchers->where('creditAcc', $ledgers)->sum('amount');
            $pyExpensesBalanceArr[] = $pyExpensesDebitAmount - $pyExpensesCreditAmount;
            // this month
            $thisMonthExpensesDebitAmount  = $thisMonthVouchers->where('debitAcc', $ledgers)->sum('amount');
            $thisMonthExpensesCreditAmount = $thisMonthVouchers->where('creditAcc', $ledgers)->sum('amount');
            $thisMonthExpensesBalanceArr[] = $thisMonthExpensesDebitAmount - $thisMonthExpensesCreditAmount;
            // cumulative
            $cumulativeExpensesDebitAmount  = $cumulativeVouchers->where('debitAcc', $ledgers)->sum('amount');
            $cumulativeExpensesCreditAmount = $cumulativeVouchers->where('creditAcc', $ledgers)->sum('amount');
            $cumulativeExpensesBalanceArr[] = $cumulativeExpensesDebitAmount - $cumulativeExpensesCreditAmount;
        }
