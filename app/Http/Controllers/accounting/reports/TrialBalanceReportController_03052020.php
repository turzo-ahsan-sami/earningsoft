...
class TrialBalanceReportController extends Controller{
    ...
    public function trialBalanceLoadReport(Request $request) {
        ...
        if ($searchType == 2) {
            ...
            if ($project != "0") {
                $opVoucherDebitInfo->where('acc_voucher.projectId', $project);
            }
            if ($projectType != "0") {
                $opVoucherDebitInfo->where('acc_voucher.projectTypeId', $projectType);
            }
            ...
            if ($project != "0") {
                $opVoucherCreditInfo->where('acc_voucher.projectId', $project);
            }
            if ($projectType != "0") {
                $opVoucherCreditInfo->where('acc_voucher.projectTypeId', $projectType);
            }
