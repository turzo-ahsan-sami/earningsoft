 	...
 	public function index(Request $request){
      ...
      $allLedgers = DB::table('acc_account_ledger')->select("id","projectBranchId")->get();
        ...
        if (!is_null($startDate) && !is_null($endDate)) {
        	$vouchers = DB::table(DB::raw("acc_voucher PARTITION ($partitionName)"))->where('projectId',$projectSelected)->whereIn('projectTypeId',$projectTypeId)->whereIn('branchId',$branchId)->where('voucherDate','>=',$startDate)->where('voucherDate','<=',$endDate)->pluck('id')->toArray();
        }
        else{
        	$vouchers = DB::table(DB::raw("acc_voucher PARTITION ($partitionName)"))->where('projectId',$projectSelected)->whereIn('projectTypeId',$projectTypeId)->whereIn('branchId',$branchId)->pluck('id')->toArray();
        }
        ...
        if (!is_null($startDate) && !is_null($endDate)) {
          $openingVouchers = DB::table(DB::raw("acc_voucher PARTITION ($partitionName)"))->where('projectId',$projectSelected)->whereIn('projectTypeId',$projectTypeId)->whereIn('branchId',$branchId)->where('voucherDate','<',$startDate)->where('voucherDate','>=',$fiscalStartDate)->pluck('id')->toArray();
        }
        else{
          $openingVouchers = DB::table(DB::raw("acc_voucher PARTITION ($partitionName)"))->where('projectId',$projectSelected)->whereIn('projectTypeId',$projectTypeId)->whereIn('branchId',$branchId)->pluck('id')->toArray();
        }
