...
class AccOtsRegisterController extends Controller
{
  ...
  public function storeOts(Request $request){
    ...
    if ($validator->fails()){
        ...
        $logArray = array(
            'moduleId'  => 4,
            'controllerName'  => 'AccOtsRegisterController',
            'tableName'  => 'acc_ots_account',
            'operation'  => 'insert',
            'primaryIds'  => [DB::table('acc_ots_account')->max('id')]
        );
...
public function editOts(Request $request){
    ...
    if ($validator->fails()){
        ...
        ///Edit Information
            ...
            // IF ANY TRANSACTION(INTEREST OR PAYMENT) THEN IT CAN NOT BE EDITABLE.
       $previousdata = AccOTSRegisterAccount::find($request->EMaccountId);
       $account = AccOTSRegisterAccount::find($request->EMaccountId);

       $isTransactionExits = AccOTSRegisterInterestDetails::where('accId_fk',$account->id)->max('id');
       $isTransactionExits += AccOTSRegisterPaymentDetails::where('accId_fk',$account->id)->max('id');
    ...
    $interestRate = DB::table('acc_ots_period')->where('id',$request->EMperiod)->first()->interestRate;
...
public function deleteOts(Request $request){
 $previousdata = AccOTSRegisterAccount::find($request->accId);
...
public function getNonMonthlyAccountBaseOnBranch(Request $request){

    $accounts = DB::table('acc_ots_account')->whereNotIn('periodId_fk',[1,5]);

    if ($request->branchId!="") {
        $accounts = $accounts->where('branchId_fk',$request->branchId);
    }

    $accounts = $accounts->select('id','accNo')->get();
 ...
 return response::json($data);
   ...
   /* public function getAccountData(Request $request)
    ...
    public function viewInterest(){
        ...
        return view('accounting.register.otsRegister.viewOtsInterest',$data);
    }
    ...
    public function generateInterest(Request $req) {
        ...
        $lastGenerationDate = Carbon::parse(AccOTSRegisterInterest::where('isForAll',1)->max('dateTo'));
        ...
        $hasInterestInThisMonth = (int) AccOTSRegisterInterest::where('isForAll',1)->where('dateTo','>=',$firstDayOfMonth)->where('dateTo','<=',$lastDayOfMonth)->value('id');        
        /*End Check Is there any interest generated in this month*/        

        /*If has existing data*/
        if ($hasInterestInThisMonth>0) {
            $interest = AccOTSRegisterInterest::where('isForAll',1)->where('dateTo','>=',$firstDayOfMonth)->where('dateTo','<=',$lastDayOfMonth)->first();
        ...
        $monthlyAccounts = DB::table('acc_ots_account')->whereIn('periodId_fk',[1,5])->where('openingDate','<',$lastDayOfMonth->format('Y-m-d'))->where('status',1)->get();
        ...
        foreach ($monthlyAccounts as $key => $monthlyAccount) {
            ...
            $interestRate = $monthlyAccount->interestRate;
        ...
        // Other Accounts
        $accounts = DB::table('acc_ots_account')->whereNotIn('periodId_fk',[1,5])->where('openingDate','<',$lastDayOfMonth->format('Y-m-d'))->where('status',1)->get();
        ...
        $logArray = array(
            'moduleId'  => 4,
            'controllerName'  => 'AccOtsRegisterController',
            'tableName'  => 'acc_ots_interest',
            'operation'  => 'insert',
            'primaryIds'  => [DB::table('acc_ots_interest')->max('id')]
        );
        Service::createLog($logArray);
        return response::json($data);
    ...
    /*generate interest account wise*/
    public function generateOtsInterestForParticularAccount(Request $req){

        $firstDayOfMonth = Carbon::parse($req->date)->startOfMonth();        
        $lastDayOfMonth = Carbon::parse($req->date)->endOfMonth();
        $generateDate = Carbon::parse($req->date);

        $account = DB::table('acc_ots_account')->where('id',$req->otsAccId)->first();

        $lastInterestGeneratedDate = DB::table('acc_ots_interest_details')->where('accId_fk',$account->id)->orderBy('dateTo','desc')->value('dateTo');

        // IF IT IS NOT MONTHLY/ONE TIME ACCOUNT
        if ($account->periodId_fk!=1 && $account->periodId_fk!=5) {   
            if ($lastInterestGeneratedDate!=null || $lastInterestGeneratedDate!='') {
                $dateFrom = Carbon::parse($lastInterestGeneratedDate)->addDay();
            }
            else{
            ...
            /*Validation*/
            if ($dateFrom->copy()->subDay()->gte($generateDate)) {
                $data = array(
                    'responseTitle' =>  'Info!',
                    'responseText'  =>  "Invalid Date. Interest is already generated or date is before the account opening dtae."
                );
                return response::json($data);
            }
            ...
            while($periodEndDate->copy()->addMonthsNoOverflow($periodLenght)->lte($generateDate)) {
                $periodEndDate->addMonthsNoOverflow($periodLenght);
                $period = $period + $periodLenght;
            }
            ...
            if ($generateDate->gte($periodEndDate)) {
                ...
                $interestDetails->dateFrom = $dateFrom;
                $interestDetails->dateTo = $periodEndDate;
                $interestDetails->generateDate = $generateDate;
                $interestDetails->amount = $amount;
                $interestDetails->periodNumber = $periodNumber;
                $interestDetails->save();
                /*End Store Data to Database*/
                ...
                $data = array(
                    'responseTitle' =>  'Info!',
                    'responseText'  =>  "Period not complited yet."
                );
                return response::json($data);
            }        
        }
            ...
            $data = array(
                'responseTitle' =>  'Info!',
                'responseText'  =>  "This is Only for non Monthly/One Time accounts."
            );
            return response::json($data);
        }
    }
    /*end generate interest account wise*/
...
public function deleteInterest(Request $request){
 $previousdata=AccOTSRegisterInterest::find($request->interestId);
...
public function getInterestInfo(Request $request){
    ...
    $branchIds = $interestDetails->pluck('branchId_fk')->toArray();
...
public function storeOtsPayment(Request $request){
    ...
    for ($i=0; $i<$array_size; $i++){
        ...
        $accId= (int) json_decode($request->accId[$i]);
            ...
            $inetestRate = (int) DB::table('acc_ots_account')->where('id',$accId)->value('interestRate');
        ...
        $logArray = array(
            'moduleId'  => 4,
            'controllerName'  => 'AccOtsRegisterController',
            'tableName'  => 'acc_ots_payment',
            'operation'  => 'insert',
            'primaryIds'  => [DB::table('acc_ots_payment')->max('id')]
        );
        Service::createLog($logArray);
...
$logArray = array(
    'moduleId'  => 4,
    'controllerName'  => 'AccOtsRegisterController',
    'tableName'  => 'acc_ots_principal_payment',
    'operation'  => 'insert',
    'primaryIds'  => [DB::table('acc_ots_principal_payment')->max('id')]
);
Service::createLog($logArray);
return response::json('success');
}
...
{  
  $previousdata = AccOTSRegisterPrincipalPayment::find($request->DMpaymentId);
...
public function storeOtsRegisterPeriod(Request $request){
    ...
    if ($validator->fails()){
        ...
        $logArray = array(
            'moduleId'  => 4,
            'controllerName'  => 'AccOtsRegisterController',
            'tableName'  => 'acc_ots_period',
            'operation'  => 'insert',
            'primaryIds'  => [DB::table('acc_ots_period')->max('id')]
        );
        Service::createLog($logArray);
        return response::json('success');
    }
