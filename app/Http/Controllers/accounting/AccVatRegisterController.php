...
class AccVatRegisterController extends Controller
{
	...
	{
		$supplierLists = DB::table('gnr_supplier')->select('id','name')->get();
		$billTypes     = DB::table('acc_vat_bill_type')->select('id','serviceName')->get();
		$projects      = DB::table('gnr_project')->select('id','name')->get();
		$filteringArr  = array(
	...
	public function vatCalculationFromBillType(Request $request)
	{
		...
		$vatAmount= DB::table('acc_vat_bill_type')->select('vatRate')->where('id',$billType)->get();
...
public function accAddVatregister(Request $request)
{
	...
	$ledgerName                       = DB::table('acc_account_ledger')->where('id',$request->ledger)->select("name")->first();
	$addVatRegister->ledger           = $ledgerName->name;
	$addVatRegister->save();
	$logArray = array(
		'moduleId'  => 4,
	...
	elseif($count>=100)
        ...
        //   $viewVatBillTypes->activeFromGeneral=Carbon::parse($viewVatBillTypes->activeFrom)->format('d-m-Y');
			...
			public function accAddVatregisterPayVat(Request $request)
			{
				...
				$vatPayment->supplierIdFk=$request->PMsupplierId;
				$vatPayment->projectId_Fk=$request->PMprojectId;
				$vatPayment->project_TypeId_Fk=$request->PMprojectTypeId;
				$vatPayment->save();
			...
			{ //$hello=$request->test;
				$selectedBillType=DB::table('acc_vat_bill_type')
				->select('serviceName as billTypeName','id')
				...
				$billTypeArr=array(
					...
					'ajaxBiilType'     => $ajaxBiilType
				);
				return response()->json($billTypeArr);
			...
			public function accViewVatBillTypeGetAjax(Request $request)
			{
				...
				->where('t1.id',$ajaxId)
				->first();
				$ajaxResult->billDate=Carbon::parse($ajaxResult->billDate)->format('d-m-Y');
			...
			public function accViewVatBillRegisterFullUpdate(Request $request)
			{
				...
				$status= DB::table('acc_vat_generate')
				->where('id',$request->billTypeId)
				->update(['voucherDate' => $voucherDate,'billDate' => $billDate,'vatInterestRate' => $request->vatRate,'voucherNo' => $request->EMvoucherNo, 'ledger' => $request->EMAccountHead,'billTypeIdFk' => $request->billType,'billAmount' => $request->billAmount, 'vatAmount'=> $request->vatAmount ]);
			...
			public function accVatRegisterPaymentList()
			{
				...
				->join('acc_vat_generate as t3','t2.vatIdFk','=','t3.id')
				->select('t1.*','t2.billNo','t2.vatAmount','t3.billAmount','t3.vatInterestRate','t3.voucherDate','t3.ledger','t3.voucherNo')
				->get();

        // $paymentLists->paymentDate=Carbon::parse($paymentLists->paymentDate);
			 //$paymentLists->voucherDate=Carbon::parse($paymentLists->voucherDate)->format('d-m-Y');;
			...
			{   
				...
				$previousdata=DB::table('acc_vat_generate')
			...
			public function accVatRegisterGetLedgerAjax(Request $request)
			{
				$projectSelected=$request->projectId;
				$branchSelected=1;
				$allLedgers = DB::table('acc_account_ledger')->where('accountTypeId',13)->select("id","projectBranchId")->get();
				...
				$allLedgersR = DB::table('acc_account_ledger')->whereIn('id',$firstLedgerMatchedId)->where('isGroupHead',0)->select("id","name","code")->get();
				return response()->json($allLedgersR);
			}
